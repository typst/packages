...
#let get-vec-cross(a, b, width, inverse) = {
  ...
  let vec-cross = (
    vec-cross-normalized.at(0) * width,
    vec-cross-normalized.at(1) * width,
  )
...
#let spring(
  ...
  group(name: name, ctx => {
    ...
    anchor("top", (
      (a.at(0) + b.at(0)) / 2 - vec-cross.at(0) * r,
      (a.at(1) + b.at(1)) / 2 - vec-cross.at(1) * r,
    ))
    anchor("bottom", (
      (a.at(0) + b.at(0)) / 2 + vec-cross.at(0) * r,
      (a.at(1) + b.at(1)) / 2 + vec-cross.at(1) * r,
...
#let wall(
  ...
  for i in range(n) {
    ...
    let start = (
      a.at(0) * (1 - rstart) + b.at(0) * rstart,
      a.at(1) * (1 - rstart) + b.at(1) * rstart,
    )
    let end = (
      a.at(0) * (1 - rend) + b.at(0) * rend + vec-cross.at(0),
      a.at(1) * (1 - rend) + b.at(1) * rend + vec-cross.at(1),
    )
    points.push(start)
    points.push(end)
    points.push(start)
