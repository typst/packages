...
// in-outline-labeled() and multicaption() helper functions // {{{
// adapted from user /u/Andrew:
// https://forum.typst.app/t/how-do-i-customize-outline-entries-it-inner-it-body/3591/7
// enables distinct captions in figures vs. outline entries
#let in-outline-labeled(start-label, end-label) = {
  let after-start = query(selector(start-label).before(here())).len() == 1
  let before-end = query(selector(end-label).after(here())).len() == 1
  after-start and before-end
}

#let multicaption(short, details) = context {
  let inside = (
    in-outline-labeled(<outline-tables-start>, <outline-tables-end>) or
    in-outline-labeled(<outline-figures-start>, <outline-figures-end>) or
    in-outline-labeled(<outline-plates-start>, <outline-plates-end>)
  )
  if inside { short } else { [#short #details] }
}
//  // }}}
...
#let init-list-of-tables() = {
  ...
  show outline: it => {
    [#metadata(none)<outline-tables-start>#it#metadata(none)<outline-tables-end>]
  }
  outline(
    title: [],
...
#let init-list-of-plates() = {
  ...
  show outline: it => {
    [#metadata(none)<outline-plates-start>#it#metadata(none)<outline-plates-end>]
  }
  outline(
    title: [],
...
#let init-list-of-figures() = {
  show heading: set block(above: 0em, below: 0.5em)
  show outline: it => {
    [#metadata(none)<outline-figures-start>#it#metadata(none)<outline-figures-end>]
  }
