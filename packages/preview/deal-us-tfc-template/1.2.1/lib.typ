...
#let TFC(
  ...
  #show figure.where(kind: image): set figure(numbering: it => numbering("1.1", counter(heading).get().at(0), it))
  #show figure.where(kind: table): set figure(numbering: it => numbering("1.1", counter(heading).get().at(0), it))
  #show figure.where(kind: "todo"): set figure(numbering: it => numbering("1.1", counter(heading).get().at(0), it))
  ...
  #[
    ...
    #text(size: 13pt)[*#if portada-ingles { "For the degree of" } else { "Para la obtención del título de" }*]\
    #[#set text(size: 14pt)
      #titulacion]
    #v(0.2in)

    #text(size: 13pt)[*#if portada-ingles { "Directed by" } else { "Dirigido por" }*]\
    #[#set text(size: 14pt)
      #director]
    #v(0.2in)

    #text(size: 13pt)[*#if portada-ingles { "In the department of" } else { "En el departamento de" }*]\
    #[#set text(size: 14pt)
      #departamento]
    #v(0.6in)
  ...
  #show heading.where(level: 6): it => {
    text(fill:red, [*QUEDA TERMINANTEMENTE PROHIBIDO USAR ESTE NIVEL DE ENCABEZADO*])
  }
  ...
  #show ref: it => {
    ...
    if el != none and el.func() in (figure, math.equation, heading) {
      if (el.func() == heading and el.level == 1) {
        ...
        link(el.location(), [#if(secciones-ingles){"Chapter"} else {"Capítulo"} #num])
      } else {
        link(el.location())[#it]
      }
