...
De manera similar, '+' permite crear listas numeradas

+ Elaborar la memoria del TFG
+ Elaborar la presentación
+ Presentar el TFG
+ Solicitar el título de Grado
...
Es posible incluir un TODO bonito usando la función ```Typst #todo(inline:false, contenido)```, que muestra el texto incluido dentro de un recuadro con un fondo de color. Por ejemplo:

#todo([Aquí un todo en bloque. El texto podría ser cualquier cosa.

  #todo("Incluso otro TODO dentro")

  Ojo, para usar la función, hay que importar el paquete al comienzo del documento.

  Se puede incluir una label al llamar la función para luego referenciarlos, como esta referencia al @todo:inline.

  Por último, si quieres que aparezca un índice con todos los TODOs del documento, puedes pasar el parámetro ```Typst incluye-toc-codigos: true``` al objeto TFC en el archivo principal. Esto hará que todos los TODOs se incluyan en el índice general del documento, con un enlace a la página donde se encuentran.
])

También se pueden incluir TODOs en línea #todo(inline: true, "Como este, cuando se quiere resaltar algo sobre una palabra o frase concreta.", label: <todo:inline>)
