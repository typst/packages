...
#example(
  [```typst
  #ctz-canvas(length: 0.75cm, clip-canvas: (-0.5, -0.5, 5.5, 4), {
    import cetz.draw: *
    ctz-init()
...
#example(
  ...
  #ctz-canvas(length: 0.75cm, {
    ...
    ctz-draw(line: ("A", "B", "C", "A"), stroke: black)
    ctz-draw-line-add("A", "Ha1", add: (2, 2), stroke: red + 0.5pt)
    ctz-draw-line-add("B", "Hb1", add: (2, 2), stroke: red + 0.5pt)
    ctz-draw-line-add("C", "Hc1", add: (2, 2), stroke: red + 0.5pt)
