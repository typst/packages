...
- `"fancy"` adds edge accents and cast shadows, giving patterned structures a
  stronger sense of depth.

In fancy shading, patterned features can cast shadows onto the layers beneath
them and onto other parts of the same layer. A dictionary customizes the style:

```typ
shading: (
  mode: "fancy",
  edge-accent: (top: 2.5%, bottom: 0%),
)
```

`shading: "fancy"` uses the values above. An edge-accent value accepts numbers
in units or ratios of the layer thickness. It changes the rendered edge without
changing the geometry used by later process steps. A material dictionary can
override it with `edge-accent: none`. Physical edge-profile facets provide the
edge treatment wherever they are present.
...
They resolve the same way. The most specific setting wins: a per-layer material
dictionary first, then the stack value here, then the material's palette entry.
`auto`, the default at every level, defers to the next; a stroke of your own
sets the style; and `none` leaves the edge undrawn. 
