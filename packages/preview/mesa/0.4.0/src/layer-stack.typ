...
#let _layer(
  ...
  if deposition == "planar" {
    ...
    assert(
      ...
      message: "planar-layer height must be a number",
    )
  } else {
  ...
  cetz.draw.get-ctx(ctx => {
    ...
    let placements = if deposition == "conformal" {
      ...
      let substrate-top = state.layers.values().first().top
      let target-top = substrate-top + height
      assert(
        target-top > 0,
        message: "planar-layer height places its surface below the sample",
      )
    ...
    // Each edge stroke resolves local-style > stack > palette. `auto` at any
    // level defers to the next; an explicit `none` draws nothing. Only when
    // every level defers does the edge fall back to `_true-edge-stroke`.
    for key in ("stroke", "internal-stroke", "edge-accent-stroke") {
      ...
      resolved-style.insert(
        key,
        if chosen.len() == 0 { _true-edge-stroke } else { chosen.first() },
      )
    }
    ...
    let edge-accent = resolved-style.at("edge-accent", default: auto)
    ...
    let bevel = if state.shading == "fancy" {
      _bevel-config(
        if edge-accent == auto { state.edge-accent } else { edge-accent },
        bevel-thickness,
        width,
        depth,
    ...
    cetz.draw.set-ctx(ctx => {
      ...
      let opaque = resolved-style.at(
        "opaque",
        default: type(fill) != color or fill.components().last() == 100%,
      )
...
#let layer-stack(
  ...
  assert(type(shading) in (str, dictionary), message: "invalid shading value")
  let shading-mode = if type(shading) == str {
    shading
  } else {
    shading.at("mode", default: "flat")
  }
  assert(
    shading-mode in ("none", "flat", "fancy"),
    message: "unknown shading mode",
  )
  let edge-accent = if type(shading) == dictionary {
    shading.at("edge-accent", default: (top: 2.5%, bottom: 0%))
  } else {
    (top: 2.5%, bottom: 0%)
  }
