...
#import "scene.typ": (
  topology-debug-styles as _topology-debug-styles,
  section-debug-styles as _section-debug-styles,
)
  ...
  align(center)[
    #text(size: 7pt)[
      #grid(
        ...
        [#line(length: 4mm, stroke: _topology-debug-styles.accent) edge accent],
        [#line(length: 4mm, stroke: _topology-debug-styles.smooth) smooth],
      )
    ]
    #v(3mm)
    #_layer-stack(body, debug: true, ..options)
  ]
}

/// Render a cross-section with each layer drawn one piece at a time, so the
/// stacked pieces a deposit or etch leaves behind — and any phantom edge
/// between them — are visible. Requires a `section:` plane.
#let section(body, ..options) = {
  let options = options.named()
  assert("debug" not in options, message: "section debug controls the debug mode")
  assert("section" in options, message: "section debug needs a section: plane")
  align(center)[
    #text(size: 7pt)[
      #grid(
        columns: 3,
        gutter: 3mm,
        [#line(length: 4mm, stroke: _section-debug-styles.outline) outline],
        [#line(length: 4mm, stroke: _section-debug-styles.internal) internal],
        [#box(circle(radius: 1.5pt, fill: _section-debug-styles.vertex, stroke: none)) vertex],
      )
    ]
    #v(3mm)
    #_layer-stack(body, debug: true, ..options)
