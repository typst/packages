...
#let surface-deposit(
  ...
  let result = cbor(geometry-kernel.surface_deposit(cbor.encode((
    version: protocol-version,
    volumes: volumes.enumerate().map(
      ((index, volume)) => _encode-volume(volume, index),
    ),
    sources: sources.enumerate().map(
      ((index, volume)) => _encode-volume(volume, index),
    ),
    bounds: _encode-shapes(bounds),
    thickness: int(calc.round(thickness * grid-scale)),
    sidewall-distance: int(calc.round(sidewall-thickness * grid-scale)),
    profile-nodes: profile-nodes.map(node => (
      distance: int(calc.round(node.distance * grid-scale)),
      inset: int(calc.round(node.inset * grid-scale)),
    )),
  ...
  result.bands.map(band => (
    bottom: band.bottom / grid-scale,
    top: band.top / grid-scale,
    lower: _decode-shapes(band.lower),
    upper: _decode-shapes(band.upper),
  ))
}
