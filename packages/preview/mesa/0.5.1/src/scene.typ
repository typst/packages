...
#let _contour-corner-count(contour) = {
  let count = 0
  for index in range(contour.len()) {
    let previous = contour.at(calc.rem(index + contour.len() - 1, contour.len()))
    let point = contour.at(index)
    let next = contour.at(calc.rem(index + 1, contour.len()))
    let cross = (
      (point.at(0) - previous.at(0)) * (next.at(1) - point.at(1))
        - (point.at(1) - previous.at(1)) * (next.at(0) - point.at(0))
    )
    if calc.abs(cross) >= 0.5 / (kernel.grid-scale * kernel.grid-scale) {
      count += 1
    }
  }
  count
...
#let _same-corner-structure(left, right) = {
  if not _same-structure(left, right) { return false }
  for (shape-index, left-shape) in left.enumerate() {
    for (contour-index, left-contour) in left-shape.enumerate() {
      if _contour-corner-count(left-contour) != _contour-corner-count(
        right.at(shape-index).at(contour-index),
      ) {
        return false
      }
    }
...
// Deposition follows the exterior of the sample, not the boundaries between
// its material volumes. Rebuild the occupied scene as one height-varying solid
// before offsetting it; the original volumes remain untouched and continue to
// carry material identity for rendering and later process steps.
#let _scene-solid(volumes) = {
  let heights = ()
  for volume in volumes {
  ...
  for index in range(heights.len() - 1) {
    ...
    let lower = _scene-shapes-at(volumes, bottom, side: "above")
    let upper = _scene-shapes-at(volumes, top, side: "below")
    if lower.len() > 0 and upper.len() > 0 {
      bands.push((bottom: bottom, top: top, lower: lower, upper: upper))
    }
  }
  _bands-to-volumes(bands)
}
...
// Cross-section of the offset kernel used to smooth a conformal shell. The
// profile occupies the outermost `profile.size` of the deposition thickness;
// below that, the full lateral thickness is retained. Mirroring these nodes
// around a source volume's top and bottom rounds upward- and downward-facing
// exposed edges in exactly the same way.
#let _conformal-profile-nodes(profile, thickness) = {
  let nodes = (
    (distance: 0, inset: 0),
    (distance: thickness - profile.size, inset: 0),
  )
  if profile.profile == "chamfer" {
    nodes.push((distance: thickness, inset: profile.size))
    return nodes
  }
  ...
  for index in range(profile.facets + 1) {
    ...
    nodes.push((
      distance: thickness - profile.size + bulge * calc.sin(angle),
      inset: profile.size - bulge * calc.cos(angle),
    ))
  }
  nodes
}
...
#let _surface-deposit(
  ...
  let profile-nodes = if edge-profile == none { () } else {
    _conformal-profile-nodes(edge-profile, thickness)
  }
  _bands-to-volumes(kernel.surface-deposit(
    volumes,
    _scene-solid(volumes),
    bounds,
    thickness,
    sidewall-thickness,
    profile-nodes: profile-nodes,
    coat-bottoms: coat-bottoms,
    profile-boundary: profile-boundary,
  ))
}

#let conformal-deposit(
  volumes,
  bounds,
  thickness,
  edge-profile: none,
...
#let deposit(
  ...
  if volumes.len() == 0 {
    ...
    if edge-profile == none { return (initial,) }
    let sections = _conformal-profile-nodes(edge-profile, thickness).map(node => (
      height: node.distance,
      shapes: if node.inset == 0 {
        shapes
      } else {
        kernel.offset(shapes, -node.inset)
      },
    ))
    assert(
      sections.last().shapes.len() > 0,
      message: "edge-profile removes the complete top surface",
    )
    initial.shapes = kernel.merge(sections.map(section => section.shapes).flatten())
...
#let _etch-tool(source, depth, isotropic, sidewall) = {
  ...
  if isotropic > 0% {
    ...
    let normalized-offset(distance) = {
      let shapes = kernel.offset(source.shapes, distance)
      if shapes.len() == 0 { () } else { kernel.merge(shapes) }
    }
    let lower = normalized-offset(lateral)
    let actual-depth = depth
    // An inward-sloping etch ends where its walls meet. Keep the last
    // non-empty grid slice as the tip instead of requiring an opening at the
    // requested depth below it.
    if lateral < 0 and not _same-corner-structure(lower, source.shapes) {
      let open = 0.0
      let closed = -lateral
      lower = source.shapes
      while closed - open > 1 / kernel.grid-scale {
        let middle = (open + closed) / 2
        let candidate = normalized-offset(-middle)
        if not _same-corner-structure(candidate, source.shapes) {
          closed = middle
        } else {
          open = middle
          lower = candidate
      ...
      actual-depth = open * calc.tan(angle)
    }
    sections = (
      (height: _grid-height(source.height - actual-depth), shapes: lower),
...
#let _grid-residue-edge(edge) = {
  let squared-length = 0.0
  for axis in range(3) {
    let distance = edge.end.at(axis) - edge.start.at(axis)
    squared-length += distance * distance
  }
  squared-length <= calc.pow(4 / kernel.grid-scale, 2)
}

#let _normal-edge-role(edge, volumes) = {
  // Polygon offsets close on the integer geometry grid and can leave a final
  // edge only a few cells long. Drawing that numerical remnant with the normal
  // device stroke turns a point closure into a visible hole.
  if edge.visibility == "occluded" or _grid-residue-edge(edge) {
    none
  } else {
...
// Express a planar outline in coordinates along and perpendicular to a section
// line. The cross-section kernel can then intersect every vertical plane at a
// constant second coordinate while preserving distances along the plane.
#let _section-shapes(shapes, plane) = {
  let (start, end) = plane
  ...
  let ux = dx / length
  let uy = dy / length
  shapes.map(shape => shape.map(contour => contour.map(point => {
    let x = point.at(0) - start.at(0)
    let y = point.at(1) - start.at(1)
    (x * ux + y * uy, -x * uy + y * ux)
  })))
}

// One volume's contribution to a vertical cross-section: a trapezoid for
...
#let _volume-section-quads(volume, plane) = {
  let quads = ()
  ...
  for plane-index in range(planes.len() - 1) {
    ...
    let (lower-shapes, upper-shapes) = _shared-contours(
      _section-shapes(lower.shapes, plane),
      _section-shapes(upper.shapes, plane),
    )
    let plane-lower-intervals = kernel.cross-section(lower-shapes, 0)
    let plane-upper-intervals = kernel.cross-section(upper-shapes, 0)
    if plane-lower-intervals.len() == plane-upper-intervals.len() {
      for interval-index in range(plane-lower-intervals.len()) {
        let lower-interval = plane-lower-intervals.at(interval-index)
        let upper-interval = plane-upper-intervals.at(interval-index)
        quads.push(((
    ...
    let heights = (lower.height, upper.height)
    // A sloped hole can enter this section between the volume's own section
    // planes. Split at every height where a moving polygon vertex crosses the
    // section plane, so each resulting strip keeps one interval
    // topology throughout.
    for (shape-index, lower-shape) in lower-shapes.enumerate() {
      let upper-shape = upper-shapes.at(shape-index)
      for (contour-index, lower-contour) in lower-shape.enumerate() {
        let upper-contour = upper-shape.at(contour-index)
        for (point-index, lower-point) in lower-contour.enumerate() {
          let upper-point = upper-contour.at(point-index)
          let lower-y = lower-point.at(1)
          let upper-y = upper-point.at(1)
          if 0 > calc.min(lower-y, upper-y) and 0 < calc.max(lower-y, upper-y) {
            let amount = -lower-y / (upper-y - lower-y)
            let height = _grid-height(
              lower.height + (upper.height - lower.height) * amount,
            )
            if height > lower.height and height < upper.height and height not in heights {
              heights.push(height)
            }
          }
        }
    ...
    for strip-index in range(heights.len() - 1) {
      let bottom = heights.at(strip-index)
      let top = heights.at(strip-index + 1)
      let inset = calc.min(
        1 / kernel.grid-scale,
        (top - bottom) / 4,
      )
      let lower-intervals = kernel.cross-section(
        _section-shapes(_volume-shapes-at(volume, bottom + inset), plane),
        0,
      )
      let upper-intervals = kernel.cross-section(
        _section-shapes(_volume-shapes-at(volume, top - inset), plane),
        0,
      )
      assert(
        lower-intervals.len() == upper-intervals.len(),
        message: "cross-section topology changes between polygon events",
      )
      for interval-index in range(lower-intervals.len()) {
        ...
        quads.push(((
          (lower-interval.first(), bottom),
          (lower-interval.last(), bottom),
          (upper-interval.last(), top),
          (upper-interval.first(), top),
        ),))
      }
    }
  }
...
#let render-section(volumes, plane) = {
  for (index, volume) in volumes.enumerate() {
    _validate-volume(volume, index)
  }
  ...
  for (index, volume) in volumes.sorted(key: volume => volume.bottom).enumerate() {
    ...
    item.shapes += _volume-section-quads(volume, plane)
...
#let render-section-debug(volumes, plane) = {
  for (index, volume) in volumes.enumerate() {
    _validate-volume(volume, index)
  }
  let pieces = ()
  for volume in volumes.sorted(key: volume => volume.bottom) {
    let quads = _volume-section-quads(volume, plane)
    if quads.len() > 0 {
