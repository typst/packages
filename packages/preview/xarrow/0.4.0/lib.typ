...
#let xarrow(
  ...
  size: 100% + 0.15em, // size of the arrow, relative to its longest decoration
  ...
  let size = if type(size) == relative {
    size
    ...
    panic("xarrow: incorrect type for `size`")
  }

  let repeated_width = (
    body_width * size.ratio + size.length.to-absolute()
    - (100% - end + begin) * sym_w
  )

  // Repeat the body to fit the given width
  let fit_width(width, body) = {
