...
// a NKU Touying theme.
// Authors: Jimmyzzt
// Modified From ShuimuTouying theme (https://github.com/JSB-Unscarred/shuimu-touying/tree/main)
...
#let _mini-frames-navigation(self: none) = {
  ...
  context {
    ...
    block(
      width: 100%, 
      fill: primary-color, 
      inset: (top: 0.6em, bottom: 0.4em, x: 2em, y: 2em),
      {
        set text(size: 0.7em)
...
#let title-slide(config: (:), ..args) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(
    ...
    config-page(
      fill: self.colors.primary,
      // margin: 2em,
      header: none, // 封面页不显示章节导航栏
      footer: none,
      background: {
        image("nku-title-background_3.pdf", fit: "stretch", width: 100%, height: 100%)
      }
    ),
  )
    ...
    block(
      ...
      breakable: false,
      {
        ...
        for (prefix, persons) in person-entries {
          align(center,
            grid(
              ...
              column-gutter: 0.5em,
              row-gutter: 0.5em,
              text(fill: white, prefix),
              ..persons.map(p => text(fill: white, p)),
            )
          )
        }
        v(0.5em)
        ...
        if info.institution != none {
          parbreak()
          align(center,
            text(fill: self.colors.neutral-lightest, size: 0.8em, info.institution)
          )
        }
        // date
        if info.date != none {
          parbreak()
          align(center, 
            text(fill: self.colors.neutral-lightest, size: 0.8em, utils.display-info-date(self))
          )
        }
      },
    )
...
#let nankai-touying-theme(
  ...
  footer-b: self => if self.info.author != none {
    self.info.author
  } else {
  ...
  show: touying-slides.with(
    ...
    config-colors(
      primary: rgb("#711A5F"), //参考了南开大学校色(https://news.nankai.edu.cn/xsw/system/2013/10/16/000146278.shtml)
      primary-dark: rgb("#420d38"),
      ...
      footer: self => {
        ...
        grid(
          columns: (
            ...
            if footer-b != none { 20% } else { 0pt },
            1fr,
            5em,
          ),
          rows: (1.5em, auto),
          cell(fill: self.colors.primary, if footer-a != none { footer-a }),
          cell(fill: self.colors.primary-dark, if footer-b != none { footer-b }),
