...
#let concat-adjacent-text(children) = {
  ...
  let as-text(el) = {
    let fn = repr(el.func())
    if fn == "text" { el.text } else if fn == "space" { " " } else if (
      fn in "linebreak"
    ) { "\n" } else if (
      fn in "parbreak"
...
#let interpret-exclude-patterns(exclude) = {
  exclude.map(element-fn => {
    if type(element-fn) in (str, label, dictionary) { element-fn } else if (
      type(element-fn) == function
    ) {
      repr(element-fn)
      ...
      panic(
        "Exclude patterns must be element functions, strings, or labels; got:",
        element-fn,
      )
    }
  })
...
#let map-tree(f, content, exclude: IGNORED_ELEMENTS) = {
  ...
  let exclude-selectors = exclude.filter(e => (
    type(e) == dictionary and "element-fn" in e
  ))
  for (element-fn, fields) in exclude-selectors {
    if fn == element-fn {
      if not fields
        .pairs()
        .any(((key, value)) => content.at(key, default: value) != value) {
        return none
      }
    }
  }
  ...
  if fn in exclude {
    ...
    fn in ("acr", "acrl", "acrs", "acrf", "acrpl", "acrlpl", "acrspl", "acrfpl")
  ) {
    ...
    for key in fields {
      ...
      if type(value) == str {
        if result == none { result = f(value) } else { result += f(value) }
      } else if type(value) == content {
        let subtree_result = map-subtree(value)
        if subtree_result != none {
          if result == none { result = subtree_result } else {
