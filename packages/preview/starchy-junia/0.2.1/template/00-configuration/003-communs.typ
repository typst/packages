...
#let junia-core(
  ...
  // Numéro court d'un élément référencé (figure/tableau/équation), résolu à
  // l'emplacement de l'élément lui-même — et non de la référence — pour
  // rester correct en cas de citation depuis un autre chapitre.
  // `motif-chapitre` ("1.1", "(1.1)"…) n'est utilisé que pour le cas
  // chapitre.numéro du corps principal, où `el.numbering` est une fonction
  // qui résout `counter(heading).get()` à l'emplacement de la référence —
  // donc au mauvais chapitre — plutôt qu'à celui de l'élément. Dans les
  // autres zones (pré-sommaire, annexes, junia-light, équations non
  // numérotées…) `el.numbering` est directement une chaîne ou `none`, et on
  // le respecte tel quel.
  let numero-court(el, compteur, motif-chapitre) = {
    let loc = el.location()
    let num = el.numbering
    if type(num) == function {
      let chap-num = counter(heading).at(loc).first()
      let item-num = compteur.at(loc).first()
      numbering(motif-chapitre, chap-num, item-num)
    } else if num != none {
      numbering(num, ..compteur.at(loc))
    } else {
      none
    }
  }
    ...
    if el != none {
      ...
      if el.func() == figure and el.kind == image {
        return context {
          ...
          let num = numero-court(el, counter(figure.where(kind: image)), "1.1")
          link(el.location())[#sup~#num]
        }
      }
      ...
      if el.func() == figure and el.kind == table {
        return context {
          ...
          let num = numero-court(el, counter(figure.where(kind: table)), "1.1")
          link(el.location())[#sup~#num]
        }
      }
      ...
      if el.func() == math.equation {
        return context {
          ...
          let num = numero-court(el, counter(math.equation), "(1.1)")
          link(el.location())[#sup~#num]
        }
      }
    }
