...
#import "../Ressources/E_Lexique.typ": v_liste-glossaire

// === Déclaration de l'état global pour la langue ===
#let v_langue_global = state("langue_document", "fr")

// ╔══════════════════════════════════════╗
// ║         VARIABLES GLOBALES           ║
// ╚══════════════════════════════════════╝
// Adresse de JUNIA
#let v_adresse_junia = [JUNIA \ 2 Rue Norbert Ségard \ 59014 Lille cedex]
// Gestion de la date du rapport de manière automatique
#let v_date_du_jour = datetime.today()
#let v_mois_long = (
...
#let v_mois_court = v_mois_long.at(v_date_du_jour.display("[month repr:long]"))
// Récupération du mois anglais
#let v_month_short = v_date_du_jour.display("[month repr:long]")
// Récupération de l'année
#let v_annee_court = v_date_du_jour.display("[year repr:full]")
...
#let liste_trad_auto = (
  ...
  // Variables (e.g Mois)
  mois_date:        ("fr": v_mois_court,                  "en": v_month_short)  
) 
...
#let f_lexique(body) = context {
  // Gestion de la traduction automatique du titre
  heading(level: 1, outlined: false)[#liste_trad_auto.at("lexique").at(v_langue_global.get())]
  print-glossary(
    v_liste-glossaire,
...
#let f_sommaire(body) = context {
  outline(
    // Gestion de la traduction automatique du titre
    title: liste_trad_auto.at("sommaire").at(v_langue_global.get()),
    depth: 3,
    indent: auto,
...
#let f_liste_figure(body) = context { 
  outline(
    // Gestion de la traduction automatique du titre
    title: liste_trad_auto.at("sommaire_fig").at(v_langue_global.get()), 
...
#let f_liste_tableau(body) = context { 
  outline(
    // Gestion de la traduction automatique du titre
    title: liste_trad_auto.at("sommaire_tab").at(v_langue_global.get()), 
...
#let f_bibliographie(body) = context {
  bibliography(
    (
    ...
    // Gestion de la traduction automatique du titre
    title: liste_trad_auto.at("bibliographie").at(v_langue_global.get()),
...
#let f_corps_document(body) = context {
  // Variable pour la page
  let v_page_app = liste_trad_auto.at("page_app").at(v_langue_global.get())
  // Variable pour le délimiteur
  let v_delim_app = liste_trad_auto.at("delim_app").at(v_langue_global.get())
  // Paramétrage du pied de page
  set page(
    numbering: "1",
    header: f_afficher_titre(), 
    footer: context {
      line(length: 100%, stroke: 0.4pt)
      // Gestion de la traduction automatique du délimitateur pour l'indication du nombre de page
      align(right)[
        #v_page_app #counter(page).display() #v_delim_app #counter(page).final().first()
      ]
    }
  )
  ...
  set heading(
    ...
    supplement: liste_trad_auto.at("chap_sup").at(v_langue_global.get()),
    numbering: (..nums) => {
...
#let f_annexes(body) = context {
  ...
  set figure(
    ...
    supplement: liste_trad_auto.at("annexe_fig").at(v_langue_global.get()),
    // Numérotation
    numbering: "1",
...
#let modele_junia(
  ...
  type_rapport:         [],
  titre_rapport:        [],             // Utile pour les métadonnées
  confidentialite :     [],
  type_diplome:         [],
  professeur:           [],
  encadrant:            [],
  ...
  promotion:            [],
  niveau_classe:        [],
  mois_annee:           auto,           // Affiche la date de manière automatique dans la page de garde
  ...
  math_num : false,                     // Permet de numéroté les équations mathématiques si besoin
  lang_doc: "fr",                       // Langue du document
  doc,                                  // Obligatoire
) = {

  // === Mise à jour de la variable global de langue ===
  // On vérifie que la langue existe, sinon on repasse en français
  if (liste_trad_auto.sommaire.keys().contains(lang_doc) == false) {lang_doc = "fr"}
  // Mise à jour de la langue global
  v_langue_global.update(lang_doc)
  
  // === Métadonnées PDF ===
  // Quand on ouvre pas le document mais qu'on passe simplement le curseur dessus
  set document(title: titre_rapport, author: auteurs.join(",")) 
    ...
    // Filigrane de confidentialité, en haut à gauche (sauf page de garde pdf)
      ...
      #place(
        ...
        rect(
          ...
          inset: 5pt,
          text(fill: rgb(255, 51, 51), size: 0.9em, weight: "bold")[#context liste_trad_auto.at("secret").at(v_langue_global.get())]
        )
      )
    ]
    // Filigrane "BROUILLON" géant, au centre de toutes les pages (sauf page de garde pdf)
    #if ebauche [
      #place(center + horizon, rotate(45deg, text(6.5em, gray.lighten(70%))[#context liste_trad_auto.at("brouillon").at(v_langue_global.get())]))
    ]
  ]
  ...
  let f_style_link(it, kind: "default") = {
    let color = v_style_des_liens.at(kind, default: none)
    if color != none {
      text(fill: color, it)
    } else {
      it
  ...
  show: make-glossary
  register-glossary(v_liste-glossaire)
  ...
  show ref: it => {
    ...
    if el != none {
      ...
      if "kind" in el.fields() and el.kind == "glossarium_entry" {
        return f_style_link(it, kind: "glossaire")
      }
      // Figure de type "annexe"
      if el.func() == figure and el.kind == "annexe" {
        return f_style_link(it, kind: "annexe")
      }
    }
    // Autres
    it 
  ...
  show cite: it => f_style_link(it, kind: "citation")
  ...
  set math.equation(numbering: if math_num == true {
  ...
  show heading: it => {
    ...
    let v_taille = (1.75em, 1.6em, 1.4em, 1.25em, 1.1em, 1em).at(it.level - 1, default: 1em)
    // On affecte la liste précédente pour les tailles des titres
    set text(size: v_taille, weight: "bold")
    ...
    block(
      ...
      if it.numbering != none {
        // Si un numéro existe on l'affiche (choix d'affiche dans la fonction : `f_corps_document`)
  ...
  if generate-cover {
    ...
    place(center+horizon)[
      #type_rapport #v(0.125em)
      #text(2em, weight: "bold")[#titre_rapport] #v(0.25em)
      #text(1.1em, fill: red, weight: "bold")[#confidentialite] #v(0.75em)
      #type_diplome #v(0.5em)
    ...
    align(bottom, grid(
      ...
      [],
      [],
      [#promotion #niveau_classe],
      [],
      [
        #align(right)[
          #context if (mois_annee == auto) {
            text()[#liste_trad_auto.at("mois_date").at(v_langue_global.get()) #v_annee_court]
          } else {text()[#mois_annee]}]
      ]
    ))
  ...
  set page(
    ...
    footer: context {
      line(length: 100%, stroke: 0.4pt)
      align(right)[
        #liste_trad_auto.at("page_app").at(v_langue_global.get()) #counter(page).display("I")
      ]
    },
    background: f_fond_de_page
  )
  ...
  context if ebauche {note-outline(title: [#liste_trad_auto.at("sommaire_ebauche").at(v_langue_global.get())])}
