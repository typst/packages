...
#let scale-9patch(image-data, target-width, target-height, scale: 1pt) = {
  // Force 4-byte little-endian encoding for u32
  let width-bytes = int(target-width/scale).to-bytes(endian: "little", size: 4)
  let height-bytes = int(target-height/scale).to-bytes(endian: "little", size: 4)
