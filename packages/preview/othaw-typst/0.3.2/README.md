...
OTH AW prescribes a very specific order for the forms and multiple title pages. In order to maintain the basic idea of clean-dhbw, increase usability, and move formalities to the appendix instead of the front matter, the option *formalities-in-frontmatter* (default true) has been added. This makes it quick and easy to generate a version during the editing process, e.g., for supervisors, which has less overhead in the front matter and therefore requires less scrolling. Please make sure to set the variable back to true before submission.
