...
in the text for the first time. Upper case and lower case letters mean different
things in symbols and units of measurement. Do not use the same symbol to
mean different things, but note that different styles applied over the same
symbol are accepted: if $vector(x)$ is a position vector, then the scalar
$x = norm(vector(x))$ could be used to denote its norm. If the position is
a random variable, then one could use yet another style such as fraktur form
$randomVec(x)$ of $vector(x)$ to eccentuate this fact.
...
texts_~@helsinki-saavutettavuusmalli @saavutettavasti-vaihtoehtoiset-tekstit
@tuni-intra-image-accessibility (which are not the same thing as figure
captions) should be included into a document. However, if this is not feasible
due to current technological limitations, one should at least try to describe
what is happening in each image or table in the body text as well as they can.
The Typst function `image` accepts the named argument `alt`, which allows you to
provide an alternative text for an included image~@typst-image-function.
...
embedding of MathML~@pdf-association-wtpdf @mozilla-mathml into PDF files to
catch up, there are some simple things you can do to make your thesis more
accessible.
...
$
you could force the integration domain $[a,b]$ onto the same line with the
integral itself by using custom restriction notation, such as
$
    restriction(
        integral f(x) dif x,
        [a,b]
    ...
    thin .
$
With the subindex notation many PDF readers assume, that since the integration
domain $[a,b]$ is vertically below the rest of the line, it must be laid out
later in the document flow.

#import "../brakets.typ": braket

However, such custom notation should be used as an _absolute last resort_, to
keep the PDF document flow as horizontal as possible. Paul Dirac did a similar
thing with his bra--ket notation for linear algebra products, to be able to
write mathematics with a normal typewriter without access to special symbols
such as  greek letters, upper and lower indices and integrals, and just to make
long subindex labels related to linear operators such as integrals cleaner to
write~@dirac-new-notation:
$
    braket(i,j)
    &:=
    integral
    ...
    thin .
$
Here $phi$ and $psi$ denote wave functions, and the left-associative unary
operator $complexConjugate(a)$ denotes the complex conjugate of its argument
$a$. However, inventing your own notations should be done sparingly, since this
is against the guideline of sticking to well-established notations. Inventing
custom notations also carries with it the burden of having to explain yourself
more.

=== Convert your final submission to PDF/A format

If you are using Typst version ≥ #version(0,13,0) locally via a command line, it
is possible to convert a thesis into PDF/A-3b format via the command
```sh
typst compile --pdf-standard a-3b main.typ
```
The PDF/A-3b standard allows you to embed arbitrary files into your PDF file,
using Typst's PDF module function `pdf.embed`~@typst-documentation-embed.
As discussed below, Typst compilers before version #version(0,14,0) cannot
yet produce truly accessible documents, so including the source code of your
thesis chapters as plain text provides an alternative means of accessing your
thesis. This template has tried to demonstrate how this can be performed at the
beginning of each front- and main matter file, and in the example appendices.
If you are using the Typst Web application, simply choose PDF/A-3b as the output
format in the file export menu.

Using PDF/A-3b as the output format makes the internal document structure
...
*However*, the transition period for producing "truly accessible"
tagged documents ends at the end of 2025~@laki-306-2019 @oulu-acta-pdfa-2025
@helsinki-thesis-accessibility-2025. In 2026, it will be mandatory to convert
a thesis into at least PDF/A-2a format, meaning it needs to contain structural
information in the form of PDF tags. Typst version #version(0,14,0), which
might be released before the end of 2025 will be using the new Krilla PDF
backend~@typst-krilla-pull-request, which supports PDF/A-2a creation. Once a
version ≥ #version(0,14,0) of Typst is released, this template will be updated
to support it, and any users of this template will be required to compile their
theses with (at least) the `--pdf-standard a-2a` flag.

To test how well a piece of textual content works, try copying it from an
output PDF file and pasting the result to a plain text editor such as NotePad
