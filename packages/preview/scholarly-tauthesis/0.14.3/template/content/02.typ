...
is possible to convert a thesis into PDF/A-2b or PDF/A-3b format via the commands
```sh
typst compile --pdf-standard a-2b main.typ
# or
typst compile --pdf-standard a-3b main.typ
```
The only difference between these two is that the latter allows you to
embed arbitrary files into your PDF file, using Typst's PDF module function
`pdf.embed`~@typst-documentation-embed. If you are using the Typst Web
application, simply choose PDF/A as the output format in the file export menu.

Using PDF/A-2b as the output format makes the internal document structure
