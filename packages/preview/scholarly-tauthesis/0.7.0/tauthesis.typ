 ...
 *
 ***/
#let aiDisclaimerPage(usedAI,language) = [
		...
		#if usedAI [
			#h(1em, weak:false) #sym.ballot.x   Kyllä\
			#h(1em, weak:false) #sym.ballot Ei
		] else [
			#h(1em, weak:false) #sym.ballot   Kyllä\
			#h(1em, weak:false) #sym.ballot.x Ei
		]

		#if usedAI {
			include "template/content/tekoalyn-kaytto.typ"
		}
		...
		#if usedAI [
			#h(1em, weak:false) #sym.ballot.x Yes\
			#h(1em, weak:false) #sym.ballot   No
		] else [
			#h(1em, weak:false) #sym.ballot   Yes\
			#h(1em, weak:false) #sym.ballot.x No
		]

		#if usedAI {
			include "template/content/use-of-ai.typ"
		}
...
/** pagesettings(docpart, doc)
 *
 * Defines page dimensions in the main matter.
 *
***/
#let pagesettings(docpart,it) = context {
	...
	set page(
		...
		binding : {
			...
			if finalpagenumber > BINDINGPAGELIMIT and meta.physically-printed {
		...
		footer: {},
		margin: if finalpagenumber > BINDINGPAGELIMIT and meta.physically-printed {
			(
				top: 2.5cm,
				bottom: 2.5cm,
				inside: 4cm,
...
#let tauthesis(
	...
	show: doc => pagesettings(TITLEPAGEPART,doc)

	show: doc => titlepage(doc)

	PREFACEPAGENUMBERCOUNTER.update(1)

	show: doc => pagesettings(DOCPREAMBLEPART,doc)
	...
	aiDisclaimerPage(usedAI, meta.language)
	...
	show: doc => pagesettings(MAINMATTERPART,doc)
