...
#let outlineEntryShowRule(language, it) = {
	...
	let entry = if el.func() == figure {
		let (carray, supplement) = if el.body.func() == image {
			...
			(UNKNOWNCOUNTER.at(eloc), UNKNOWNSUPPLEMENT)
		}
		if carray != none {
			carray.last() += 1
			link(
 ...
 *
***/
#let pageSettings(docpart, physically-printed, language, displayLinkToToC, it) = context {
	...
	set page(
		...
		footer: if docpart >= MAINMATTERPART and not physically-printed {
...
#let template(
	...
	show: doc => pageSettings(TITLEPAGEPART, physically-printed, language, displayLinkToToC, doc)
	...
	show: doc => pageSettings(DOCPREAMBLEPART, physically-printed, language, displayLinkToToC, doc)
	...
	show: doc => pageSettings(MAINMATTERPART, physically-printed, language, displayLinkToToC, doc)
