...
#let refShowRule(it, language, figNumberWithinLevel, eqNumberWithinLevel : 1) = {
	...
	if el != none {
		...
		let refText = if el.func() == figure {
			...
			carray.last() += 1
			supplement + if supplement != [] { sym.space.nobreak } else { none } + array2SectionNumbering(carray,inAppendix or inPublicationAppendix)
		}
...
#let figShowRule(language,figNumberWithinLevel,it) = {
	...
	let caption = block(
		...
		layout( containerSize => {
			let caption = [*#supplement~#array2SectionNumbering(ctr,inAppendix or inPublicationAppendix).*~#it.caption.body]
