...
#let figShowRule(language,figNumberWithinLevel,it) = {
	...
	let fig = if it.kind == image {
		...
		block(it.body)
		block[
			#text( fill:titleColor)[*#it.supplement #array2SectionNumbering(ctr,inAppendix).* ]
			#it.caption.body
		]
	}
		...
		block(it.body)
		block[
			#text(fill:titleColor)[*#codeSupplement #array2SectionNumbering(ctr,inAppendix).* ]
			#it.caption.body
		]
	}
...
#let headingShowRule(language, figNumberWithinLevel, eqNumberWithinLevel, it) = {
	...
	let numberSpace = if it.outlined and it.level <= 3 { sym.space.nobreak } else { "" }
...
#let glossary(language, glossaryDict) = {
	...
	grid(
		...
		.. for key in glossaryDict.keys().sorted() {
			...
			(
				text(fill: black, weight: "bold", hyphenate: false)[#name],
				[~],
				[#description],
			)
		}
			...
			block(
				...
				align(
					...
					text(
						weight: "bold",
						fill: black,
						supplement + sym.space.nobreak + array2SectionNumbering(theoremCounter,inAppendix)
					) +
