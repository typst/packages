...
this template is [`0.15.0`][v0.15.0].  See the [Typst
...
```sh
typst compile template/main.typ --pdf-standard ua-1,a-3a
```
in the shell of your choice to compile the document from
scratch. Alternatively, type
```sh
typst watch template/main.typ --pdf-standard ua-1,a-3a
```
...
combined PDF/UA-1 + PDF/A-3a files, when run with the
command
```sh
typst compile --pdf-standard ua-1,a-3a template/main.typ
```
...
guide](https://typst.app/docs/guides/accessibility/) for
details.
...
combined PDF/UA-1 + PDF/A-3a profile. Profile selection
should happen automatically, if the file was generated with
the correct PDF standard flags of the Typst compiler.
...
In addition to using veraPDF to perform basic
PDF/UA-1 + PDF/A-3a conformance checks, you should
also have a quick look at the accessibility tags
generated by Typst. This can be done on the service
[showtags](https://texlive.net/showtags) provided by the
helpful people of The LaTeX Project. On Microsoft Windows®︎,
the program [PAC](https://pac.pdf-accessibility.org)
is also available. You do not have to go through the
entire tag tree, as it is mainly meant for screen reader
consumption, but it is a good idea to look up alternative
texts of images and equations (`Formula` elements) and
check that they seem sensible.
...
[v0.15.0]: https://github.com/typst/typst/releases/tag/v0.15.0
