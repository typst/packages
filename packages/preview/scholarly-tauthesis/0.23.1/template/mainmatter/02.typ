...
You must refer to all images in the body text. The
...
To showcase how figures with subfigures are generated,
@fig-gradient-examples displays a few different gradient
or color progression shapes. Other colormaps than the
`rainbow` seen here are also available. The subfigures are
generated by calling the function `tauthesis.subfigure`
inside of a figure of type `image`.
...
#figure(
    grid(
        ...
        #tauthesis.subfigure(
            ...
            alt: "A subfigure showcasing a linear gradient or color progression in rainbow color.",
        )
        <fig-linear-gradient>
    ][
        #tauthesis.subfigure(
            rect(
                width: 3cm,
                height: 3cm,
                fill: gradient.radial(..color.map.rainbow)
            ),
            alt: "A subfigure showcasing a radial gradient or color progression in rainbow color.",
        )
        <fig-radial-gradient>
    ][
        #tauthesis.subfigure(
            rect(
                width: 3cm,
                height: 3cm,
                fill: gradient.linear(..color.map.rainbow, angle: 45deg).sharp(5)
            ),
            alt: "A subfigure showcasing a linear gradient or color progression tilted by 45 degrees and sampled at 5 points using the sharp method of Typst gradients.",
        )
    ...
    caption: tauthesis.figureCaption(
        short: [
            A display of a few different color gradient
            shapes available in Typst.
        ],
        long: [
            A display of a few different color gradient
            shapes available in Typst. These include the
            linear gradient in @fig-linear-gradient, the
            radial gradient in @fig-radial-gradient and
            the tilted linear gradient with 5 samples in
            @fig-tilted-sampled-linear-gradient. Typst
            would also have a `conic` gradient, but it
            is not supported by PDF/A-3a and hence not
            presented here.
        ],
    ),
) <fig-gradient-examples>

@fig-gradient-examples also demonstrates how a short
caption can be attached to a figure using the function
`tauthesis.figureCaption`. The short caption is only
displayed in the list of figures, whereas the long or full
caption appears in the text itself.
...
version #version(0,15,0), including PDF files as images
...
box has been typeset with the command seen in @theorem-block-code-example.
#figure(
    ```typst
    #tauthesis.example(
        title: [Pythagorean theorem],
    ...
    ```,
    caption: [Example code for displaying a theorem block.]
) <theorem-block-code-example>

The reference label is placed after the call to
`tauthesis.example` via the `<pythagorean-theorem>` syntax.
The module `tauthesis` defines the following theorem box
functions:
- `definition`,
- `theorem`,
- `lemma`,
...
- `seurauslause` and
- `esimerkki`
are also defined. These all follow the same counter, and
can all be referenced by suffixing the equivalent function
call with a tag.
...
version #version(0,15,0) supports adding alternative
...
=== Convert your final submission to PDF/UA-1 + PDF/A-3a format <sec-convert-submission-to-pdfa>

If you are using Typst version ≥ #version(0,15,0) locally
via a command line, it is possible to produce a thesis into
a tagged PDF/UA-1 format and the archival format PDF/A-3a
simultaneously via the command
```sh
typst compile --pdf-standard ua-1,a-3a main.typ
```
...
Typst compilers before version #version(0,15,0) cannot yet
