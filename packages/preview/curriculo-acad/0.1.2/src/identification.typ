...
#let create-identification(detalhes) = {
  ...
  if identificacao.ENDERECO.FLAG-DE-PREFERENCIA == "ENDERECO_RESIDENCIAL" {
    telefon = (
      "("
        + identificacao.ENDERECO.ENDERECO-RESIDENCIAL.DDD
        + ") "
        + identificacao.ENDERECO.ENDERECO-RESIDENCIAL.TELEFONE
    )
    email = identificacao.ENDERECO.ENDERECO-RESIDENCIAL.E-MAIL
  } else if identificacao.ENDERECO.FLAG-DE-PREFERENCIA == "ENDERECO_PROFISSIONAL" {
    telefon = (
      "("
        + identificacao.ENDERECO.ENDERECO-PROFISSIONAL.DDD
        + ") "
        + identificacao.ENDERECO.ENDERECO-PROFISSIONAL.TELEFONE
    )
  ...
  // criando nascimento
  _create-cols([*Nascimento*], custom-date-format(date_birth, pattern: "dd 'de' MMMM 'de' yyyy", lang: "pt"), "small")
