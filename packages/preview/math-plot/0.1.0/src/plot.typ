...
#import "styles.typ": _plot-defaults, merge-styles
#import "transform.typ": to-cm, clip-segment, clip-segment-ellipse, merge-segment-runs, format-number, side-to-anchor
#import "axes.typ": generate-ticks, draw-marker
#import "helpers.typ": _resolve-fn, riemann-heights, riemann-clip-rect

/// Create a mathematical plot.
///
/// Supports 2D function plots, scatter data, vector fields, contour lines,
/// and 3D surfaces. When `zmin`/`zmax` or a 3D child is present the plot
/// delegates to the 3D renderer.
#import "3d/plot3d.typ": plot3d as _plot3d-impl
...
#let plot(
  ...
  /// -> auto | int | float
  ymax: auto,
  /// Minimum z value. Setting this (or `zmax`) promotes the plot to 3D.
  /// -> none | int | float
  zmin: none,
  /// Maximum z value. Setting this (or `zmin`) promotes the plot to 3D.
  /// -> none | int | float
  zmax: none,
  /// Plot width (default: 6).
  /// -> auto | int | float | length
  width: auto,
  /// Plot height (default: 6).
  /// -> auto | int | float | length
  height: auto,
  /// Global scale factor applied to the entire plot.
  /// -> auto | int | float
  scale: auto,
  /// X-axis label.
  /// -> auto | content
  ...
  /// -> auto | content
  zlabel: auto,
  /// Camera azimuth angle (3D only).
  /// -> angle
  azimuth: 45deg,
  /// Camera elevation angle (3D only).
  /// -> angle
  elevation: 30deg,
  /// Position of x label (`"end"`, `"center"`, or `(x, y)`).
  /// -> auto | string | array
  xlabel-pos: auto,
  /// Position of y label (`"end"`, `"center"`, or `(x, y)`).
  /// -> auto | string | array
  ylabel-pos: auto,
  /// Anchor for x label.
  /// -> auto | string
  ...
  /// -> auto | string
  ylabel-anchor: auto,
  /// Offset `(dx, dy)` for x label.
  /// -> auto | array
  xlabel-offset: auto,
  /// Offset `(dx, dy)` for y label.
  /// -> auto | array
  ylabel-offset: auto,
  /// X tick positions (`auto` = computed, `none` = hidden).
  /// -> auto | none | array
  xtick: auto,
  /// Y tick positions (`auto` = computed, `none` = hidden).
  /// -> auto | none | array
  ytick: auto,
  /// Step between x ticks.
  /// -> auto | int | float
  xtick-step: auto,
  /// Step between y ticks.
  /// -> auto | int | float
  ytick-step: auto,
  ...
  /// Grid display (`"major"`, `"minor"`, `"both"`, `true`, `false`).
  /// -> auto | bool | string
  show-grid: auto,
  /// Minor grid subdivisions per major step.
  /// -> auto | int
  minor-grid-step: auto,
  /// Break grid lines around labels.
  /// -> auto | bool
  grid-label-break: auto,
  /// Show only the unit tick label.
  /// -> auto | bool
  unit-label-only: auto,
  /// X-axis position (`"bottom"`, `"center"`, a number, or `none`).
  /// -> auto | string | int | float | none
  axis-x-pos: auto,
  /// Y-axis position (`"left"`, `"center"`, a number, or `none`).
  /// -> auto | string | int | float | none
  axis-y-pos: auto,
  /// How far the x axis extends past the plot area.
  /// -> auto | array | length | float
  axis-x-extend: auto,
  /// How far the y axis extends past the plot area.
  /// -> auto | array | length | float
  axis-y-extend: auto,
  /// Show the origin "0" label.
  /// -> auto | bool
  show-origin: auto,
  /// Offset for origin label.
  /// -> auto | array
  origin-label-offset: auto,
  /// Anchor for origin label.
  /// -> auto | string
  origin-label-anchor: auto,
  /// Draw a leader line to the origin label.
  /// -> auto | bool
  origin-leader: auto,
  /// Stroke for origin leader.
  /// -> auto | stroke
  origin-leader-stroke: auto,
  /// Gap between origin and leader start.
  /// -> auto | float
  origin-leader-gap: auto,
  /// Gap between leader end and label.
  /// -> auto | float
  origin-leader-end-gap: auto,
  /// Size of tick labels.
  /// -> auto | length
  tick-label-size: auto,
  /// Background color for axis labels.
  /// -> auto | color | none
  label-bg: auto,
  /// Background color for tick labels.
  /// -> auto | color | none
  tick-label-bg: auto,
  /// Size of axis labels.
  /// -> auto | length
  axis-label-size: auto,
  /// Font for all text in the plot.
  /// -> auto | string
  font: auto,
  /// Label sizing mode: `"inherit"` or `"plot"`.
  /// -> auto | string
  label-sizing: auto,
  /// Show tick marks at axis endpoints.
  /// -> auto | bool
  show-end-ticks: auto,
  /// Minimum spacing between ticks in cm.
  /// -> auto | float
  min-tick-spacing: auto,
  /// Hide tick labels crossed by a curve.
  /// -> auto | bool
  hide-crossed-tick-labels: auto,
  /// Number of sample points per function curve (default: 100). Higher values
  /// produce smoother curves at the cost of rendering time. Used as the
  /// default for every function series that does not specify its own samples.
  /// -> auto | int
  samples: auto,
  /// Style overrides dictionary.
  /// -> none | dictionary
  style: none,
  /// Deprecated series parameter; prefer positional arguments.
  /// -> none | array
  series: none,
  /// Function/data specifications to plot.
  ..functions,
) = context {
  ...
  // 3D delegation: if zmin/zmax are given, or any child is a 3D series,
  // forward to the 3D renderer.
  let has-3d-child = functions.pos().any(s =>
    type(s) == dictionary and ("surface" in s or "parametric-surface" in s or "parametric-curve" in s or "quiver3d" in s))
  if zmin != none or zmax != none or has-3d-child {
    let xmin = if xmin == auto { -3 } else { xmin }
    let xmax = if xmax == auto { 3 } else { xmax }
    let ymin = if ymin == auto { -3 } else { ymin }
    let ymax = if ymax == auto { 3 } else { ymax }
    let zmin = if zmin == none { -3 } else { zmin }
    let zmax = if zmax == none { 3 } else { zmax }
    let width = to-cm(if width == auto { 10 } else { width })
    let height = to-cm(if height == auto { 8 } else { height })
    let xlabel = if xlabel == auto { $x$ } else { xlabel }
    let ylabel = if ylabel == auto { $y$ } else { ylabel }
    let zlabel = if zlabel == auto { $z$ } else { zlabel }
    return _plot3d-impl(
      ..functions.pos(),
      xmin: xmin, xmax: xmax,
      ymin: ymin, ymax: ymax,
      ...
      ztick-step: auto,
      show-grid: if show-grid == auto { true } else { show-grid },
    )
  }
  ...
  cetz.canvas(length: 1cm, {
    ...
    let is-area-spec(f) = (type(f) == dictionary and (
      ...
      "riemann" in f or "fill-closed" in f or
      f.at("contour-fill", default: false) == true
    ))
    ...
    for func-spec in all-funcs {
      ...
      // Keys: quiver:fn(x,y)=>(u,v), x-step, y-step, scale, pivot, stroke, color, tip
      } else if "quiver" in func-spec {
        let field = func-spec.at("quiver")
        let qx-step = func-spec.at("x-step", default: 1)
        let qy-step = func-spec.at("y-step", default: 1)
        let q-scale = func-spec.at("scale", default: auto)
        let q-pivot = func-spec.at("pivot", default: "center")
        let q-stroke = func-spec.at("stroke", default: 1pt + black)
        let q-color = func-spec.at("color", default: black)
        let q-tip = func-spec.at("tip", default: auto)
        ...
        for gx in grid-xs {
          for gy in grid-ys {
            let (u, v) = field(gx, gy)
            if float(u).is-nan() or float(v).is-nan() { continue }
            let mag = calc.sqrt(u * u + v * v)
            magnitudes.push(mag)
            arrows.push((x: gx, y: gy, u: u, v: v, mag: mag))
          }
        }
        ...
        if arrows.len() > 0 {
          ...
          for a in arrows {
            ...
            let (ox, oy) = if q-pivot == "center" { (-su / 2, -sv / 2) }
                           else if q-pivot == "end" { (-su, -sv) }
                           else { (0, 0) }
            ...
            let clipped = clip-segment((sx, sy), (ex, ey), xmin, ymin, xmax, ymax)
            if clipped == none { continue }
            let ((sx, sy), (ex, ey)) = clipped

            let (cx1, cy1) = to-canvas(sx, sy)
            let (cx2, cy2) = to-canvas(ex, ey)

            let arrow-stroke = q-stroke

            if a.mag < 1e-12 {
              circle((cx1, cy1), radius: 0.02, fill: q-color, stroke: none)
            } else {
              let tip-mark = if q-tip == auto {
                tiptoe.stealth.with(length: 400%)
              } else if q-tip == none {
                none
              } else {
                q-tip
              ...
              cetz.draw.content(
                ((cx1 + cx2) / 2, (cy1 + cy2) / 2),
                tiptoe.line(
                  start: ((cx1 - cx2) / 2 * 1cm, (cy2 - cy1) / 2 * 1cm),
                  end: ((cx2 - cx1) / 2 * 1cm, (cy1 - cy2) / 2 * 1cm),
                  stroke: arrow-stroke,
                  tip: tip-mark,
                ),
              )
      ...
      // Keys: contour:fn, levels, samples, fill, stroke, colors
      } else if "contour" in func-spec {
        let c-fn = func-spec.at("contour")
        let c-levels = func-spec.at("levels", default: (-2, -1, 0, 1, 2))
        let c-samples = func-spec.at("samples", default: 80)
        let c-fill = func-spec.at("contour-fill", default: false)
        let c-stroke = func-spec.at("stroke", default: 1pt + black)
        let c-colors = func-spec.at("colors", default: auto)

        let xs = range(c-samples + 1).map(i => xmin + i * (xmax - xmin) / c-samples)
        let ys = range(c-samples + 1).map(i => ymin + i * (ymax - ymin) / c-samples)

        let contour-data = komet.contour(xs, ys, c-fn, c-levels)

        let n-levels = c-levels.len()
        let ramp = gradient.linear(..color.map.viridis)
        let auto-colors = if c-colors == auto and n-levels > 1 {
          range(n-levels).map(i => ramp.sample(i * 100% / (n-levels - 1)))
        } else {
          none
        }
        ...
        if c-fill {
          ...
          // Close an open contour path against the domain boundary.
          // Adapted from Lilaq's close-path-at-boundaries: inserts
          // boundary corners between the two boundary endpoints, then
          // reverses the result (Lilaq convention for orientation).
          let bnd-corners = ((xmax, ymax), (xmax, ymin), (xmin, ymin), (xmin, ymax))
          let close-path-boundary(path) = {
            if path.len() < 2 { return none }
            let first = path.first()
            let last = path.last()
            if calc.abs(first.at(0) - last.at(0)) < eps and calc.abs(first.at(1) - last.at(1)) < eps {
              return path.slice(0, -1).rev()
            }
            let get-edge(pt) = {
              if calc.abs(pt.at(1) - ymax) < eps { 0 }
              else if calc.abs(pt.at(1) - ymin) < eps { 2 }
              else if calc.abs(pt.at(0) - xmax) < eps { 1 }
              else if calc.abs(pt.at(0) - xmin) < eps { 3 }
              else { none }
            }
            let edge1 = get-edge(last)
            let edge2 = get-edge(first)
            if edge1 == none or edge2 == none { return none }

            if edge1 == edge2 {
              path.push(first)
              return path.rev()
            } else if calc.rem(edge1 + 2, 4) == edge2 {
              if edge1 in (0, 2) {
                let verts = (bnd-corners.at(0), bnd-corners.at(1))
                if edge1 == 2 { verts = verts.rev() }
                path += verts
              } else {
                let verts = (bnd-corners.at(3), bnd-corners.at(0))
                if edge1 == 1 { verts = verts.rev() }
                path += verts
              }
            } else {
              if calc.rem(edge1 + 1, 4) == edge2 {
                path.push(bnd-corners.at(edge1))
              } else if calc.rem(edge2 + 1, 4) == edge1 {
                path.push(bnd-corners.at(edge2))
              } else { return none }
            }
            path.push(first)
            return path.rev()
          ...
          // Shoelace polygon orientation (from Lilaq).
          // Returns "left" (CCW) or "right" (CW) or none.
          let poly-orient(pts) = {
            if pts.len() < 3 { return none }
            let s = 0
            for i in range(pts.len()) {
              let a = pts.at(i)
              let b = pts.at(calc.rem(i + 1, pts.len()))
              s += (a.at(0) - b.at(0)) * (a.at(1) + b.at(1))
            }
            if s > 1e-10 { "left" }
            else if s < -1e-10 { "right" }
            else { none }
          }

          // Close all contour paths at boundaries
          let closed-contours = ()
          for paths in contour-data {
            let closed = paths.map(p => close-path-boundary(p)).filter(p => p != none and p.len() >= 3)
            closed-contours.push(closed)
          }

          // CW boundary rect in data coords (matching Lilaq's canvas-rect)
          let data-rect = ((xmax, ymax), (xmax, ymin), (xmin, ymin), (xmin, ymax))

          // N colors for N levels (matching Lilaq)
          let fill-colors = if c-colors != auto and type(c-colors) == array {
            let fc = c-colors
            while fc.len() < n-levels { fc.push(fc.last()) }
            fc.slice(0, n-levels)
          } else {
            range(n-levels).map(i => ramp.sample(i * 100% / calc.max(1, n-levels - 1)))
          }

          // Paint from lowest level to highest. Each layer overlays
          // the previous, so only the topmost color is visible per band.
          for li in range(n-levels) {
            let fill-color = fill-colors.at(li)
            let cpaths = closed-contours.at(li)

            if li == 0 {
              cpaths = (data-rect,)
            }
            if cpaths.len() == 0 { continue }

            let orient = poly-orient(cpaths.first())
            if orient == "left" {
              cpaths = (data-rect,) + cpaths
            }

            compound-path(fill: fill-color, stroke: none, fill-rule: "non-zero", {
              for cpath in cpaths {
                let cpts = cpath.map(pt => to-canvas(pt.at(0), pt.at(1)))
                line(..cpts, close: true)
              }
            })
          }

          // No contour lines in filled mode — overlay a second
          // contour(fill: false) series for lines on top of fills.
        } else {
          for (li, level-lines) in contour-data.enumerate() {
            let level-stroke = if type(c-stroke) != length {
              c-stroke
            } else if auto-colors != none {
              auto-colors.at(li) + c-stroke
            } else if type(c-colors) == array and li < c-colors.len() {
              c-colors.at(li) + c-stroke
            } else {
              c-stroke
            }

            for contour-line in level-lines {
              if contour-line.len() < 2 { continue }
              let canvas-pts = contour-line.map(pt => {
                to-canvas(pt.at(0), pt.at(1))
              })
              line(..canvas-pts, stroke: level-stroke)
            }
          }
        }
      }
...
/// Quick single-function plot with automatic y-bound scaling.
///
/// Evaluates `fn` over `domain`, computes y-bounds with 10% padding,
/// and delegates to `plot()`.
#let plot-fn(
  /// The function `x => y` to plot.
  /// -> function
  fn,
  /// Domain `(xmin, xmax)` for the function.
  /// -> array
  domain: (-5, 5),
  /// Minimum y value (`auto` = computed from samples).
  /// -> auto | int | float
  ymin: auto,
  /// Maximum y value (`auto` = computed from samples).
  /// -> auto | int | float
  ymax: auto,
...
///
/// A thin convenience wrapper around `plot()` that sets a centered grid,
/// axis labels, and a single function series. Automatically splits the
/// domain around vertical asymptotes.
#let plot-rational(
  /// The function `x => y` to plot.
  /// -> function
  fn,
  ...
  /// -> stroke
  stroke: blue + 1.2pt,
  /// Domain `(xmin, xmax)` for evaluation (`auto` = use `xmin`/`xmax`).
  /// -> auto | array
  domain: auto,
  /// Number of sample points per domain segment.
  /// -> int
  samples: 200,
  /// X-coordinates of vertical asymptotes to draw as dashed lines.
  /// -> array
  vertical-asymptotes: (),
  /// Stroke style for vertical asymptote lines.
  /// -> stroke
  asymptote-stroke: stroke(paint: luma(80), thickness: 0.6pt, dash: "dashed"),
  /// Gap between the asymptote and the nearest function segment.
  /// -> float
  asymptote-gap: 0.05,
  /// Plot width.
  /// -> auto | int | float | length
...
#let limit-schema(
  ...
  /// -> int | float
  a: 0,
  /// Left-hand limit: `"+oo"`, `"-oo"`, a finite number, or `none`.
  /// -> none | string | int | float
  left: none,
  /// Right-hand limit: `"+oo"`, `"-oo"`, a finite number, or `none`.
  /// -> none | string | int | float
  right: none,
  /// Defined value `f(a)`, shown as a filled dot.
  /// -> none | int | float
  val: none,
  /// Custom label for the x-coordinate (`auto` = the value of `a`).
  /// -> auto | content
  a-label: auto,
  /// Show a horizontal dashed line at the limit value. Pass `true` to
  /// auto-detect or a number for a specific y-coordinate.
  /// -> bool | int | float
  show-limit: false,
  /// Custom label for the limit line (`auto` = `$L$`).
  /// -> auto | content
  l-label: auto,
  /// Plot width in cm.
  /// -> int | float
...
/// Draw a solid of revolution (volume-of-revolution diagram).
///
/// Given a profile function `fn(x)`, draws the 3D solid obtained by rotating
/// the curve around the x-axis (or an arbitrary axis via `axis-slope` and
/// `axis-y`). For hollow solids, pass `inner-fn` to carve out the interior.
///
/// ```typst
/// #volume-of-revolution(x => calc.sqrt(x + 1), domain: (0, 3), n-disks: 4)
/// ```
#let volume-of-revolution(
  /// The profile function `x => y` defining the outer boundary.
  /// -> function
  fn,
  /// Inner profile function for hollow solids, or `none` for solid bodies.
  /// -> none | function
  inner-fn: none,
  /// Domain `(a, b)` over which to revolve.
  /// -> array
  domain: (0.0, 4.0),
  /// Number of disk cross-sections to draw.
  /// -> int
  n-disks: 4,
  /// Diagram width in cm.
  /// -> int | float
  width: 5.0,
  /// Diagram height in cm.
  /// -> int | float
  height: 3.5,
  /// Number of sample points for the profile curve.
  /// -> int
  samples: 60,
  /// Show the revolution axis.
  /// -> bool
  show-axis: true,
  /// Show a vertical y-axis.
  /// -> bool
  show-y-axis: false,
  /// X-coordinate for the y-axis (`auto` = left edge of the domain).
  /// -> auto | int | float
  y-axis-x: auto,
  /// Vertical offset for the y-axis.
  /// -> float
  y-axis-offset: 0.45,
  /// Extend `(below, above)` for the y-axis past the solid.
  /// -> array
  y-axis-extend: (0.35, 0.45),
  /// Y-intercept of the revolution axis line `y = m*x + b`.
  /// -> int | float
  axis-y: 0.0,
  /// Slope `m` of the revolution axis line `y = m*x + b`.
  /// -> int | float
  axis-slope: 0.0,
  /// Show a radius marker from the axis to the curve.
  /// -> bool
  show-radius-marker: false,
  /// X-coordinate at which to draw the radius marker.
  /// -> auto | int | float
  radius-marker-x: auto,
  /// Show endpoint and function labels.
  /// -> bool
  show-labels: true,
  /// Draw the back half of the ellipses (behind the solid).
  /// -> bool
  show-back: true,
  /// Stroke style for the profile curve.
  /// -> stroke
  profile-stroke: blue + 1.5pt,
  /// Fill color for the disk cross-sections.
  /// -> color
  disk-color: luma(218),
  /// Stroke style for the disk outlines.
  /// -> stroke
  disk-stroke: luma(90) + 0.6pt,
  /// Stroke style for the inner boundary (`auto` = matches `profile-stroke`).
  /// -> auto | stroke
  inner-stroke: auto,
  /// Fill color for the hollow center.
  /// -> color
  hole-color: white,
  /// Stroke style for the revolution axis.
  /// -> stroke
  axis-stroke: black + 0.7pt,
  /// Label for the left domain endpoint.
  /// -> content
  label-a: $a$,
  /// Label for the right domain endpoint.
  /// -> content
  label-b: $b$,
  /// Label for the outer profile curve.
  /// -> content
  label-f: $f$,
  /// Label for the inner profile curve, or `none` to hide.
  /// -> none | content
  label-inner: none,
  /// Fractional position along the domain for function labels.
  /// -> float
  label-pos: 0.18,
  /// Label for the y-axis.
  /// -> content
  label-y: $y$,
  /// Deprecated named arguments (kept for backwards compatibility).
  ..legacy,
) = {
...
/// Alias for `volume-of-revolution`.
