...
//
#import "@preview/simple-plot:0.8.0": plot, fill-area, riemann-sum
  ...
  #grid(columns: (1fr,) * 5, gutter: 0.4em,
    ..for (meth, col) in (
      ...
      align(center)[
        ...
        #v(0.2em)
        #plot(
          xmin: 0, xmax: 3.3, ymin: -0.1, ymax: 2.3,
          width: 3.5, height: 3,
...
#demo("3 · `show-xi: true` + `xi-show-values: true` — numeric subdivision labels")[
  #grid(columns: (1fr, 1fr), gutter: 1.5em,
    align(center)[
      _`xi-show-values: false` (default) — symbolic $x_0, x_1, \ldots$_
      #v(0.3em)
      #plot(
        xmin: -0.2, xmax: 4.2, ymin: -0.1, ymax: 2.5,
        width: 7, height: 4,
        axis-x-pos: "bottom", axis-y-pos: "left",
        ...
        (fn: x => calc.sqrt(x), domain: (0.0, 4.2), stroke: blue + 1.3pt, samples: 80),
      )
    ],
    align(center)[
...
#demo("4 · `show-dx` — Δx dimension bracket, `dx-rect` selects which rectangle")[
  #grid(columns: (1fr, 1fr), gutter: 1.5em,
    align(center)[
      _`dx-rect: auto` (middle rectangle)_
      #v(0.3em)
      #plot(
        xmin: -0.2, xmax: 3.5, ymin: -0.1, ymax: 2.5,
        width: 7, height: 4,
        axis-x-pos: "bottom", axis-y-pos: "left",
        xtick: (0, 1, 2, 3), ytick: (1, 2),
        show-origin: false,
...
#demo("5 · `show-points` — evaluation dots; `point-label` and `point-label-pos`")[
  #grid(columns: (1fr, 1fr), gutter: 1.5em,
    align(center)[
      ...
      #v(0.3em)
      #plot(
        xmin: -0.2, xmax: 3.5, ymin: -0.1, ymax: 2.3,
        width: 7, height: 4,
        axis-x-pos: "bottom", axis-y-pos: "left",
        xtick: (0, 1, 2, 3), ytick: (1, 2),
        show-origin: false,
        riemann-sum(x => calc.sqrt(x + 0.2), domain: (0.2, 3.2), n: 4, method: "mid",
                    ...
                    show-points: true),
        (fn: x => calc.sqrt(x + 0.2), domain: (0.0, 3.4), stroke: blue + 1.3pt),
      )
    ],
    align(center)[
      ...
      #plot(
        ...
        (fn: x => calc.sqrt(x + 0.2), domain: (0.0, 3.4), stroke: red + 1.3pt),
      )
    ],
  )
]
...
#demo("6 · Hatch controls: `hatch-spacing` and `hatch-stroke` on Riemann rectangles")[
  #grid(columns: (1fr, 1fr, 1fr), gutter: 0.5em,
    ..for (spacing, label-txt) in ((3pt, "3pt"), (5pt, "5pt (default)"), (9pt, "9pt")) {(
      align(center)[
        #text(8pt)[`hatch-spacing: #label-txt`]
        #v(0.2em)
        #plot(
          xmin: -0.2, xmax: 3.5, ymin: -0.1, ymax: 2.2,
...
#demo("7 · `\"lower\"` and `\"upper\"` — true inf/sup; works for U-curves")[
  #grid(columns: (1fr, 1fr), gutter: 1.5em,
    align(center)[
      _`\"lower\"` — true minimum on each subinterval_
      #v(0.3em)
      #plot(
        xmin: -2.2, xmax: 2.2, ymin: -0.3, ymax: 4.5,
        width: 7, height: 4.5,
        ...
        fill-area(x => x * x, domain: (-2.0, 2.0), color: blue.transparentize(88%)),
        (fn: x => x * x, domain: (-2.1, 2.1), stroke: blue + 1.3pt,
         label: $x^2$, label-pos: 0.1, label-anchor: "south"),
      )
    ],
    align(center)[
      ...
      #plot(
        ...
        fill-area(x => x * x, domain: (-2.0, 2.0), color: red.transparentize(88%)),
...
#demo("8 · Pedagogical: right-sum approximation with all annotations enabled")[
  #align(center)[
    #plot(
      xmin: -0.2, xmax: calc.pi + 0.3, ymin: -0.05, ymax: 1.3,
      width: 14, height: 6,
      axis-x-pos: "bottom", axis-y-pos: "left",
      xlabel: $x$,
      xtick: (0, 1, 2, 3), ytick: (0.5, 1.0),
      show-origin: false,
      riemann-sum(
        x => calc.sin(x),
        domain: (0.0, calc.pi),
      ...
      fill-area(x => calc.sin(x), domain: (0.0, calc.pi),
                color: blue.transparentize(88%)),
      (fn: x => calc.sin(x), domain: (-0.1, calc.pi + 0.2),
       stroke: blue + 1.5pt, samples: 100,
       label: $sin(x)$, label-pos: 0.25, label-anchor: "south"),
    )
  ]
]
