...
#import "locale.typ": TABLE_OF_CONTENTS, APPENDIX, REFERENCES, PREFACE, GENDER_STATEMENT
...
#let clean-fuh(
  ...
  if (show-abstract and abstract != none) {
    for a in abstract {
      heading(level: 1, numbering: none, outlined: false, ABSTRACT.at(a.language))
      text(a.content)
      pagebreak()
    }
  }
  ...
  if show-gender-notice {
    heading(level: 1, numbering: none, outlined: false, PREFACE.at(language))
    text(GENDER_STATEMENT.at(language))
    pagebreak()
  }
 ...
 // ---------- Body Text ---------------------------------------
  ...
  if (show-confidentiality-statement) {
    confidentiality-statement(
      authors,
      title,
      confidentiality-statement-content,
