...
#let tasks-config = state("tasks-config", (
  ...
  answer-lines: 0,              // dotted lines drawn under each task, for exam sheets
  answer-line-gap: 1.2em,       // spacing between those lines
  column-gutter: 1em,
  row-gutter: 0.6em,
...
// How deep the current tasks call is nested inside other tasks calls, so that
// label: "enum" can pick the matching level of a multi-level enum numbering
// pattern ("1) a)" gives "1)" at the top level and "a)" one level in).
#let _taskize-depth = state("taskize-depth", 0)
...
#let tasks-setup(
  ...
  tasks-config.update(cfg => {
    ...
    if answer-lines != none { new.answer-lines = answer-lines }
    if answer-line-gap != none { new.answer-line-gap = answer-line-gap }
...
/// Dotted lines to write an answer on, filling the width they are given.
///
/// Inside a task they span the column, which is what an exam sheet wants:
///   #tasks(columns: 2, answer-lines: 2)[...]
/// or by hand, anywhere:
///   #answer-dots(3)
#let answer-dots(count: 1, gap: 1.2em) = {
  if count <= 0 { return none }
  stack(
    spacing: gap,
    ..range(count).map(_ => box(width: 1fr, repeat[.])),
  )
}
...
// Returns true if the content is not inline as a whole, but *begins* with
// inline content: a text line followed by a block (a nested tasks call, a
// figure, a table). The label can then still share that first line.
#let starts-inline(node) = {
  if is-inline-content(node) { return true }
  if type(node) != content { return false }
  if repr(node.func()) == "styled" { return starts-inline(node.child) }
  if not node.has("children") { return false }
  // Leading whitespace, or the empty text node the parser leaves in front of a
  // block, is not a first line on its own
  let blank(c) = {
    let f = repr(c.func())
    if f in ("space", "h") { true } else if f == "text" { c.text.trim() == "" } else { false }
  }
  for child in node.children {
    if blank(child) { continue }
    return is-inline-content(child)
  }
  false
}
...
// Rigid content (a table with fixed columns, a fixed-size box, an image given
// an explicit width) keeps exactly the same height at any width: it overflows
// silently instead of adapting, so the height test alone can never reject it.
// Compare its natural width too, but only when the height proves that nothing
// adapted -- content whose height changes did adapt to the narrower track:
// text and table cells wrap (height grows) and unconstrained images scale
// down (height shrinks). Both fit and must stay judged by height alone.
#let _fits-content-width(content, width, tol) = {
  let nat = measure(content)
  nat.width <= width + tol or calc.abs(_task-height-at-width(content, width) - nat.height) > tol
}

// A regular item fits a track of `width` when it introduces no new wrap
// (height) and does not overflow it (width).
#let _fits-at-width(content, width, base-h, tol) = {
  _task-height-at-width(content, width) <= base-h + tol and _fits-content-width(content, width, tol)
}
  ...
  let fits-cols(cols) = {
    for (i, item-data) in task-items.enumerate() {
      ...
      if mode == "uniform" or explicit-span > 1 {
        ...
        // fill mode, regular item: auto-span until it fits. Height drives the
        // test — text always reflows, so natural width alone would reject long
        // sentences even at full-row span — but rigid content that cannot
        // reflow is also checked against its natural width.
        let fitted = false
        for s in range(1, cols + 1) {
          let width = _task-content-width(available-width, cols, s, label-width, col-gut, indent-after, indent)
          if _fits-at-width(content, width, base-heights.at(i), tol) {
            fitted = true
            break
          }
        }
...
#let render-tasks-grid(
  ...
  // Same, for an item whose first line is inline but which continues with a
  // block (a nested tasks call, a figure, a table). The item is indented by
  // the label column and the label is pulled back into the margin that frees,
  // so it sits on the first line and shares its baseline. The content is
  // passed through untouched, so its own inner spacing — a nested #tasks and
  // its `above`, a figure's block spacing — is preserved exactly. Splitting it
  // into head and tail instead would drop the weak spacing at the seam.
  let make-hanging-cell(label, content, span) = {
    let lbl = text(weight: lbl-weight)[#label]
    let halign = if type(lbl-align) == alignment and lbl-align.x != none {
      lbl-align.x
    } else {
    ...
    let indent = actual-label-width + indent-after
    grid.cell(colspan: span * 2, align(left + top, pad(left: indent, {
      // zero width: the label rides on the content's first line, so it shares
      // that line's baseline whatever the line's height
      box(width: 0pt, move(dx: -indent, label-box))
      wrap-content(content)
    })))
  }
  ...
  let make-item-cells(label, content, span) = {
    ...
    } else if use-inline-baseline and starts-inline(content) {
      (make-hanging-cell(label, content, span),)
    } else if span == 1 {
...
#let tasks(
  ...
  label-format: auto,
  answer-lines: auto,       // dotted lines under each task (exam sheets)
  answer-line-gap: auto,
  start: 1,
  resume: false,
  column-gutter: auto,
  row-gutter: auto,
  ...
  // label: "enum" follows the document's own enum numbering, so a task list
  // matches the surrounding lists without repeating their format here.
  // A multi-level pattern is split on spaces ("1) a)" -> "1)", "a)") and the
  // level matching the nesting depth is used; deeper than the pattern goes,
  // the last level repeats. A numbering *function* is called with the item
  // number, since its levels cannot be taken apart.
  let nesting = _taskize-depth.get()
  if fmt == "enum" {
    let pattern = enum.numbering
    fmt = if type(pattern) == function {
      n => pattern(n)
    } else if type(pattern) == str {
      let levels = pattern.split(" ").filter(p => p != "")
      let level = if levels.len() == 0 { "1." } else { levels.at(calc.min(nesting, levels.len() - 1)) }
      n => numbering(level, n)
    } else {
      "a)"
    }
  ...
  let flatten-all-enum-items(node) = {
    ...
    if node.func() == enum {
      ...
      // Handle +(N) and +() syntax, which is parsed as text rather than as an
      // enum.item because there is no space after the +.
      //
      // Only "+(" opens an item here. A bare + in the middle of a line is
      // ordinary text to Typst, which makes no item of it: taking it for an
      // item split "What is 2 + 2?" in two and repeated the rest of the line
      // (issue #2). A list written on one line has no item syntax to rely on;
      // pass an array instead, see below.
  ...
  let ans-lines = if answer-lines == auto { cfg.at("answer-lines", default: 0) } else { answer-lines }
  let ans-gap = if answer-line-gap == auto { cfg.at("answer-line-gap", default: 1.2em) } else { answer-line-gap }

  // An array spells the items out one by one. Typst has no inline list syntax:
  // "+ One + Two" on a single line is one item to it, the pluses being plain
  // text, so a short list written on one line needs an explicit form (issue #2).
  //   #tasks(("One", "Two", "Three"))
  // An entry may carry its column span as (content, span).
  task-items = if type(body) == array {
    body.map(entry => {
      if type(entry) == array and entry.len() == 2 and type(entry.at(1)) == int {
    ...
    flatten-all-enum-items(body)
  }

  // Room to write the answer, under each task (issue #1). The lines fill the
  // column they land in, so they follow the layout rather than a fixed width.
  if ans-lines > 0 {
    task-items = task-items.map(item-data => (
      item-data.at(0) + answer-dots(count: ans-lines, gap: ans-gap),
      item-data.at(1),
    ))
  }
  ...
  // Everything emitted from here on is one level deeper, so a nested tasks
  // call reading label: "enum" picks the next level of the pattern.
  _taskize-depth.update(n => n + 1)
        ...
        let render-items = if fit-mode == "fill" {
          ...
          task-items.map(item-data => {
            ...
            for s in range(1, cols + 1) {
              ...
              if _fits-at-width(content, w, base-h, tol) {
                return (content, s)
              }
            }
            (content, cols)
