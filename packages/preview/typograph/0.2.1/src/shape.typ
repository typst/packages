...
#let fit-box(label, pad, style, clear-x: 1.0, clear-y: 1.0, square: false) = {
  let width = calc.max(style.min-width, label.width * clear-x + pad.left + pad.right)
  let height = calc.max(style.min-height, label.height * clear-y + pad.top + pad.bottom)
  if square {
...
#let _polygon-properties(points) = {
  let magnitude = points.fold(0, (largest, point) => calc.max(largest, calc.abs(point.at(0)), calc.abs(point.at(1))))
...
#let _polygon-outline(points, label-offset) = {
  assert(
    type(points) == array and points.len() >= 3 and points.all(point => (type(point) == array and point.len() == 2 and point.all(component => type(component) == length))),
    message: "polygon-outline() needs at least three absolute-length point pairs",
  )
  assert(
    type(label-offset) == array and label-offset.len() == 2 and label-offset.all(component => type(component) == length),
    message: "polygon-outline() label-offset must be an absolute-length pair",
  )
  let extents = points.fold((half-width: 0pt, half-height: 0pt), (extents, point) => (
  ...
  let numeric = points.map(point => (point.at(0) / 1pt, point.at(1) / 1pt,))
  let properties = _polygon-properties(numeric)
  assert(
    calc.abs(properties.twice-area) > 1e-9,
    message: "polygon-outline() points must span a non-zero area on both axes",
  )
  assert(properties.contains-origin, message: "polygon-outline() requires the node origin strictly inside the polygon")
  (
    kind: "polygon",
    points: points,
...
#let _polygon(points, label-offset: (0pt, 0pt)) = {
  (kind: "polygon", points: points, label-offset: label-offset)
}

#let _require-unrotated(name, style) = {
  assert(
    style.rotate == 0deg,
    message: "rotate is not supported by shapes." + name + "; use a polygonal shape or a custom builder",
  )
}

#let _resolve-rect-radius(spec, width, height) = {
  assert(type(spec) in (length, ratio, relative), message: "rect/square radius must be a uniform length or percentage")
  ...
  let radius = if type(spec) == length { spec } else if type(spec) == ratio { spec * base } else { spec.length + spec.ratio * base }
...
#let ellipse(label, pad, style) = {
  ...
  let (width, height) = fit-box(label, pad, style, clear-x: 1.25, clear-y: 1.25)
  (kind: "ellipse", half-width: width / 2, half-height: height / 2)
}
...
#let stadium(label, pad, style) = {
  ...
  (kind: "rect", half-width: half-width, half-height: half-height, radius: calc.min(half-width, half-height))
}
...
#let _numeric-point(point) = (type(point) == array and point.len() == 2 and point.all(component => type(component) in (int, float)))

#let _clearance(spec) = {
  let pair = if type(spec) in (int, float) { (spec, spec) } else { spec }
  assert(
    type(pair) == array and pair.len() == 2 and pair.all(value => type(value) in (int, float) and value > 0),
...
#let regular(vertices: 3, rotate: 0deg, clearance: auto) = {
  assert(type(vertices) == int and vertices >= 3, message: "shapes.regular() vertices must be an integer of at least 3")
...
#let polygon(vertices, anchor: auto, rotate: 0deg, clearance: 1, label-offset: (0, 0)) = {
  assert(
    type(vertices) == array and vertices.len() >= 3 and vertices.all(_numeric-point),
  ...
  assert(_numeric-point(label-offset), message: "shapes.polygon() label-offset must be a numeric point pair")
  assert(
  ...
  let bounds = vertices.slice(1).fold((min-x: first.at(0), max-x: first.at(0), min-y: first.at(1), max-y: first.at(1)), (bounds, point) => (
  ...
  let template = rotate-points(vertices.map(point => (point.at(0) - origin.at(0), point.at(1) - origin.at(1),)), rotate)
  let template-label-offset = rotate-point(label-offset, rotate)
  let extents = template.fold((half-width: 0, half-height: 0), (extents, point) => (
    half-width: calc.max(extents.half-width, calc.abs(point.at(0))),
    half-height: calc.max(extents.half-height, calc.abs(point.at(1))),
  ))
  let (template-half-width, template-half-height) = (extents.half-width, extents.half-height,)
  let properties = _polygon-properties(template)
  assert(
    template-half-width > 0 and template-half-height > 0 and calc.abs(properties.twice-area) > 1e-9,
    message: "shapes.polygon() vertices must span a non-zero area on both axes",
  )
  assert(
    properties.contains-origin,
    message: "shapes.polygon() requires its anchor inside the polygon" + if anchor == auto { "; pass anchor: (...) when the bounding-box center is outside" } else { "" },
  )
  (label, pad, style) => {
    let (width, height) = fit-box(label, pad, style, clear-x: clear.at(0), clear-y: clear.at(1))
    let fitted-scale = calc.max(width / (2 * template-half-width), height / (2 * template-half-height))
    // An unlabelled polygon with no inset or minimum size has no requested
    // physical extent. Keep its valid template non-degenerate instead of
    // collapsing every vertex to the origin; any real sizing constraint still
    // wins unchanged. The fallback normalizes the longer axis to just 1pt.
    let scale = if fitted-scale == 0pt {
      0.5pt / calc.max(template-half-width, template-half-height)
    } else {
      fitted-scale
    }
    let scaled = template.map(point => (point.at(0) * scale, point.at(1) * scale,))
    let scaled-offset = (template-label-offset.at(0) * scale, template-label-offset.at(1) * scale,)
    _polygon(orient-points(scaled, style), label-offset: orient-point(scaled-offset, style))
  }
}
...
#let _triangle-isosceles(ratio, label, pad, style) = {
  ...
  _polygon(orient-points(points, style), label-offset: orient-point((-width / 2 * 0.16, 0pt), style))
}

#let _validate-triangle-angles(angles, source) = {
  assert(
    type(angles) == array and angles.len() == 3,
    message: source + " must be an array of three angles",
  )
  for value in angles {
    assert(type(value) == angle, message: source + " entries must be angles")
  }
}
...
#let _triangle-angles(angles, label, pad, style) = {
  ...
  _polygon(
    ...
    label-offset: orient-point((-half-width * 0.16, 0pt), style),
  )
}

#let triangle(..args, mode: "equilateral", ratio: 1.0, angles: none) = {
  if args.len() == 3 {
    let label = args.at(0)
    let pad = args.at(1)
    let style = args.at(2)
    return _triangle-equilateral(label, pad, style)
  }

  let requested = if args.len() == 1 {
    let first = args.at(0)
    assert(type(first) == str, message: "triangle() factory-first argument must be a string mode")
    first
  } else {
    mode
  }
  assert(
    args.len() in (0, 1),
    message: "triangle() factory accepts zero args, or a string mode as the only positional argument",
  )

  if requested == "equilateral" {
    _triangle-equilateral
  } else if requested == "isosceles" {
    (label, pad, style) => _triangle-isosceles(ratio, label, pad, style)
  } else if requested == "angles" {
    _validate-triangle-angles(angles, "triangle(\"angles\")")
    (label, pad, style) => _triangle-angles(angles, label, pad, style)
  } else {
    panic("unknown triangle mode " + repr(requested))
  }
}
...
#let arrow(label, pad, style) = {
  ...
  _polygon(orient-points(points, style), label-offset: orient-point((-(1 - tip) / 2 * half-width, 0pt), style))
}
...
#let _validate-outline(outline) = {
  ...
  assert(kind in supported, message: "node shape builder returned unsupported outline kind " + repr(kind))
  let required = if kind in ("empty", "bare") { () } else if kind == "circle" { ("radius",) } else if kind == "ellipse" { ("half-width", "half-height") } else if kind == "rect" { ("half-width", "half-height", "radius") } else { ("points", "label-offset") }
  let missing = required.filter(field => field not in outline)
  assert(
    missing.len() == 0,
    message: "node shape builder returned " + repr(kind) + " without required field(s): " + missing.join(", "),
  )
  if "label-offset" in outline {
    assert(
      type(outline.label-offset) == array and outline.label-offset.len() == 2 and outline.label-offset.all(component => type(component) == length),
      message: "outline label-offset must be an absolute-length pair",
    )
    assert(kind not in ("empty", "bare"), message: kind + " outlines cannot use label-offset")
  }
  if kind == "circle" {
    assert(
    ...
    assert(
      type(outline.half-width) == length and outline.half-width >= 0pt and type(outline.half-height) == length and outline.half-height >= 0pt,
...
#let build-outline(builder, label, pad, style) = {
  assert(type(builder) == function, message: "node shape must be a builder function such as shapes.circle")
