...
```typst
#import "@preview/touying-liu-nyckel:0.1.3": *
```
...
```
`title-background` can be `none`, `image` (see utility functions), a `color`, or a `gradient`.
...
#align(horizon, text-block("Available utility functions", [
  ...
  - `focus-slide` - create a slide with a single focus item
...
#focus-slide(text(font: "New Computer Modern", size: 35pt, [
  _This is an important message_
]))
