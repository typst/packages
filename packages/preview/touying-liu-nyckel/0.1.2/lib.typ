...
#let text-box(content, alignment: center, stroke: 2pt + liu-colors.darkblue, width: auto) = {
...
#let slide(title: auto, empty: false, ..args) = touying-slide-wrapper(self => {
  ...
  let header(self) = {
    ...
    set par(spacing: 0mm)
    set align(horizon)
    set text(font: self.store.header-font, size: 1.6 * self.store.size)
    pad(x: 1.2em, align(
  ...
  let footer(self) = {
    ...
    set text(size: self.store.size, font: self.store.header-font)

    context pad(
      x: 2em,
      y: 0.9em,
      align(right, text(size: 14pt, style: "italic", utils.slide-counter.display())),
    )
  ...
  self = utils.merge-dicts(
    self,
    config-page(
      header: if empty { none } else { header },
      footer: if empty { none } else { footer },
    ),
  )
...
#let title-slide(
  ...
  let body = {
    ...
    block(
      ...
      inset: 2em,
      {
        set text(font: self.store.title-font)
  ...
  self = utils.merge-dicts(
    ...
    config-page(
      background: if (
        self.store.title-background != none
          and type(self.store.title-background) != color
          and type(self.store.title-background) != gradient
      ) {
        self.store.title-background
      } else { none },
      fill: if self.store.title-background == none { self.colors.theme } else if (
        type(self.store.title-background) == color or type(self.store.title-background) == gradient
      ) { self.store.title-background } else { black },
    ),
  )
...
#let liu-sectioning-slide(config: (:), level: 1, numbered: true, body) = touying-slide-wrapper(self => {
  let slide-body = {
    // set page(fill: self.colors.theme)
    set std.align(top)
    ...
    place(left + bottom, float: false, dx: -9mm, dy: 19mm, image(
  ...
  self = utils.merge-dicts(
    self,
    config-page(fill: if self.colors.section == none { self.colors.theme } else { self.colors.section }),
  )
...
#let end-slide(
  ...
  let body = {
    set par(justify: false, spacing: 0.5em, leading: 0.4em)
    set text(fill: white, font: self.store.body-font, weight: "regular")
...
#let liu-theme(
  ...
  show: touying-slides.with(
    ...
    config-colors(
      primary: liu-colors.blue,
      neutral-lightest: rgb("#ffffff"),
      neutral-darkest: rgb("#000000"),
      theme: liu-colors.blue,
      block: none,
      section: none,
    ),
