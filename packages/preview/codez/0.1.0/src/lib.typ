...
#let __codly-mark-anchor(mark-label) = {
  // Zero-size inline anchor used for locate().
  let lab = mark-label
  [#box(width: 0pt, height: 0pt)[ ]#lab]
}

#let __codly-slice-text(child, from, to) = {
  if not child.has("text") {
    return none
  }
  if to <= from {
    return none
  }
  let txt = child.text.slice(from, to)
  if txt.len() == 0 {
    return none
  }

  // Preserve styles when possible.
  if child.has("styles") {
    (child.func())(text(txt), child.styles)
  } else {
    text(txt)
  }
}
...
#let codly-line(
  ...
  let marks = if marks == none {
    ()
  } else {
    marks
      .filter(x => x.line == it.number)
      .map(x => {
        if not "label" in x or x.label == none {
          panic("codly: marks require a label")
        }
        if not "start" in x or x.start == none {
          x.insert("start", 0)
        }
  ...
  // If there are no highlights or marks, return the line as is.
  if highlights.len() == 0 and marks.len() == 0 {
    if width != none {
  ...
  if marks.len() > 0 {
    ...
    for child in body {
      ...
      if len == 0 {
        // Non-text or zero-length element: insert any marks at this position.
        while pending.len() > 0 and pending.at(0).start <= i {
          children.push(__codly-mark-anchor(pending.at(0).label))
      ...
      // Marks that fall within this child.
      let end = i + len
      if child.has("text") and pending.at(0).start < end {
        let offset = 0
        while pending.len() > 0 and pending.at(0).start < end {
          let rel = pending.at(0).start - i
          let before = __codly-slice-text(child, offset, rel)
          if before != none {
        ...
        let rest = __codly-slice-text(child, offset, len)
        if rest != none {
          children.push(rest)
        }
      } else {
        // No text splitting possible; insert marks that have passed.
        while pending.len() > 0 and pending.at(0).start <= end {
          children.push(__codly-mark-anchor(pending.at(0).label))
          pending = pending.slice(1)
        }
    ...
    // Any remaining marks go at the end of the line.
    while pending.len() > 0 {
      children.push(__codly-mark-anchor(pending.at(0).label))
      pending = pending.slice(1)
    }
...
#let __codly-raw(
  ...
  show raw.line.where(label: <codly-highlighted>): codly-line.with(
    ...
    (__codly-args.marks.type_check)(if "marks" in extra {
      extra.marks
    } else {
      state("codly-marks", __codly-args.marks.default).get()
    }),
  ...
  let line-leading = (__codly-args.at("line-leading").type_check)(if "line-leading" in extra {
    extra.at("line-leading")
  } else {
    state("codly-line-leading", __codly-args.at("line-leading").default).get()
  })
  let line-gap = (__codly-args.at("line-gap").type_check)(if "line-gap" in extra {
    extra.at("line-gap")
  } else {
    state("codly-line-gap", __codly-args.at("line-gap").default).get()
  })

  if line-leading == none {
    set par(justify: false, first-line-indent: 0pt)
  } else {
    set par(justify: false, first-line-indent: 0pt, leading: line-leading)
  }
  ...
  if highlighted-lines == none {
    ...
    if highlighted-lines.len() > 0 {
      let line-colors = (:)
      for l in highlighted-lines {
        if type(l) == int {
          line-colors.insert(l, highlighted-default-color)
        } else if type(l) == array {
          assert(l.len() == 2, message: "codly: a highlighted line definition must be an integer or an array of two elements (length mismatch)")
          let ln = l.at(0)
          ...
          if type(val) == int {
            ...
            let end = val
            if end < start {
              panic("codly: highlighted-lines range end must be >= start")
            }
            for i in range(start, end + 1) {
              line-colors.insert(i, highlighted-default-color)
            }
          } else {
            // Single line with explicit color.
            assert(
              type(val) == color or type(val) == gradient or type(val) == pattern,
              message: "codly: the type of a `highlighted-lines` color must be either a color, a gradient, or a pattern, found: " + str(type(val))
            )
            line-colors.insert(ln, val)
          }
        } else {
  ...
  // Get the height of an individual line (allow custom line-leading).
  let base-line-height = if line-leading == none {
    measure(raw.line(1, 1, "X", [X])).height
  } else {
    line-leading
  }
  let line-height = base-line-height + padding.top + padding.bottom

  let items = ()
  let lines_to_number = ()
  let height = if line-leading == none {
    measure[1].height
  } else {
    line-leading
  }
  ...
  let marks = state("codly-marks").get()
  if marks != none and marks != () {
    state("codly-marks").update(())
  }
}
