...
/// - marks (array, none, function): marks
...
#let codly(
  ...
  line-leading: __codly-default,
  line-gap: __codly-default,
  ...
  if line-leading != __codly-default {
    out.insert("line-leading", line-leading)
  }
  if line-gap != __codly-default {
    out.insert("line-gap", line-gap)
  }
  if line-leading != __codly-default {
    out.insert("line-leading", line-leading)
  }
  ...
  if marks != __codly-default {
    out.insert("marks", marks)
  }
...
// Generate start/end marks for a line range.
#let codly-mark-range(prefix, start, end) = {
  let out = ()
  for i in range(start, end + 1) {
    out.push((line: i, start: 0, label: label(prefix + "-l" + str(i) + "-s")))
    out.push((line: i, start: 999999, label: label(prefix + "-l" + str(i) + "-e")))
  }
  out
}

// Draw a bounding box around all lines in a range using the marks above.
#let codly-bbox-info(prefix, start, end, pad-x: 2pt, pad-y: 1pt) = {
  let ascent = measure(text("X", top-edge: "cap-height", bottom-edge: "baseline")).height
  let descent = measure(text("X", top-edge: "baseline", bottom-edge: "descender")).height
  let xs = ()
  let ys = ()
  let page = none

  for i in range(start, end + 1) {
    let ls = label(prefix + "-l" + str(i) + "-s")
    let le = label(prefix + "-l" + str(i) + "-e")
    let ps = locate(ls).position()
    let pe = locate(le).position()
    if ps.page == pe.page {
      if page == none { page = ps.page }
      xs.push(ps.x)
      xs.push(pe.x)
      ys.push(ps.y)
      ys.push(pe.y)
  ...
  let min-x = xs.fold(xs.at(0), (a, b) => calc.min(a, b))
  let max-x = xs.fold(xs.at(0), (a, b) => calc.max(a, b))
  let min-y = ys.fold(ys.at(0), (a, b) => calc.min(a, b))
  let max-y = ys.fold(ys.at(0), (a, b) => calc.max(a, b))

  (
    page: page,
    left: min-x - pad-x,
    right: max-x + pad-x,
    top: min-y - ascent - pad-y,
    bottom: max-y + descent + pad-y,
    cx: (min-x + max-x) / 2,
    cy: (min-y + max-y) / 2,
    ascent: ascent,
    descent: descent,
...
#let codly-bbox-origin-state = state("codly-bbox-origin", (:))

#let codly-bbox-origin(prefix) = context {
  let pos = here().position()
  codly-bbox-origin-state.update((m) => m.insert(prefix, pos))
  none
}

#let codly-bbox-origin-get(prefix) = {
  let m = codly-bbox-origin-state.get()
  if m == none {
    return none
  }
  m.at(prefix, default: none)
}

#let codly-bbox-anchor(info, anchor: "east") = {
  if info == none {
    return none
  }
...
#let codly-bbox-arc(
  ...
  let b1 = codly-bbox-info(prefix-a, start-a, end-a)
  let b2 = codly-bbox-info(prefix-b, start-b, end-b)
  if b1 == none or b2 == none {
    return none
  }
  if b1.at("page") != b2.at("page") {
    return none
  }
  ...
  let a = codly-bbox-anchor(b1, anchor: anchor-a)
  let b = codly-bbox-anchor(b2, anchor: anchor-b)
  if a == none or b == none {
    return none
  }
  ...
  let by = b.at(1) + oy
  let midx = (ax + bx) / 2
  let midy = (ay + by) / 2 - bend
  let arc-stroke = stroke

  place(top + left, dx: 0pt, dy: 0pt)[
    #cetz.canvas(length: 1pt, {
      import cetz.draw: *
      arc-through((ax, ay), (midx, midy), (bx, by), stroke: arc-stroke)
    })
  ]
}

#let codly-bbox(prefix, start, end, stroke-color: green, pad-x: 2pt, pad-y: 1pt) = context {
  import "@preview/cetz:0.3.4" as cetz
  let info = codly-bbox-info(prefix, start, end, pad-x: pad-x, pad-y: pad-y)
  if info == none {
    return none
  }
  ...
  move(dx: dx, dy: dy)[
    #cetz.canvas(length: 1pt, {
      import cetz.draw: *
      rect((0pt, 0pt), (w, h), stroke: stroke-color)
    })
  ]
}
