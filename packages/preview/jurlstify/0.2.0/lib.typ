...
// show-hyphen : bool — only meaningful when `every` is set. Controls the
//                        marker inserted at those forced break opportunities.
//                        true (default): U+00AD SOFT HYPHEN — shows a "-" if
//                        the line breaks there, signalling the run continues.
//                        false: U+200B ZERO WIDTH SPACE — still breakable, but
//                        nothing is shown, avoiding a spurious "-" that a
//                        reader could mistake for a real character of the URL.
  ...
  while i < n {
    ...
    if c in all-breaks {
      ...
      if every != none and word-run >= every and i + 1 < n {
        // soft hyphen (U+00AD) shows a "-" only when the line breaks here;
        // zero-width space (U+200B) breaks just the same but shows nothing.
        if show-hyphen { "\u{00AD}" } else { _zwsp }
        word-run = 0
      }
    }
