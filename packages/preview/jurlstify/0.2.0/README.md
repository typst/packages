...
- **Long-run fallback** — opt-in `every: N` inserts invisible break points every N chars within unbroken letter/digit runs, shown only when the line actually breaks there; `show-hyphen: false` makes those breaks show nothing at all (no spurious `-`)
...
```typst
#import "@preview/jurlstify:0.2.0": jurlstify, jurl, jurlstify-links
...
| `show-hyphen` | `bool` | `true` | Only relevant when `every` is set. `true` → the forced break shows a soft hyphen (`U+00AD`, a visible `-`) when the line breaks there. `false` → the break uses a zero-width space (`U+200B`): still breakable, but nothing is shown, avoiding a `-` that could be mistaken for part of the URL. |
...
// Insert a break opportunity every 8 chars in long unbroken runs
#jurlstify("https://example.com/averylongpathsegmentwithnobreaks", every: 8)

// Same break points, but show nothing at the break (no visible "-")
#jurlstify("https://example.com/averylongpathsegmentwithnobreaks",
  every: 8, show-hyphen: false)
...
- **长串兜底** — 可选 `every: N`，在连续字母/数字段每隔 N 个字符插入不可见的换行机会，仅在该处实际换行时才显示；配合 `show-hyphen: false` 可让这些断点换行时不显示任何字符（不出现多余的 `-`）
...
| `show-hyphen` | `bool` | `true` | 仅在设置了 `every` 时有意义。`true` → 在该处换行时显示软连字符（`U+00AD`，可见的 `-`）。`false` → 改用零宽空格（`U+200B`）：同样可断行，但不显示任何字符，避免出现可能被误认为 URL 一部分的 `-`。 |
...
// 每隔 8 个字符插入换行机会，处理长段无断点路径
#jurlstify("https://example.com/averylongpathsegmentwithnobreaks", every: 8)

// 相同断点，但换行处不显示任何字符（不出现可见的 “-”）
#jurlstify("https://example.com/averylongpathsegmentwithnobreaks",
  every: 8, show-hyphen: false)
