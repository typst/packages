#import "@preview/modern-hfut-report:0.1.1": *
...
#show: doc => hfut-report(
  ...
  supervisor: "填写你的导师",
  date: "today",
  font-text: ("Times New Roman", "SimSun"), // 正文字体（换字体改这里）
  font-heading: ("Times New Roman", "SimHei"), // 一级标题与图表标题字体
  // font-codeblock: ("DejaVu Sans Mono", "SimSun"), // 默认跟随 font-text 的中文
  show-cover: true, // 封面页
  show-abstract: true, // 摘要页
  show-contents: true, // 目录页
  show-references: true, // 参考文献
  show-appendix: true, // 附录
  doc,
)

#abstract(keywords: ("Typst", "课程设计", "报告模板", "合肥工业大学"))[
...
模板的 Universe 页面（含预览与文档）见脚注#footnote[https://typst.app/universe/package/modern-hfut-report]。

官网中文字体不全，在线编辑体验不佳，建议按下面的方式在本地编辑。
...
- `date`: 日期设置
  - `"today"`: 自动使用今天日期（推荐）
  - `"2025年6月25日"`: 手动指定具体日期
- `font-text`: 正文字体（默认 `("Times New Roman", "SimSun")`）
- `font-heading`: 一级标题与图表标题字体（默认 `("Times New Roman", "SimHei")`）
- `font-codeblock`: 代码块与行内代码字体（默认 `none`，即等宽西文 + `font-text` 的中文）
- `show-cover`: 是否渲染「封面页」（默认 `true`）
- `show-abstract`: 是否渲染「摘要页」（默认 `true`）
- `show-contents`: 是否渲染「目录页」（默认 `true`）
- `show-references`: 是否渲染「参考文献」（默认 `true`）
- `show-appendix`: 是否渲染「附录」（默认 `true`）

换字体改上面的 `font-text` / `font-heading` 即可；字体需先装在系统上（本地也可用 `--font-path` 指定），在线编辑器支持有限。

摘要、参考文献与附录的*内容*都不通过参数传入，而是用模板提供的标记函数写在正文里：
...
`#abstract[..]` 写在正文最前面，由模板在前置页排版（自动居中标题、缩进正文、渲染关键词行）；
二者会自动补上章节标题与分页，位置随书写顺序，且不参与章节编号。
对应开关置 `false` 时，该部分完全不出现在文档中，章号与页码自动重排。
...
=== 正文与标签
- *正文*: 宋体 12pt
- *加粗文字*: 宋体加粗（封面字段名、摘要页"关键词："、正文中的 `*强调*` 等）
...
模板*默认已开启*首行缩进：正文与摘要的每一段都缩进 2 字符，封面、目录以及摘要的关键词行保持顶格。

需要临时顶格时，在 `#[...]` 里 `set` 一下即可（只影响该块）：
...
#figure(
  image("assets/HFUT_badge_zh&en_Vertical.svg", width: 50%),
  caption: "合肥工业大学校徽",
) <fig:校徽>
...
=== 行间公式
带编号的行间公式：
// <>中要写 eqt: 前缀（与图表的 fig: / tbl: 同理）
$ sum_(i=1)^n i = (n(n+1))/2 $ <eqt:求和公式>

引用公式 @eqt:求和公式，我们可以得到...

不带编号的公式：
// 用 numbering: none 显式取消编号
#math.equation(block: true, numbering: none)[$ integral_0^1 x^2 dif x = 1/3 $]
...
```typst
// Typst 代码示例
#text(fill: blue, weight: "bold")[蓝色粗体文字]
#align(center)[居中文字]
```
...
外部依赖：`zh-format`（中文格式化），初始化时自动安装，无需手动配置。
...
本模板托管在 GitHub#footnote[https://github.com/KercyDing/modern-hfut-report]

欢迎通过以下方式参与：

- *报告问题*：遇到 bug 或排版异常，欢迎在 Issues#footnote[https://github.com/KercyDing/modern-hfut-report/issues] 中反馈，最好附上最小复现的 `.typ` 片段和编译日志
...
反馈时会注意：模板以「开箱即用、贴近学校格式要求」为目标，因此对默认样式的改动会比较谨慎，望理解。

= 致谢

本模板参考并借鉴了以下项目的思路，在此致谢：

- modern-xmu-thesis#footnote[https://typst.app/universe/package/modern-xmu-thesis]（HPCesia）：本模板借鉴了该模板的部分思路
- zh-format#footnote[https://typst.app/universe/package/zh-format]（本人）：中文粗体、斜体与下划线的处理
