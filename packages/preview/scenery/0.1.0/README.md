...
The [manual source](manual.typ) is a comprehensive, example-driven showcase where each figure is generated by the exact code shown beside it. Compile it with `typst compile --root . manual.typ`; CI in the [source repository](https://github.com/GiggleLiu/scenery) also publishes the compiled PDF as the `scenery-manual` artifact.
...
which contains the tests, build scripts, and full example gallery omitted from
the downloadable package. From a monorepo checkout:
...
`make test` and `make examples` fan out across all packages and resolve local
`@preview` imports via `TYPST_PACKAGE_PATH`.
