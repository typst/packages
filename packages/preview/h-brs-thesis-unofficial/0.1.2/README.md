...
Set `print-list-of-figures: true` to include the *Abbildungsverzeichnis* in the front matter. Use `figure` with `kind: image` (the default when wrapping an `image()`) and a `caption`:
...
The template loads all abbreviations and expands them on first use in the body. Set `print-abbreviations: true` to render the *Abkürzungsverzeichnis* in the front matter.
