...
/// - fields: Named arguments split between aesthetics and layer parameters. Aesthetic names are `x`, `y`, `xend`, `yend`, `xmin`, `xmax`, `ymin`, `ymax`, `colour`, `fill`, `size`, `alpha`, `shape`, `linetype`, `label`, `group`, `slope`, `intercept`, `nudge-x`, `nudge-y`. For `geom = "text"` and `geom = "label"`, `size` is treated as a layer parameter (the text size, a Typst length) rather than an aesthetic. Anything else (e.g., `stroke`, `fontsize`, `xintercept`, `yintercept`) is forwarded to the geom constructor as a layer parameter. A `none` or `auto` value on an aesthetic name is also forwarded as a layer parameter, so it reaches the geom as the disable / scale-resolve sentinel (e.g., `fill: none` draws an unfilled shape) instead of being stored as a data cell.
///
