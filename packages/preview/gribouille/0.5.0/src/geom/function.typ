...
///
/// The trained x-domain is used by default; pass `x-limits` to override it. `n` samples are taken across that range. Sampled points where `fun` returns `none` are dropped silently.
///
...
/// - x-limits: Optional `(lo, hi)` overriding the trained x-domain.
...
/// ```
///
/// Pass `x-limits` to override the domain when the training data does not match the function's natural range.
///
/// ```typst
