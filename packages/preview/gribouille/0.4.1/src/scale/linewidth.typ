...
///
/// - n-breaks: Number of legend bins. Ignored when `breaks` is set.
/// - breaks: Array of bin edges for the legend, or `auto` to derive equal-width bins from `n-breaks`. Affects the legend only; the per-row mapping stays continuous.
/// - range: Pair of Typst lengths `(min, max)` bounding the output thickness.
/// - name: Legend title. Overrides any name set via `labels` when both are present.
/// - limits: Pair `(lo, hi)` clipping the trained domain, or `none`.
/// - oob: Out-of-range policy: `"drop"` (default) removes rows whose value falls outside `limits`; `"squish"` clamps continuous values to the nearest endpoint.
