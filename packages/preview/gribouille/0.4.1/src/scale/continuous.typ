...
///
/// - name: Axis title. Overrides any name set via `labels` when both are present.
/// - limits: Pair `(lo, hi)` clipping the trained domain, or `none` for automatic limits. Either element may be `auto` to keep the trained bound on that side.
...
/// - minor-breaks: Array of minor gridline positions, or `auto` to derive them automatically (midpoints between major breaks, extended one step beyond each end).
/// - n-minor: Number of minor gridlines between adjacent major breaks when `minor-breaks` is `auto`; `auto` resolves to `1`.
/// - labels: Array of tick labels aligned with `breaks`, or `auto`.
/// - transform: Transformation keyword: `"identity"`, `"log10"`, `"sqrt"`, or `"reverse"`.
/// - expand: Padding around the domain. Accepts a `ratio` (`5%`) for proportional breathing room, a `length` (`5pt`) for canvas-space padding, a `relative` (`5pt + 5%`) for both, or a `(lo, hi)` 2-tuple for asymmetric padding, where either element may be `auto` to keep the per-scale default on that side. `auto` keeps the per-scale default; `false` collapses to zero.
...
///
/// - name: Axis title. Overrides any name set via `labels` when both are present.
/// - limits: Pair `(lo, hi)` clipping the trained domain, or `none`. Either element may be `auto` to keep the trained bound on that side.
/// - oob: Out-of-range policy: `"drop"` (default) removes rows whose value falls outside `limits`; `"squish"` clamps them to the nearest endpoint.
/// - n-breaks: Number of bins to partition the domain into. Ignored when `breaks` is set.
/// - breaks: Array of bin edges, or `auto` to derive equal-width bins from `n-breaks`. Edges define the bin boundaries; ticks sit at each interval midpoint and the domain widens to cover them.
