...
/// - limits: Pair `(lo, hi)` clipping the trained domain (numeric days since 2000-01-01 or ISO-8601 `YYYY-MM-DD` strings; either side may be `auto`), or `none` for automatic limits.
...
/// - breaks: Array of break values (numeric days since 2000-01-01 or ISO-8601 `YYYY-MM-DD` strings), or `auto` for automatic tick selection.
...
///
/// - name: Axis title. Overrides any name set via `labels` when both are present.
/// - limits: Pair `(lo, hi)` clipping the trained domain (numeric seconds since the epoch or ISO-8601 strings; either side may be `auto`), or `none` for automatic limits.
/// - oob: Out-of-range policy: `"drop"` (default) removes rows whose value falls outside `limits`; `"squish"` clamps continuous values to the nearest endpoint.
/// - breaks: Array of break values (numeric seconds since the epoch or ISO-8601 strings), or `auto` for automatic tick selection.
/// - labels: Array of tick labels aligned with `breaks`, or `auto`.
/// - date-format: Typst `datetime.display` pattern used for break labels.
/// - expand: Padding around the domain. Accepts a `ratio` (`5%`) for proportional breathing room, a `length` (`5pt`) for canvas-space padding, a `relative` (`5pt + 5%`) for both, or a `(lo, hi)` 2-tuple for asymmetric padding, where either element may be `auto` to keep the per-scale default on that side. `auto` keeps the per-scale default; `false` collapses to zero.
///
