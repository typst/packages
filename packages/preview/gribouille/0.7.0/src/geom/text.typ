...
///
/// One text block is drawn per row at the mapped `(x, y)`. Offsets are mapped via the `nudge-x` and `nudge-y` aesthetics: a number shifts in data units on a continuous scale and in level units on a discrete one, a Typst length shifts in canvas units. Setting `segment: true` draws a connector back to the anchor point, routed to avoid the other labels of the same layer.
///
/// - mapping: Layer-specific aesthetic mapping built with `aes`. Must map `x`, `y`, and `label`. May map `nudge-x` and `nudge-y` for per-row offsets, in data units on a continuous scale and in level units on a discrete one.
...
/// - font: Label font family. `auto` uses the theme `text` font, then the document font.
...
/// - angle: Rotation applied to each label (a Typst angle, e.g., `45deg`). Positive angles rotate anticlockwise about the anchor.
...
/// - arrow: Arrowhead spec built with `arrow`, drawn on the connector. `ends: "last"` (the default) marks the anchor end, `ends: "first"` the label end. `none` draws a plain connector.
