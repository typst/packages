...
/// - arrow: Arrowhead spec built with `arrow`, marking the direction of each curve. The head is tangent to the sampled bezier. `none` draws no head.
...
///
/// See also: `geom-segment`, `geom-line`, `arrow`.
///
...
/// ```
///
/// A connected scatter read in time order: an `arrow` head on each hop carries the direction whatever the hop is long enough to show.
///
/// ```typst
