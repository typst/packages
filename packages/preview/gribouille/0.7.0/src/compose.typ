...
/// - gutter: Spacing between panels and between the panel block and the shared legend. A length (e.g., `0.5cm`) applies to both axes, or a dictionary `(x:, y:)` sets the column and row gaps independently (a missing key falls back to `0.5cm`).
...
/// - labels: Composition-level labels built with `labels`; only `title`, `subtitle`, and `caption` apply (panel-level labels stay on each `plot`). They reuse the same chrome as a single plot, so a composition reads like one figure.
/// - theme: Theme object such as `theme-grey`, `theme-minimal`, or `theme-classic`, controlling the composition's non-data ink: its labels, the hoisted shared legend, and panel tags. When set, it also propagates into panels that declare no theme of their own (a panel with its own theme keeps it, and a nested composition inherits it recursively); otherwise the theme shared by every panel is used, falling back to the global theme set with `theme-set`, else the default. The hoisted legend is built under this theme, so its key size, its side, and its text all follow the composition. A panel theme styles that panel alone.
...
///
/// See also: `defer`, `plot`, `aes`, `guides`, `labels`.
///
