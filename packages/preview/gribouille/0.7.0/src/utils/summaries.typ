...
///
/// (`none` for unit weights). Forwarded to helpers that honour weights (`mean`, `mean-se`, `mean-sd`, `mean-cl-normal`); helpers without a weighted formulation ignore the parameter.
///
...
/// - weights: Optional array of non-negative weights aligned with `values`
...
///
/// Distinct from `_apply-summary`, which returns a `(y, ymin, ymax)` triple for 1D bands.
///
