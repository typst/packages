...
///
/// Priority order: 1. `params.fill == none` → returns `none` (user disabled the fill). 2. Fixed `params.fill` when it is not `auto`. 3. The fill scale, when `fill-mapping` is `true`, a fill mapping is set, and the fill scale is trained. 4. `default-fill` otherwise.
///
...
///
/// - params: The layer's resolved parameters, providing `fill` and `alpha`.
