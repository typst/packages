...
///
/// Reads the alpha channel back off a colour (`components()` puts it last in every colour space), the inverse question to `apply-alpha`.
///
/// - colour: A resolved colour value or `none`.
///
...
///
/// Priority order: 1. `params.alpha == none` → returns `1` (user opted out of transparency). 2. Pinned `params.alpha` when it is not `auto`. 3. The trained alpha scale (continuous/discrete/identity), if `mapping.alpha` is set. 4. `default-alpha` otherwise (defaults to `1`, geoms with intrinsic translucency pass their own).
///
/// - params: The layer's resolved parameters, providing `alpha`.
...
/// Read `col` through the trained scale of a length-valued aesthetic (`linewidth`, `stroke`, `size`), falling back to `fallback` when the column or scale is missing.
///
/// - aesthetic: Trained-scale key (`"linewidth"`, `"stroke"`, `"size"`).
/// - default-range: Output range when the trained spec sets none.
/// - col: Source column name or `none`.
/// - ctx: Plot context exposing `trained`.
/// - sample-row: The row to read.
/// - fallback: Fallback length.
/// - map-continuous: Override for the continuous branch, `(trained, value, range) => length`; `none` uses `continuous-numeric`.
///
...
///
/// Priority order: 1. `params.linewidth == none` → returns `0pt` (no stroke). 2. Pinned `params.linewidth` when set to a non-`auto` length. 3. The trained linewidth scale, if `mapping.linewidth` is set. 4. Pinned `params.stroke` when set to a length. 5. `theme.geom.linewidth` when the layer's `stroke:` is `auto`. 6. `params.stroke-fallback` when set (used by wrapper layers like `geom-contour` / `geom-quantile` that dispatch through another geom). 7. `default-thickness` otherwise.
///
/// - params: The layer's resolved parameters, providing `linewidth`.
/// - mapping: The resolved aesthetic mapping.
...
///
/// Priority order: 1. Pinned `params.stroke` when set to a length. 2. Pinned stroke dictionary's `thickness` field, when one is supplied. 3. The trained stroke scale, if `mapping.stroke` is set. 4. `default-thickness` otherwise.
///
/// - params: The layer's resolved parameters, providing `stroke`.
/// - mapping: The resolved aesthetic mapping.
/// - ctx: The plot context exposing `trained`.
...
///
/// Priority order: 1. `params.size == none` → returns `0pt` (caller should skip the marker). 2. Pinned `params.size` when set to a non-`auto` length. 3. The trained size scale, if `mapping.size` is set. 4. `default-size` otherwise.
///
/// - params: The layer's resolved parameters, providing `size`.
/// - mapping: The resolved aesthetic mapping.
/// - ctx: The plot context exposing `trained`.
...
///
/// Priority order: 1. `params.colour == none` → returns `none` (user disabled the stroke colour). 2. Pinned `params.colour` when it is not `auto`. 3. The trained colour scale, when `mapping.colour` is set. 4. `default-colour` otherwise.
///
/// Applies the per-row alpha (mapped or pinned) as a transparentise step.
///
/// - params: The layer's resolved parameters, providing `colour`/`alpha`.
/// - mapping: The resolved aesthetic mapping.
