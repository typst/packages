...
///
/// - size: Text size. Either an absolute Typst length (e.g., `12pt`), a ratio (e.g., `80%`) scaling the parent surface size, or `none` to inherit the parent size unchanged. Absolute lengths win outright; ratios cascade proportionally, so setting the base `text` size resizes every surface that inherits via a ratio.
/// - weight: Font weight (e.g., `"regular"`, `"bold"`), or `none` to inherit.
/// - colour: Text colour, or `none` to inherit.
/// - angle: Rotation angle (a Typst angle), or `none` to inherit. Honoured on every text surface: axis tick labels (`axis-text`, seeding the `guide-axis` `angle`, which overrides it), axis titles, strip text, the legend title and entry labels, and the plot title, subtitle, and caption. Axis titles fall back to their natural angle (0deg for x, 90deg for y) when unset.
...
///
/// - size: Text size. Either an absolute Typst length (e.g., `12pt`), a ratio (e.g., `80%`) scaling the parent surface size, or `none` to inherit the parent size unchanged.
...
/// - stroke: Line thickness. Either an absolute Typst length (e.g., `1pt`), a ratio (e.g., `80%`) scaling the parent surface stroke, or `none` to inherit the parent thickness unchanged. Absolute lengths win outright; ratios cascade proportionally, so setting the base `line` stroke rescales every surface that inherits via a ratio. The native `1pt + red` form is also accepted: its paint fills in `colour` unless `colour` is set.
///
/// Returns: Element dictionary consumed by `theme`.
///
...
///
/// Under `coord-radial` the same surfaces drive the angular axis: the marks ring the circle, pointing outward, and are read off whichever scale carries the sweep, `x` on a rose or radar and `y` on a pie.
///
/// - colour: Tick colour, or `none` to inherit.
/// - stroke: Tick thickness. Either an absolute Typst length (e.g., `1pt`), a ratio (e.g., `80%`) scaling the parent surface stroke, or `none` to inherit the parent thickness unchanged. The native `1pt + red` form is also accepted: its paint fills in `colour` unless `colour` is set.
/// - length: Tick mark length, measured outward from the panel edge. Either an absolute Typst length (e.g., `0.2cm`), a ratio (e.g., `50%`) scaling the parent surface length, or `none` to inherit the parent length unchanged. `0cm` hides the marks while keeping their cascade in place.
///
/// Returns: Element dictionary consumed by `theme`.
///
/// See also: `theme`, `element-line`, `element-blank`.
///
...
/// - stroke: Outline thickness. Either an absolute Typst length (e.g., `1pt`), a ratio (e.g., `80%`) scaling the parent surface stroke, or `none` for no outline. Ratios with no absolute ancestor resolve against the default thickness. The native `1pt + red` form is also accepted: its paint fills in `colour` unless `colour` is set.
...
/// ```
///
/// An inside-panel legend keeps its painted background within the panel even flush against a corner.
///
/// ```typst
/// #let d = range(0, 10).map(i => (x: i, y: i * 0.5, k: if calc.rem(i, 2) == 0 { "a" } else { "b" }))
/// #plot(
