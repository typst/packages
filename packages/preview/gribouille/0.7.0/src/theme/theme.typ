...
///
/// - theme: Merged theme dictionary.
/// - surface: Leaf surface key, e.g., `"panel-grid-major-y"`.
/// - until: Ancestor key the walk stops before, e.g., `"panel-grid"`.
///
/// Returns: Boolean.
...
///
/// `auto` (and a partial theme missing the key) yields the library default, Okabe-Ito. An array is used as-is and becomes the render context's fallback, which a scale's own `palette:` still overrides through `spec-palette`. A `none` theme means no theme at all, so it takes the library default too; the per-layer stat paths run themeless.
///
/// - theme: Merged theme dictionary, or `none`.
///
...
///
/// The per-layer `font` wins unless it is `auto` or `none`, in which case the theme default applies (itself possibly `none`). A `none` result tells the consumer to omit the `text(font: ...)` argument so the document font is kept.
///
/// - layer-font: Per-layer `font` parameter (`auto` when unset).
/// - default-font: Theme default font from `resolve-geom-defaults` (`none` when unset).
///
...
///
/// - `"tint"`: `colour-mix(ink, paper, fill-tint-amount)` for the bar / area / rect / tile family;
/// - `"paper"`: the paper role (`geom-boxplot`, `geom-crossbar`, `geom-point`, `geom-label`);
/// - `"ink"`: the ink role (`geom-dotplot`);
/// - `none`: no default fill (`geom-density`), so the absence of a fill survives until the user maps `fill` or pins `element-geom.fill`.
///
/// - defaults: Element-geom record from `resolve-geom-defaults`.
/// - role: Fill role key: `"tint"`, `"paper"`, `"ink"`, or `none`.
///
/// Returns: A colour or `none`.
...
///
/// - theme: Merged theme dictionary.
/// - surface: Tick surface key, e.g., `"axis-ticks-x-bottom"`.
///
...
///
/// The axis band reserves depth from this rather than from the major ticks alone, so a theme that lengthens a sub-decade tier past the major one still has room for it. The cartesian counterpart of the radial `reach`.
///
/// - theme: Merged theme dictionary.
/// - side: Tick surface suffix of the primary edge, e.g., `"x-bottom"`.
/// - axis: `"x"` or `"y"`; the sub-decade tiers have no per-side variant.
///
/// Returns: Tick mark length as an absolute Typst length.
...
///
/// See also: `theme-grey`, `theme-minimal`, `theme-classic`, `theme-void`, `element-text`, `element-line`, `element-tick`, `element-rect`, `element-blank`, `margin`.
///
...
/// ```
///
/// Longer tick marks, tick labels hidden on both axes, and a tinted panel background padded via `element-rect`'s `inset` (inner padding).
///
/// ```typst
/// #let d = range(0, 10).map(i => (x: i, y: i * 0.5))
/// #plot(
