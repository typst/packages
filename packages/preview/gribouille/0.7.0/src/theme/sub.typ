...
/// - ticks: `element-tick` or `element-blank` for `axis-ticks`.
///
...
/// - ticks: `element-tick` for `axis-ticks-x`.
///
...
/// - ticks: `element-tick` for `axis-ticks-y`.
///
...
/// - position: Global legend placement for `legend-position`; accepts the same values as `guide-legend`'s `position` and is overridden by any explicit `guides` placement.
///
