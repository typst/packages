...
/// - position: Where the custom block sits. Same accepted values as `guide-legend.position` (`"top"`, `"right"`, `"bottom"`, `"left"`, `"none"`, a Typst alignment, or a `(dx:, dy:)` / `(x:, y:)` dict). `auto` (default) inherits the side from a `guides(default: ...)` entry or from `theme(legend-position:)` when either is set, otherwise falls back to `"right"`.
