...
///
/// Draws the same ticks as the primary axis but on the top edge for x or the right edge for y, optionally with a different title. Degrades to a no-op under `coord-radial`, which has no opposite edge to draw on.
///
/// - name: Title shown above or beside the secondary axis, or `none`.
/// - breaks: Array of break values in primary units, a function of the trained values returning them (the `breaks-*` helpers return such a function), or `auto` to mirror the primary axis.
/// - labels: Array of labels aligned with `breaks`, a function of the break value, or `auto` to format each break.
///
/// Returns: Secondary axis dictionary consumed by `scale-continuous`.
///
/// See also: `sec-axis`, `scale-continuous`.
///
...
///
/// `transform` is a function mapping a primary-axis value to its secondary-axis value. Use `"identity"` to mirror the primary axis exactly, or pass any callable, e.g., `x => x * 9 / 5 + 32` for Celsius to Fahrenheit. Degrades to a no-op under `coord-radial`, which has no opposite edge to draw on.
///
/// - transform: Function or `"identity"` mapping primary values to secondary values.
/// - name: Title shown above or beside the secondary axis, or `none`.
/// - breaks: Array of break values in primary units, a function of the trained values returning them (the `breaks-*` helpers return such a function), or `auto` to mirror the primary axis.
/// - labels: Array of labels aligned with `breaks`, a function of the transformed break value, or `auto` to format each break.
///
/// Returns: Secondary axis dictionary consumed by `scale-continuous`.
///
/// See also: `dup-axis`, `scale-continuous`.
///
