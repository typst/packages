...
///
/// - title: Override the legend title; `none` keeps the default from labels or scale.
/// - nrow: Number of rows when laying out levels in a grid; `none` for default. Applies to discrete (swatch) and continuous `size` scale legends; a continuous colourbar is a single bar and ignores it.
/// - ncolumn: Number of columns when laying out levels in a grid; `none` for default. Applies to discrete (swatch) and continuous `size` scale legends; a continuous colourbar is a single bar and ignores it.
...
/// - key-size: Diameter of the swatch key glyph as a Typst length (e.g. `0.3cm`); `none` keeps the theme `legend-key` value. Applies to discrete (swatch) legends; a continuous colourbar or size-ladder legend ignores it because its glyph encodes the scale.
///
