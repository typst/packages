...
//
// `integer` forces a whole-number step, for scales whose trained values are
// all whole numbers (years, counts, epoch days): without it a narrow range
// such as 2020..2023 lands on step 0.5 and labels half-years.

#let pretty(lo, hi, n: 5, integer: false) = {
  if lo == hi {
  ...
  // Every `nice * mag` with `mag >= 1` is already whole, so only a sub-unit
  // step needs lifting; `start` then snaps to a whole number on its own.
  let step = if integer { calc.max(1.0, nice * mag) } else { nice * mag }
