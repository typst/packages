...
///   - minor-breaks: Array of minor-gridline positions in data units, a function of the trained values returning positions (the `breaks-*` helpers return such a function), or `auto` (default) to subdivide the majors; a `"log10"` axis uses sub-decade lines instead. `x`/`y` only.
...
/// - args: Named arguments forwarded to the bound scale.
///   - name: Axis or legend title, as a string or content; `none` (default) falls back to the mapped column name.
///   - limits: Array of level names selecting which levels appear and in which order; `none` (default) keeps the trained levels.
...
///   - minor-breaks: Array of minor-gridline positions in data units, a function of the trained values returning positions (the `breaks-*` helpers return such a function), or `auto` (default) to subdivide the majors; a `"log10"` axis uses sub-decade lines instead.
