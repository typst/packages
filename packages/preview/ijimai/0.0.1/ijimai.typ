...
#let ijimai(conf: none, photos: (), logo: none, bib-data: none, body) = {
  ...
  let first-paragraph(content) = {
    let first-space = content.text.position(regex("\s"))
    dropcap(
      height: 2,
      gap: 1pt,
      hanging-indent: 1em,
  ...
  let intro-section = body.fields().at("children").at(1)
  let first-par = body.fields().at("children").at(3)
  intro-section
  body.fields().at("children").at(2)
  first-paragraph[#first-par]
  body.fields().at("children").slice(4, -1).map(x => [#x]).join()[]
