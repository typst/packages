...
#let mainmatter(
  ...
  separator: " ",
  // caption 样式
  caption-style: text.with(font: 字体.宋体, style: "normal", size: zh(5)),
  caption-size: zh(5),
  // figure 计数
  ...
  show table.cell.where(y: 0): set text(font: 字体.黑体)
  // show figure.where(kind: table): set figure.caption(position: top)
  set figure.caption(separator: separator)
  show figure.caption: caption-style
  show figure.caption: set text(size: caption-size, font: 字体.宋体)
  ...
  // 4.  处理标题
  // 4.1 设置标题的 Numbering
  set heading(numbering: numbly("{1:第1章}", "{1:1}.{2:1}", "{1:1}.{2:1}.{3:1}", "{1:1}.{2:1}.{3:1}.{4:1}"))
  ...
  show ref: it => {
    ...
    if (el != none and el.func() == heading and el.level == 1) {
      link(el.location(), text(fill: color.black, numbering(..counter(heading).at(el.location())).trim()))
    } else {
      it
    }
  }
