...
#let letter-preset(
  ...
  assert(client.gender-marker in ("f", "F", "m", "M", "o", "O"), message:  "Gender Marker not recognized. Use only \"[fFmMoO]\"")
...
#let invoice(
  ...
  let t = if lang == "en" {(
    ...
    request-payment: (total-with-vat) => {[Please pay the amount of #format-currency(total-with-vat) until #payment-due-date at the latest to the following account with reference #invoice-number:]},
    payment: (
      recipient: [Recipient:],
      iban: [IBAN:],
      bic: [BIC:],
    ...
    request-payment: (total-with-vat) => {[Es wird um Leistung der Zahlung von #format-currency(total-with-vat) bis spätestens #payment-due-date auf unser Bankkonto unter Angabe der Rechnungsnummer '#invoice-number' gebeten.]},
    payment: (
      recipient: [Empfänger:],
      iban: [IBAN:],
      bic: [BIC:],
