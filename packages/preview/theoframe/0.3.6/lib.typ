// theoframe:0.3.6 A Typst package providing 19 theorem-like environments (e.g., Definition, Theorem, Proof)
...
#let thmbox(trans-supplement: [], kind: "", color: auto, name: [], content) = figure(
  context {
    ...
    if target() == "html" {
      html.elem("div", attrs: (
        class: "thmbox",
        style: "border-left: 5px solid #565656a2; border-radius: 10px;  background: #adadad2f; padding: 0.8em;",
      ))[
        #strong()[#trans-supplement] #fig-number(kind, here()) #str("  ") #name #linebreak() #linebreak()  #content
      ]
    } else {
  ...
  context {
    if target() == "html" {
      html.elem("div", attrs: (
        ...
        #strong()[#trans-supplement] #context fig-number(kind, here()) #str("  ")
