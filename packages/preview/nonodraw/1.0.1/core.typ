...
#let draw-board(
  ...
  block(breakable: false, [
    #grid(columns: (cell-size,) * num-columns, rows: (cell-size,) * height, ..cells)
    // Mass of code for the guide numbers, placed afterwards, in an absolute manner
      ...
      if "left" in guide-number-sides or "right" in guide-number-sides {
        for j in range(guide-number-step, height + 1, step: guide-number-step) {
          ...
          if "right" in guide-number-sides {
            place(left + top, dy: (j - 1 + (col-max-height * if hide-clues { 0 } else { 1 })) * cell-size, dx: cell-size * (width + (if not hide-clues { row-max-width } else { 0 })), block(
              width: cell-size,
              height: cell-size,
