...
/// Bipolar Junction Transistor symbol to use inside a circuit
///
/// - name (str): symbole unique identifier
/// - node (coordinate): symbol position coordinates
/// - polarisation ('npn' | 'pnp'): transistor polarisation
/// - envelope (bool): displays a circle envelope around the symbol
/// -> content
#let bjt(name, node, polarisation: "npn", envelope: false, ..params) = {
    ...
    assert(params.pos().len() == 0, message: "bjt supports only one node")
    ...
    let draw(ctx, position, style) = {
        ...
        line((to: "base", rel: (0, -style.base-distance * sgn)), "e", stroke: cetz.util.resolve-stroke(ctx.style.zap.wire.stroke).thickness, mark: center-mark(
