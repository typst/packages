  ...
  if self.store.navigation == "sidebar" {
    ...
    grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/nudt-vi-nudtgate.png", height: 0.9cm)),
      ),
...
#let slide(
  ...
  let self = utils.merge-dicts(
    self,
    config-page(header: nudt-header, footer: nudt-footer, margin: (x: 1.25cm)),
...
#let title-slide(
  ...
  self = utils.merge-dicts(self, config, config-common(freeze-slide-counter: true), config-page(
    header: align(right + horizon, block(inset: (right: 0.3em, top: 0.3em), image("vi/nudt-vi-logo.svg"))),
  ...
  let body = {
    ...
    set page(background: align(left + bottom, image("vi/nudt-vi-nudtbg.png", width: if self.show-notes-on-second-screen
      == right { 50% } else { 100% })))
...
#let title-slide-green(
  ...
  let body = {
    ...
    set page(background: align(top, image("vi/nudt-vi-nudtphoto.png")))
    ...
    block(fill: self.colors.primary, width: 100%, {
      ...
      grid(
        ...
        align(right, block(inset: (x: 2em), image("vi/nudt-vi-logo-white.svg"))),
      )
      v(2em)
    })
  }
...
#let outline-slide(config: (:), title: utils.i18n-outline-title, ..args) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(self, config-common(freeze-slide-counter: true), config-page(footer: nudt-footer, margin: (
    top: 3em,
  )))
  ...
  self = utils.merge-dicts(self, config-page(
    header: grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/nudt-vi-nudtgate.png", height: 1.5cm)),
      ),
      v(-2cm),
      align(center + horizon, line(length: 100%, stroke: (paint: self.colors.primary, thickness: 1.5pt))),
    ),
...
#let end-slide(config: (:), body) = touying-slide-wrapper(self => {
  ...
  let body = {
    ...
    block(width: 80%, grid(
      columns: (40%, 1fr),
      column-gutter: 0pt,
      image("vi/nudt-vi-logo-ud.svg", width: 80%), body,
    ))
  }
  touying-slide(self: self, config: config, align(horizon + center, body))
})
...
#let end-slide-green(config: (:), body) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(self, config-common(freeze-slide-counter: true), config-page(
    fill: self.colors.primary,
    margin: 1em,
  ))
  ...
  let body = {
    set page(background: align(left, image("vi/nudt-vi-end.png")))
    image("vi/nudt-vi-logo-ud-white.svg", width: 25%)
    v(3fr)
    body
    v(8fr)
  }
...
#let section-suboutline(self) = context {
  ...
  let current = sections.at(current-index)
  let next = if current-index + 1 < sections.len() {
    sections.at(current-index + 1)
  } else {
    none
  }
  let end-page = if next != none { next.location().page() } else { calc.inf }
  let subs = query(heading.where(level: 2))
    .filter(it => it.location().page() >= current.location().page() and it.location().page() < end-page)
    .filter(it => it.outlined != false)
    .filter(it => if it.has("label") {
      let label = str(it.label)
      label != "touying:unoutlined" and label != "touying:hidden"
    } else {
      true
    })
  if subs == () {
  ...
  let bullet = text(fill: self.colors.primary, sym.circle.filled)
  let render-list = list => list.map(hd => {
    [#bullet #h(0.6em) #hd.body]
    linebreak()
  }).sum()
  let split-threshold = 5
  if subs.len() > split-threshold {
    let split-index = calc.ceil(subs.len() / 2)
    grid(
      columns: (1fr, 1fr),
      column-gutter: 2em,
      render-list(subs.slice(0, split-index)),
      render-list(subs.slice(split-index)),
    )
  } else {
    render-list(subs)
  }
...
#let new-section-slide-compact(config: (:), ..args, body) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(self, config-page(
    header: grid(
      inset: (x: 1.9em),
      rows: (auto, auto),
...
///
/// ```typst
/// #show: nudt-theme.with(aspect-ratio: "16-9", config-colors(primary: blue))`
/// ```
///
/// The default colors:
///
