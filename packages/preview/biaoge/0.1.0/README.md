...
| `group-csv` | `summary-table-style-noTotal` | 无总计行的样式汇总表 |
| `group-csv` | `grouped-sum-table` | 二级分组汇总排序表格 |
| `group-csv-tools` | `mergeSelectedColumns` | 合并选中列 |
| `group-csv-tools` | `splitColumnInRows` | 按分隔符拆分行 |
| `group-csv-tools` | `trimString` | 去除首尾空格 |
| `merge-cells` | `merge-col` | 提取某列值 |
| `merge-cells` | `merge-table-data` | 纵向合并相同单元格（数据） |
| `merge-cells` | `merge-table-data-valueCol` | 纵向合并并汇总数值列 |
| `merge-cells` | `merge-table` | 纵向合并相同单元格（表格） |
| `merge-cells` | `merge-table-valueCol` | 纵向合并并汇总（表格） |
