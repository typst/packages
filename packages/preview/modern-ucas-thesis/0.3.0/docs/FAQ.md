...
字体警告多，通常是因为模板自带很长的 fallback 列表，本机缺哪个 family 就报一条。按"英文字体在前、中文字体在后"传入自己的 `fonts`（"宋体""黑体""楷体""仿宋""等宽"），警告基本就没了。
...
单文件即可。Typst 语法简洁，内容集中在一个文件里也容易分辨各部分；编译速度快，不用像 LaTeX 那样拆文件换编译效率；预览窗口支持双向跳转，直接定位源码位置。单文件也便于同步和分享。
