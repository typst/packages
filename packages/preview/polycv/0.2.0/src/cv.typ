...
/// - text-blocks (dictionary): Free-text sidebar/main blocks keyed by name;
///   each value is (title: default key, icon: "circle-info", lines). Each line
///   is a string, or (subtitle: "...") for a bold sub-heading; bulleted: true
///   renders a chevron list. Place a block by its key in sidebar/main-sections.
...
/// - summary-in-header (bool): Render the summary in the header area (any
///   layout: header-band, ats-split or plain), instead of as a main-column
///   section.
/// - contact-in-header (bool): Render the contact line in the header area
///   (any layout) instead of in the sidebar.
/// - paper (str): Page paper size (default "a4"; e.g. "us-letter").
/// - hyphenate (bool): Hyphenate justified text (default true). Set false to
///   avoid mid-word breaks (soft hyphens), e.g. for stricter ATS parsing.
/// - scale (float): Scale the text and spacing (default 1.0). Smaller values
///   shrink fonts and gaps within fixed margins, so more fits per line and
///   per page (the text reflows tighter).
...
#let cv(
  ...
  summary-in-header: false,
  contact-in-header: false,
  ats-split: false,
  paper: "a4",
  hyphenate: true,
  scale: 1.0,
  ...
  // --- Computed layout ---
  // `scale` shrinks the text and spacing (fonts, gaps) by a factor while the
  // page margins and content width stay fixed. A smaller scale therefore fits
  // more per line and more per page: the text reflows tighter, it is not a
  // photocopy. Ratios (sidebar width, photo scale) are unaffected; em gaps
  // follow the scaled font.
  let s = scale
  let scale-lengths = d => d.pairs().map(
    ((k, v)) => (k, if type(v) == length { v.abs * s + v.em * 1em } else { v }),
  ).to-dict()
  let ts = scale-lengths(ts)
  let gap = scale-lengths(gap)
  let tl = scale-lengths(tl)
  let badge = scale-lengths(badge)
  let page-width = if paper == "a4" { 210mm } else { 8.5in }
        ...
        if cfg != none {
          items.push([
            ...
            #link(cfg.url-base + profile.username)[
              #text(size: ts.header-location)[#(cfg.url-base.replace("https://", "") + profile.username)]
            ]
          ])
        }
      }
  ...
  let entry(
    ...
    let entry-header = if entry-inline-meta {
      ...
      [
        // A single-line flow (title + 1fr + meta) rather than a 2-column grid:
        // the meta still sits flush right, but the line stays one linear text
        // run in reading order, which ATS parsers handle far better than a table.
        #text(
          font: ff.entry-text,
          size: ts.entry-text,
          weight: fw.entry-title,
          title,
        )#h(1fr)#meta-parts.join(text(fill: t.accent)[#h(0.4em)·#h(0.4em)])
        #if subtitle != none [
      ...
      if education != none {
        sidebar-section(si.education, st.education)[
          ...
          #for edu in education [
            ...
            #let d = format-date(edu.at("start_date", default: none), edu.at("end_date", default: none))
            #if loc != none or d != "" [
              #text(fill: t.accent)[
      ...
      #show link: set text(fill: t.primary)
        ...
        for profile in profiles {
          ...
          if cfg != none [
            ...
            )[#(cfg.url-base.replace("https://", "") + profile.username)]
            #h(gap.icon-to-text)
          ]
        }
        // Profiles render inline; break to a new line for what follows.
        linebreak()
        v(gap.sidebar-contact-between-items)
      }
      #if address != none {
        if type(address) == array [
          ...
          #fa-icon(address-icon) #h(gap.icon-to-text-tight) #address
        ]
      }
    ],
    skills: () => {
          ...
          #for key in order [
            #let group = skills.at(key, default: none)
              ...
              #v(0pt)
                ...
                // A list of items renders as a chevron list (like hobbies);
                // a single string stays inline.
                icon-list(bullet-icon, group.items, spacing: gap.sidebar-hobbies-between-items)
                v(gap.sidebar-skill-between-items)
              } else [
                #group.items \
                #v(gap.sidebar-skill-between-items)
  ...
  let timeline-entries(entries) = stack(spacing: 0pt, ..entries
    ...
    .map(((i, e)) => entry(
      ...
      format-date(e.at("start_date", default: none), e.at("end_date", default: none)),
  ...
  // --- Free-text blocks (cv.text-blocks) ---
  // User-defined sidebar/main sections: a section title plus free-text lines
  // rendered as-is (normal weight, inline *bold*/_italic_ markup), placed by
  // their key in sidebar-sections / main-sections.
  let block-sidebar = (:)
  let block-main = (:)
  for (key, blk) in text-blocks {
    let icon = blk.at("icon", default: "circle-info")
    let title = blk.at("title", default: key)
    let raw-lines = blk.at("lines", default: ())
    let content = if blk.at("bulleted", default: false) {
      // Chevron list (like hobbies); a {subtitle} degrades to its text.
      icon-list(
        bullet-icon,
        raw-lines.map(l => if type(l) == dictionary { l.subtitle } else { l }),
        spacing: gap.sidebar-hobbies-between-items,
      )
    } else {
      // Each line is a plain string, or {subtitle: "..."} for a bold sub-heading
      // styled like the other sidebar sub-titles (skill-group weight).
      raw-lines.map(line => {
        if type(line) == dictionary and "subtitle" in line {
          text(weight: fw.skill-group, parse-markup(line.subtitle))
        } else {
          parse-markup(line)
        }
      }).join(linebreak())
    }
    block-sidebar.insert(key, () => sidebar-section(icon, title, content))
    block-main.insert(key, () => main-section(icon, title, content))
  }
  let sidebar-renderers = sidebar-renderers + block-sidebar
  let main-renderers = main-renderers + block-main

  // --- Effective sidebar sections ---
  // Sections already shown in the header are excluded from the sidebar.
  let excluded-sections = {
    // Band/ats-split show the photo in the header, so drop it from the sidebar.
    let ex = if show-header-band or ats-split { ("photo",) } else { () }
    // The contact block moves to the header in any layout when requested.
    if contact-in-header { ex += ("contact",) }
    ex
  }
  let effective-sidebar-sections = sidebar-sections.filter(s => (
    s not in excluded-sections
  ))
  let effective-main-sections = if summary-in-header {
  ...
  let build-header-band() = context {
    ...
    let right-cell = [
      ...
      #if summary-in-header and summary != none [
  ...
  set text(
    ...
    weight: fw.body,
    lang: locale,
    hyphenate: if hyphenate { auto } else { false },
  )
