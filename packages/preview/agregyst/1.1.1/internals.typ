...
#let recap() = {
  ...
  context {
    ...
    // everything until the next item.
    let within(sel, elt) = {
      //
    }

    // Build the list notable elements for each of the six columns.
    let recaped-elements = query(recaped.before(here(), inclusive: false))
    let column-elements = ((),) * column-count
    for it in recaped-elements {
              ...
              if it.func() == title {
                ...
                grid(
                  ...
                  item-number(it, width: item-number-width),
                  {
                    ...
                    [ ]
                    // let topic = query(selector.or(strong, emph).after(it.location())).first(default: none)
                    // if topic != none {
                    //   topic.body
                    // }
                    it.caption.body
