...
#let process_arrow(arrow_text) = {
  ...
  if conditions == "" { 
    return if is_reversible { $stretch(#sym.harpoons.rtlb, size: #120%)$ } 
    else { $stretch(->, size: #120%)$ }
  }
  ...
  for cond in conditions {
    ...
    } else if cond.starts-with("T=") or cond.starts-with("P=") or cond.starts-with("t=") or cond.starts-with("p=") {
      bottom.push(cond)
    } else if cond.ends-with("°C") or cond.ends-with("K") or cond.ends-with("atm") or cond.ends-with("bar") {
      bottom.push(cond)
    } else if cond != "" {
      top.push(cond)
    }
  }
  let arrow = if is_reversible { $stretch(#sym.harpoons.rtlb, size: #150%)$ }
  else { $stretch(->, size: #120%)$ } 
  if top.len() > 0 and bottom.len() > 0 { arrow = $#arrow^#top.join(",")_#bottom.join(",")$ }
  else if top.len() > 0 { arrow = $#arrow^#top.join(",")$ }
  else if bottom.len() > 0 { arrow = $#arrow _#bottom.join(",")$ }
  arrow
}
...
#let ce(formula) = {
  ...
  while remaining.len() > 0 {
    ...
    for pattern in patterns {
      ...
      if match != none {
        if pattern == "coef" { result += $#match.text$ }
        else if pattern == "plus" { result += $+$ }
...
#chem("2SO2 + O2 <-V2O5-> 2SO3")

#chem("CH4 + H2O -Ni,T=750°C-> CO + 3H2")
