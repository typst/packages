...
#let title-slide(
  ...
  self = utils.merge-dicts(
    ...
    config-page(fill: none, margin: 0pt),
    config,
  )
  let info = self.info + args.named()
  let body = {
    fullscreen-bg("imgs/bg.png")
    std.align(center + horizon, block(
      width: 100%,
      height: 100%,
      inset: 2em,
      {
        set text(fill: self.colors.title-color)
        ...
        block(
          ...
          {
            if info.logo != none {
              block(spacing: 1.2em, text(2em, utils.call-or-display(self, info.logo)))
            }
            text(size: 1.3em, weight: "medium", info.title)
            if info.subtitle != none {
              block(spacing: .8em, text(size: 0.9em, info.subtitle))
            }
...
///
/// ```typst
/// #show: ist-theme.with(aspect-ratio: "16-9", config-colors(primary: blue))
/// ```
///
/// Consider using:
///
...
#let ist-theme(
  ...
  show: touying-slides.with(
    ...
    config-colors(
      primary: rgb("#1e3782"),
      primary-light: rgb("#bcc6e3"),
      secondary: rgb("#1e3782"),
      neutral-lightest: rgb("#f6f3f3"),
      neutral-dark: rgb("#2e4078"),
      neutral-darkest: rgb("#040e2d"),
