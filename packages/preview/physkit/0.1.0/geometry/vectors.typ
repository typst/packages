...
/// Scalar 2D cross product. Zero means that the vectors are parallel.
#let cross(a, b) = a.at(0) * b.at(1) - a.at(1) * b.at(0)

/// Euclidean magnitude of a 2D vector.
#let magnitude(v) = calc.sqrt(dot(v, v))
...
/// Convert local polar coordinates to a global coordinate.
...
#let normal(start, end) = perpendicular(tangent(start, end))

/// Angle of a vector relative to the positive x axis.
/// Typst's `atan2` explicitly receives `(x, y)`.
#let angle-of(v) = calc.atan2(v.at(0), v.at(1))

/// Distance between two points.
#let distance(a, b) = magnitude(sub(b, a))
