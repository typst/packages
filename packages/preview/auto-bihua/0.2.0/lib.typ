...
///   - "digit"  (default): single string of GB13000 digits, e.g. "汉" → "44154"
///                        (1=横 2=竖 3=撇 4=点 5=折)
///   - "name"   : array of Chinese stroke names, e.g. ("点","点","提","横撇","捺")
///   - "shape"  : array of stroke shape characters, e.g. ("丶","丶","㇀","㇇","㇏")
///   - "letter" : raw cnchar letter encoding, e.g. "kkiel"
///
...
#let bihua-sort(items) = {
  ...
  // Join with U+001F (unit separator) rather than "\n" so items containing
  // newlines survive the round-trip. strs has >= 1 item here, so the result
  // always has >= 1 item too; split unconditionally (a lone "" stays "").
  let sep = "\u{1f}"
  let joined = strs.join(sep)
