  ...
  if plural{
    state("acronyms",none).display(acronyms=>{
      if acr in acronyms{
        ...
        let def = if type(defs) == "string"{ // If user forgot the trailing comma the type is string
          defs
...
#let acr(acr) = {
  ...
  state(state-key,true).display(seen => {if seen{acr}else{[#display-def(plural: false, acr) (#acr)]}})
}
...
#let acrpl(acr) = {
  ...
  let state-key = "acronym-state-" + acr
  // Create a state to keep track of the expansion of this acronym
  state(state-key,true).display(seen => {if seen{acr+"s"}else{[#display-def(plural: true, acr) (#acr\s)]}})
}

#let print-index(level: 1, outlined: false, sorted:"") = {
  ...
  [\ ] // add a space that respects the line size
