#import "@preview/chribel:1.2.0": callout, chribel, chribel-add-callout-template
...
3. Then put the following block before your content to then edit or to see where the fields are all used in the document.
  ...
  #show: chribel.with(
    ...
    titleblock: (title, subtitle, authors, links, paint) => ..
  )
  ```
...
#argument[
  ...
  / subject: #tytyp.dictionary#h(1fr) #default-normal[\(short: #text(red)[none], long: #text(red)[none])]\
  ...
  / font-text: #tytyp.dictionary#h(1fr)\
    An argument/dictionary containing `#text` configuration for paragraph text.
  
  / font-heading: #tytyp.dictionary#h(1fr)\
    An argument/dictionary containing `#text` configuration for headings.
  
  / font-mono: #tytyp.dictionary#h(1fr)\
    An argument/dictionary containing `#text` configuration for codes and enumerated lists.

  / titleblock: #tytyp.function#h(1fr)#default-normal(`chribel.title-block`)\
    A function which builds the titleblock with five parameters. Either pass a function or directly build it using #raw(`(title, subtitle, authors, links, paint) => {..}`.text, lang: "typc")
]
...
#callout(type: "example")[
  ...
  links: (
    ...
    (text: [Touch Grass], link: "http://www.touchgrasss.com/")
  )
  ```
