...
Finde more implementation details in the subsections on the backend
database in @backend-db and caching in @backend-cache.

=== Database <backend-db>

The decision to use a relational database (over a NOSQL) was mainly driven
by the underlaying data structure at hand.

=== Caching <backend-cache>

To improve UX by reducing the average response time, caching was implemented
not only on the client, but also on the server side.
