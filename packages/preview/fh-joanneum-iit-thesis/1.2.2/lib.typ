...
#let quote(message, by) = {
  block(radius: 1em, width: 90%, inset: (x: 2em, y: 0.5em), [
    #message,
    #par(first-line-indent: 25em, text(font: "Inria Serif", size: 9pt, [
      (#by)
    ]))
  ])
}
        ...
        *Exposé*
        ...
        #todo(
          [TODO: Specify the title, subtitle, author, submission date, study, language,
            your name, and supervisor/advisor in the main _expose.typ_ file. Then compile
            with _typst compile expose.typ_. \
            Finally, remove all TODOs (todo marcos) within your typst ource code. ] + align(center + bottom)[Preview printed #datetime.today().display().],
        )
      ],
    )
    set align(left)
