...
```typst
#import "@preview/modern-hust-cse-report:0.1.3": report
...
// 插入图片（原生 figure，自动识别 kind: image）
#figure(image("image.png", width: 80%), caption: "这是图片标题")
// 这将自动显示为：图 1-1-1：这是图片标题

// 插入表格（原生 figure，自动识别 kind: table）
#figure(
  table(
    columns: 3,
    [姓名], [学号], [成绩],
...
模板会通过 `show/set figure` 规则自动给原生 `#figure(...)` 套上所需样式，*不需要也不导出* 任何包装函数。无论图片还是表格，调用方式完全统一：

```typst
#figure(image("path/to/image.png", width: 80%), caption: "图片描述")
...
- **supplement**：图片用「图」、表格用「表」，均为黑体；
- **编号**：`图/表 x-y-z` 格式，x 是一级标题编号、y 是二级标题编号、z 是该节内的序号，每个二级标题下重置；
- **caption 字体**：Times New Roman + 黑体混合，12pt；
- **位置**：图注居图片下方、表注居表格上方（Typst 对 `kind: table` 的默认行为），且不浮动（`placement: none`），避免漂到章节标题之上。
