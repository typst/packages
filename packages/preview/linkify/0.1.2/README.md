...
- Wechat (_gōngzhònghào_ accounts and articles)
...
* Creating links to contents.
    ...
    #moegirl("孤独摇滚！") \
    #moegirl("哈基米") \
    #moegirl("oo是本体") // wildcard characters converted automatically
...
* Added support to link to a specific section for Wikipedia and MoeGirl links.
* `url-as-raw` can now be used as `show: url-as-raw`, no need to say `show link: url-as-raw` any more.

### 0.1.2

* Fixed the bug that the parameters of YouTube videos are incorrectly parsed.
* Added conversion support for wildcard characters `oo` and `xx` in MoeGirl article titles for them to be rendered as `○○` or `××`.
* Added different sources for MoeGirl Wiki to choose from including `zh.moegirl.org.cn` (`zh`), `mzh.moegirl.org.cn` (`mzh`) and `moegirl.icu` (`icu`). This can be controlled by the `src` argument, which is now defaulted to `icu`.
...
* 修复 Youtube 视频链接函数参数传递错误的问题
* 添加支持使萌娘百科文章标题中的通配符 `oo` 和 `xx` 显示时自动转化为 `○○`, `××`
* 萌娘百科现在支持从 `zh.moegirl.org.cn` (`zh`), `mzh.moegirl.org.cn` (`mzh`) 和 `moegirl.icu` (`icu`) 三个站点中选择，通过 `src` 参数控制。`src` 参数默认值现设为 `icu`.