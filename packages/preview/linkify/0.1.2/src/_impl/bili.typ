...
///
/// 检测有效 BV 号的正则表达式。
#let /*pub*/ bv-regex = regex("^(?:(?:B|b)(?:V|v))?[1-9A-HJ-NP-Za-km-z]{10}$")

/// Fails with an error if the input is not a valid BVID.
///
/// 若输入不是有效的 BV 号字符串，则报错。
#let /*pub*/ validate-bv(bvid) = {
...
/// -> str
#let /*pub*/ av2bv(avid, prefix: false) = {
...
#let invalid-video-id-type() = {
  panic("Invalid `video-id` type. Expects `str` or `int`, got `" + repr(type(video-id)) +  "`")
}
...
/// -> str
#let /*pub*/ video-id-fmt(video-id, format: auto, prefix: true) = {
