...
```bash
typst c template/usd.typ example-usd.png --root .
```

This command compiles the template and generates a PNG output. You can also compile to PDF by changing the output extension:

```bash
typst c template/usd.typ output.pdf --root .
```
