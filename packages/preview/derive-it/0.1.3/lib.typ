...
// this may be removed at any time
#let derive-it-internal-validate-line(line, line-num, lineSize: none) = {
  ...
  if ty != array {
    ...
    if (lineSize != none) and (line.len() != lineSize){
      panic(line-text + "Line (array) has an incorrect number of elements: " + str(line.len()) + ". It should have " + str(lineSize) + ": " + stringified)
    } else if lineSize == none and (line.len() != 3 and line.len() != 4) {
...
#let ded-nat(stcolor: black,  arr: array) = context {
  ...
  for (i, line) in arr.enumerate(start: 0) {
    ...
    let isLastIndented = (arr.len() > i+1) and ({
      ...
      derive-it-internal-validate-line(next-line, i+1, lineSize: lineSize)
      next-line.at(ix) 
      } < indent)
