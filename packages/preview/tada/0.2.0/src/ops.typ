...
/// #to-table(agg(td, a: array.sum, b-average: "b.sum() / b.len()"))
/// ```
/// -> TableData
#let agg(
  ...
  /// -> TableData
  td,
  /// Optional overrides to the initial table's field info. This is useful in case an
  /// aggregation function changes the field's type or needs a new display function.
  /// -> dictionary
  field-info: (:),
  /// A mapping of field names to aggregation functions or expressions. Expects a function
  /// accepting named arguments, one for each field in the table. The return value will be
  ...
  let values = (:)
  let cleaned-data = H.dict-from-pairs(td.data.keys().zip(td.data.values().map(arr => arr.filter(v => v != none))))
  ...
  TableData(
    ..td,
    field-info: H.merge-nested-dicts(valid-field-info, field-info),
    data: values,
  )
}
...
#let chain(
  ...
  ///  -> TableData
  td,
  /// A list of table operations. Each operation is applied to the table in sequence.
  /// Operations must be compatible with TableData.
  /// -> TableData
  ..operations,
) = {
...
/// ```example
/// #let td = TableData(data: (a: (1, 2, 3), b: (4, 5, 6)))
/// #to-table(filter(td, expression: "a > 1 and b > 5"))
/// ```
/// -> TableData
#let filter(
  /// The table to filter.
  /// -> TableData
  td,
  /// A boolean expression used to filter rows. The expression can reference fields in the
  /// table and must result in a truthy output.
  /// -> string
  expression: none,
) = {
  ...
  for key in td.data.keys() {
    out-data.insert(
      key,
      td.data.at(key).zip(mask).filter(val-mask => val-mask.at(1)).map(val-mask => val-mask.at(0)),
    )
  }
...
/// -> TableData
#let sort-values(
  /// The table to sort -> TableData
  td,
  /// The field name to sort by -> string
  by: none,
  /// A function that transforms the values of the field before sorting. Defaults to the
  /// identity function if not provided.
  /// -> function
  key: values => values,
  ...
  let indexes = values-and-indexes.sorted(key: idx-vals => idx-vals.at(1)).map(idx-vals => idx-vals.at(0))
  if descending {
    indexes = indexes.rev()
  }
  let sorted-data = (:)
...
/// Aggregated:
/// #to-table(group-by(td, by: "a", aggs: (count: "a.len()")))
/// ```
/// -> array | TableData
#let group-by(
  /// The table to group -> TableData
  td,
  /// The field whose values are used for grouping -> string
  by: none,
  /// A dictionary of aggregations to apply to each group. The keys are the field names
  /// for the resulting table, and the values are the aggregation functions or expressions.
  /// -> dictionary
  aggs: (:),
  /// Optional overrides to the initial table's field info. This is useful in case an
  /// aggregation function changes the field's type or needs a new display function.
