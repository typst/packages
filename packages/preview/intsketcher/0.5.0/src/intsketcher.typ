...
/// Draw a Riemann or Darboux sum of a function, and optionally plot the
/// function.
///
/// - `fn` (function): The function to draw a Riemann sum of.
/// - `domain` (array): Tuple of the domain of `fn`. If a tuple value is `auto`,
///   that value is set to `start`/`end`.
/// - `start` (number): The starting point for the bars. Used only if
///   `partition` is not a valid array; otherwise, the first value of
///   `partition` is used.
...
/// - `n` (number): Number of bars. Used only if `partition` is not a valid
///   array; otherwise, the number of bars is determined by the length of
///   `partition`.
/// - `method` (string): Determines where the sample points for bar heights are
///   taken (`left/right`, `mid`/`midpoint`, or `lower/upper`). Used only if
///   `tags` is not a valid array; otherwise, bar heights are taken from `tags`.
/// - `sample-points` (number): (optional) Number of sample points in a
///   subinterval for Darboux sum illustration, default to `50`.
/// - `partition` (array, none): (optional) Array of partition points. If valid,
///   it overrides `start`, `end`, and `n`; otherwise, equal partitions are
///   generated from `start`, `end`, and `n`.
/// - `tags` (array, none): (optional) Array of sample points for bar heights.
///   If valid, it overrides `method`; otherwise, sample points are determined
///   by `method`.
...
#let intsketcher(
  ...
  } else if method == "lower" or method == "upper" {
		bar-y = range(0, n).map(k => {
			let a = partition.at(k)
			let b = partition.at(k + 1)
			let xs = range(0, sample-points + 1)
				.map(i => a + (b - a) * i / sample-points)
			let ys = xs.map(fn)
			let extreme-y = if method == "lower" {
				ys.fold(ys.at(0), (acc, v) => calc.min(acc, v))
			} else {
				ys.fold(ys.at(0), (acc, v) => calc.max(acc, v))
			}
			// return the x where the extremum is achieved (or midpoint as fallback)
			let idx = ys.position(v => v == extreme-y)
			(xs.at(idx), extreme-y)
		})
  } else {
