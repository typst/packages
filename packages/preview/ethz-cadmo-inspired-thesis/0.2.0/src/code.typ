...
#let multi-code(
  ...
  let highlight-background = background.darken(15%)

  let separator = line(
    length: 100%,
    stroke: (paint: foreground, cap: "round", dash: "dashed"),
  )
  let numpad = "0"
  ...
  let blocks = blocks.map(lines => lines.map(((line, code)) => {
    let highlighted = highlight != none and line == highlight

    (
      if highlighted {
        strong(rraw(padnum(line, width, padding: numpad)))
      } else {
        rraw(padnum(line, width, padding: numpad))
      },
      table.cell(
        code,
        fill: if highlighted { highlight-background } else { none },
      ),
    )
  }))
  ...
  block(
    ...
    #table(
      ...
      align: left,
      stroke: (x, y) => (
        top: if x == 0 { foreground },
        right: if x == 0 { foreground },
      ),
      fill: (x, y) => if x == 0 or (y == 0 and header.len() > 0) {
        foreground
      },
      ..header,
      ..lines.flatten(),
...
#let code(code, start: 1, filename: none, projectname: none, breakable: false, highlight: none) = multi-code(
  ((code, start),),
  filename: filename,
...
#let code-file(content, lang: none, from: none, to: none, filename: none, projectname: none, highlight: none) = {
