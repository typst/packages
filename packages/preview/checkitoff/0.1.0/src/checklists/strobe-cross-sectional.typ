...
// reports of cross-sectional studies. Transcribed from `checklists/
// STROBE_checklist_cross-sectional.docx`, verified line by line against
// a plain-text conversion of the .docx.
//
// Data only: no rendering, no logic. Same 22-item shape as
// `strobe-cohort.typ` (see that file's docstring for why item numbers
// aren't split per lettered sub-point) — items 6, 12(d), 14 and 15 are
// the ones that differ from the cohort/case-control wording, reflecting
// a cross-sectional design's lack of follow-up or matching: item 6 has
// only a single (a) part (no "(b) matched studies" clause at all — cross-
// sectional studies aren't matched), item 14 has only (a)/(b) (no "(c)
// summarise follow-up time", same reason), and item 15 drops "over time"
// from its wording.
#let strobe-cross-sectional = (
  name: "STROBE (cross-sectional)",
  full-name: [STROBE Statement — checklist of items that should be included in reports of cross-sectional studies],
  ...
  items: (
    ...
    (section: "Methods", topic: "Participants", group: none, id: "6",
      description: [(a) Give the eligibility criteria, and the sources and methods of selection of participants]),
