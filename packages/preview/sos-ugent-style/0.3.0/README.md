...
* To get the latest version or to work on the package itself:
  ...
  to `~/.local/share/typst/packages/preview/sos-ugent-style/0.3.0`.
* **Use GUIX**  
  ...
  To get the template with guix, paste the following snippet in a file, for
  example `/tmp/channels.scm`.
  ```guile
  ; Lines starting with a ; are comments in guile (the language used in guix)
  ; File: /tmp/channels.scm
  (list
    (channel
     (name 'guix)
     (url "https://git.guix.gnu.org/guix.git")
     (branch "master")
     ; guix time-machine loads the latest available commit when not specified.
     ; To prevent long loading times, uncomment this line and change this commit
     ; hash to your current guix commit (see `guix describe`)
     ;(commit "aad612c143e19c1a4b64ae066a1fdfbe16c71226")
     )
    (channel
     (name 'sos-ugent-style)
     (url "https://codeberg.org/th1j5/typst-sos-ugent-style")
     (branch "main")
     ; Get the latest template version. If you want the exact same
     ; template as on Typst Universe, uncomment the version tag.
     ; You can also specify a specific commit with the hash.
     ;(commit "v0.3.0")
     (introduction
  ...
  shell typst typst-sos-ugent-style-next -C -- \
  typst init @next/sos-ugent-style
  ```
  If you want the release version, you don't need guix, just use the Typst
  Universe instructions (it still contains everything for a reproducible document).

  If you are on a tagged release (like `v0.3.0`), use `typst-sos-ugent-style`
  in your manifest and `@preview/sos-ugent-style:0.3.0` in `.typ` files.
  ...
  `@next/sos-ugent-style:0.3.0` in `.typ` files.
...
- [ ] GUIX
  - [ ] Ensure `typst init @preview/sos-ugent-style` results directly in a
        reproducible document with Guix.
        Inspiration: See https://codeberg.org/th1j5/ugent-FEARS-2025
    - [x] When on a release version, the template import should be
          `@preview/sos-ugent-style`, but when on any other version, it
          should be `@next/sos-ugent-style`.
    - [x] Add this repository as the `'sos-ugent-style` channel in
          `template/guix/channels-lock.scm`.
          Not needed for development, of course (self-referential).
    - [?] Add the correct manifest & an explanation in the readme.
