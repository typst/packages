...
#let ugent-cover-layout(
  ...
  layout(size => {
    let gridunit = gridunit(size, factor: gridunit-factor)
    state("ugent-gridunit").update(gridunit)
...
#let ugent-dissertation-box(
  ...
  // Taken from ugent-heading-title-text, ugent-heading-rules
  show std.title: upper
  show std.title: underline.with(evade: true, offset: 4pt)
  show std.title: block.with(below: 20pt)
  show std.title: set text(weight: "extrabold", fill: ugent-blue)
  show std.title: set text(size: 25pt)
  show std.title: set par(leading: 0.45em)
  show std.title: set underline(stroke: 2pt)

  std.title()

  if info.subtitle != none {
    set text(weight: "extrabold", fill: ugent-blue)
...
#let ugent-cover-layout-page(..args) = {
  // TODO: set ugent-cover-layout as page(background: ...) and fix margins,
  // such that things like page(columns: 2) work as intended
  // page.* : https://forum.typst.app/t/can-i-access-the-dimensions-of-a-page/344/3
  // open issue on using context in margin: https://github.com/typst/typst/issues/5182
  set page(margin: 0pt)
