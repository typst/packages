  ...
  let header(self) = {
    ...
    pad(
      ...
      stack(dir: ltr,
        ...
        move(dx: -2pt, dy: 22pt,
          if title != auto {
            utils.fit-to-width(grow: false, 98%, title)
          } else {
...
- header (content, function): The header of the slide. Default is `self => utils.display-current-heading(setting: utils.fit-to-width.with(grow: false, 98%), depth: self.slide-level)`.
...
#let inria-theme(
  ...
  header: self => utils.display-current-heading(
    setting: utils.fit-to-width.with(grow: false, 98%),
