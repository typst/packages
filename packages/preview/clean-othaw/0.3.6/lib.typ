...
#let clean-othaw(
  ...
  set page(
    paper: "a4",
    margin: (top: 2.7cm, bottom: 3cm, left: 2.5cm, right: 2.5cm),
    header:
      grid(
        columns: (1fr, 1fr),
        align: (left, right),
 ...
 // ---------- Glossary  ---------------------------------------
  ...
  show heading.where(level: 1): it => {
    ...
    context{
      if in-body.get() {
        ...
        place(              // place heading number prominently at the upper right corner
          ...
          text(counter(heading).display(),
            top-edge: "bounds",
            size: page-grid * 10, weight: 900, luma(130),
          )
        )
      ...
      } else if in-bibliography.get(){
        v(2 * page-grid)
        text(size: h1-size,it.body)
      }
      else {
        v(2 * page-grid)
        text(size: h1-size, counter(heading).display(it.numbering) + h(0.25em) + it.body)   // appendix
      }
    }
  }
...
in-bibliography.update(true)
 // show std-bibliography: set heading(numbering: none)
  ...
  set heading(numbering: it => appendix-name + numbering("1", it))  //show heading: it => block(it)
 ...
 #let appendix-pdf(title, data, scale: 1.0) = {
    heading(level: 1, title)
    block(
      width: 96%,
      muchpdf(data, scale: scale)
    )
  }
