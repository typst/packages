...
#let titlepage(
  ...
  if (type-of-thesis != none and type-of-thesis.len() > 0) {
    align(center, text(weight: "bold", fill: luma(80), size: 2 * page-grid, type-of-thesis))
    align(center, text(size: page-grid, TYPE_OF_THESIS_SECTION_A.at(language)))
  }
  v(0.5 * page-grid)
  // ---------- Author(s) ---------------------------------------
    grid(
      ...
      ..authors.map(author => align(
        center,
        {
          text(weight: "bold", fill: luma(80), size: 2 * page-grid, author.name)
        },
      ))
    )
  ...
  if( not first-titlepage){
    ...
    place(
      ...
      grid(
        ...
        ..if ("university-first" in supervisor) {
          (
            text(
              weight: "bold", fill: luma(80),
              TITLEPAGE_FIRST_SUPERVISOR.at(language) + university-short + [:]
            ),
            if (type(supervisor.university-first) == str) {text(supervisor.university-first)}
          )
        },
        ..if ("university-second" in supervisor) {
          (
            text(
              weight: "bold", fill: luma(80),
              TITLEPAGE_SECOND_SUPERVISOR.at(language) + university-short + [:]
            ),
            if (type(supervisor.university-second) == str) {text(supervisor.university-second)}
          )
        },
        ...
        // company supervisor
        ..if ("company" in supervisor and at-university!=true) {
          (
