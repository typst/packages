...
#let project(
  ...
  let display-study = if study-mode in s-map {
    ...
    text(fill: red, weight: "bold")[无效形式(请填f/p)]
  }
  
  // 1. 基础页面设置 (封面边距)
  set page(
    paper: "a4",
    margin: (top: 2.1cm, bottom: 1.5cm, left: 2cm, right: 2cm),
  )
  ...
  align(center)[
    ...
    #v(0.5cm)
    #text(weight: "bold", size: 26pt)[硕士研究生学位论文中期检查报告]\
    #v(0.0cm)
    #text(size: 15pt, weight: "bold")[Mid-term Examination Report for SJTU Master Student]
    #v(1cm)
  ]
  ...
  align(center)[
    #grid(
      ...
      ..field("姓名", "Name", name),
      ..field("学生类别", "\nDegree Program", v(0.6em)+display-degree+v(0.6em)),
      ..field("学习形式", "\nStudy Mode", v(0.6em)+display-study+v(0.6em)),
  ...
  set page(
    ...
    header: [
      ...
      #align(center)[上海交通大学硕士论文中期检查报告#h(0.5em)Mid-term Examination for SJTU Master Student]
      #v(-0.6em)
  ...
  align(center)[
    ...
    #text(weight: "bold")[填报说明#h(0.5em)Instruction]
    #v(0.5cm)
  ]
  ...
  enum(
    ...
    [
      硕士研究生学位论文中期检查应通过#blue-link("http://my.sjtu.edu.cn/", "数字交大")在线提交申请，填写本表并上传系统。特殊情况下经研究生院事先同意，可不上传系统，并使用《上海交通大学硕士论文中期检查评审表》完成评审。
      #v(0.15em)
      The application for thesis mid-term examination should be submitted online through #blue-link("http://my.sjtu.edu.cn/", "My SJTU"). The student shall filled this form and upload it in the system. Under special circumstance, this form does not need to be uploaded and the review can be proceeded with the review form with prior consent from the graduate school.
    ],
    [
      本报告请用A4纸双面打印，于左侧订在一起。各栏空格不够时，请自行加页。考核前提前一周送交导师、评审专家审阅。
      #v(0.15em)
    ...
    [
      中期检查报告通过后，定稿版报告由研究生、导师各存档一份，无需上传系统。
      #v(0.15em)
  ...
  table(
    ...
    stroke: 0.5pt,
    align: horizon,
    text(font: ("Times New Roman", "FangSong"))[论文题目 \ Thesis Title], 
    text(font: ("Times New Roman", "FangSong"))[#thesis-title],
    ...
    text(font: ("Times New Roman", "FangSong"))[论文开题日期 \ Thesis Proposal Date],
    text(font: ("Times New Roman", "FangSong"))[#thesis-proposal-date]
  )
