...
#let indent-area = 0in           // Area headings have no indent
#let indent-category = 0.5in
#let indent-id = 0.75in
#let indent-note = 1in
#let indent-note-step = 0.25in   // how far #info sits in from its heading
...
#let heading_4_color = rgb("#000000")
...
#let toc() = {
  ...
  show outline.entry.where(level: 3): it => text(fill: id-color.lighten(25%).desaturate(50%), it)
   show outline.entry.where(level: 4): it => text(fill: heading_4_color.lighten(45%).desaturate(50%), it)
  outline(title: none, indent: auto)
  colbreak()
}

// ---- Leaf notes (info / physloc / subid) ----
#let note(body, fill: black, weight: idx-body-weight, style: "normal", indent: indent-note) = pad(left: indent)[
  #set text(font: idx-font, size: size-note, fill: fill, weight: weight, style: style)
  #box[•~#body]
]

// Shared helper: looks up the last heading before the note and indents
// one step past that heading's indent. #info and #physloc both use it.
#let note-under-heading(body, ..args) = context {
  let hs = query(selector(heading).before(here()))
  let lvl = if hs.len() == 0 { 0 } else { hs.last().level }
  let base = if lvl <= 1 { indent-area }
    else if lvl == 2 { indent-category }
    else if lvl == 3 { indent-id }
    else { indent-note }
  note(body, indent: base + indent-note-step, ..args)
}

#let info(body) = note-under-heading(body)
#let physloc(body) = note-under-heading(body, fill: physloc-color, weight: "bold", style: "italic")
