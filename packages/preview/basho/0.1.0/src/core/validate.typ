...
#let validate-config(config) = {
  ...
  assert(config.font == none or type(config.font) == str, message: "basho: config.font must be a string or none")
  assert(type(config.features) == array, message: "basho: config.features must be an array (e.g. (\"vert\", \"vrt2\"))")
  ...
  if type(config.rendering) == array {
    ...
    assert(false, message: "basho: config.rendering must be an array of rendering modules")
  }

  if type(config.tcy) == array {
    assert(config.tcy.len() > 0, message: "basho: config.tcy must contain at least one TCY module")
    for tcy-mod in config.tcy {
      assert("filter" in tcy-mod, message: "basho: each config.tcy entry must have a 'filter' function")
      assert(type(tcy-mod.filter) == function, message: "basho: each config.tcy.filter must be a function")
      assert("pattern" in tcy-mod, message: "basho: each config.tcy entry must have a 'pattern' field (regex)")
      assert("sizes" in tcy-mod, message: "basho: each config.tcy entry must have a 'sizes' field (array of lengths)")
    }
  } else {
  ...
  if type(config.kinsoku) == dictionary {
    assert("resolve" in config.kinsoku, message: "basho: config.kinsoku must have a 'resolve' function")
    assert(type(config.kinsoku.resolve) == function, message: "basho: config.kinsoku.resolve must be a function")
  } else {
    assert(false, message: "basho: config.kinsoku must be a dictionary with resolver settings")
  }
  ...
  if type(config.sizing) == dictionary {
    assert(type(config.sizing.char-box) == length, message: "basho: config.sizing.char-box must be a length (e.g. 1em)")
  }
