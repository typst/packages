...
#let code-fichier(
  ...
  let lignes-extraites = ()
  for i in range(debut-index, fin-index) {
    if i < total {
      lignes-extraites.push(lignes.at(i))
    }
  }

  let code-extrait = lignes-extraites.join("\n")
  let premier-numero = debut
...
#let photo(largeur: 15%, nom: none) = {
  align(center)[
    #image("../template/src/assets/picture/logo-photo.png", width: largeur)
