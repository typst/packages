...
#let course-info(info, fill: lightgreen) = {
  ...
  set grid(
    ...
    stroke: none,
    align: (x, y) => if (y <= 0) and (x != 2 and x !=1 and x != 5) { center + top}
      else { left },
        row-gutter: 8pt,
    column-gutter: 1pt,
...
let course = text[#ci.course.slice(0,4) #ci.course.slice(4,7)] // inserting " "
  grid(
    ...
    // rect(width: 100%,stroke:none)[#smcps[#ci.title]],
     rect(width: 100%,stroke:none)[#text[#eval(ci.title, mode: "markup")]],
    rect(width: 100%,stroke:none)[#smcps[#ci.points pts]],
    rect(width: 100%,stroke:none)[#smcps[#tris]],
...
let vuw-font = "Inter"
let vuw-font = "Fira Sans"
let vuw-font2 = "Inter"
let vuw-font2 = "Source Sans Pro"

set text(size:9pt, font: vuw-font, weight: 300)
...
set page(width: 210mm, height:297mm, margin:1.72cm, background: rect(fill:white,width:100%,height:100%),   
   ...
   footer: [
    #set align(center)
    #set text(8pt, number-type: "old-style",weight: 400,font: vuw-font)
#show grid.cell.where(y: 0): set text(size:9pt, font: vuw-font, weight: 300)
 #set grid(
     fill: white,
    stroke: none
  )
    ...
    align(center)[#context counter(page).display((num, total) => [#set text(number-type: "lining");#num ⁄ #total], both: true)],align(right)[#author]
  )])
    ...
    set text(font: vuw-font, size:11pt,
         weight: 600, tracking: 0.5pt, number-type: "old-style", fill:  black)
     smallcaps(lower(it))
     v(0.51em)}

    show heading.where(level: 3): it => {
    set text(font: vuw-font,  size:9pt,
...
v(1.5cm)
align(top + left)[
  #set text(font: vuw-font2, size: 48pt, weight: "light", fill: darkgreen)
...
show "GEOG": smallcaps[#text[geog]]
show "ESCI": smallcaps[#text[esci]]
show "QUAN": smallcaps[#text[quan]]
show "SCIS": smallcaps[#text[scis]]
show "BIOL": smallcaps[#text[biol]]
show "BMSC": smallcaps[#text[bmsc]]
show "BTEC": smallcaps[#text[btec]]
