/// Source: #import "@preview/shadowed:0.3.0": shadow
/// Modified to remove alt.
...
#let frame(
  ...
  size => {
    // Return only the body if no stroke is specified
    if (stroke == none) {
      body
    }
    ...
    block(breakable: false)[
      ...
      #place(center + horizon)[
        #box(width: width, height: height, radius: radius, stroke: stroke)
      ]
    ]
  },
)
