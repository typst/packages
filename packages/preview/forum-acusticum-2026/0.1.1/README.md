...
- `correct-font`: A boolean indicating whether the correct fonts are used, defaults to `true`. This parameter is intended only for the next iteration of Forum Acusticum to require user confirmation that the correct fonts are used.
- `bibliography`: A call to the `bibliography` function with the path to the `.bib` file as an argument

## Basic Example

```typst
#import "@preview/forum-acusticum-2026:0.1.1": fa2026
