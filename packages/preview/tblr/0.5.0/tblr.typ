...
#let expand-position(x, rng, extras: (:)) = {
  ...
  if is-type(x, "span") {
    return range(
      expand-position(x.start, rng, extras: extras).at(0),
      1 + expand-position(x.stop, rng, extras: extras).at(0),
      step: x.step,
    )
  }
  if type(x) == array {
...
#let split-content(
  ...
  if type(x) == str {
    if x.contains(marker) {
      let p = x
        .matches(marker)
        .map(y => if split == "before" { y.start } else { y.end })
        .reduce((a, b) => if direction == ltr { calc.min(a, b) } else {
          calc.max(a, b)
        })
      return (
        x.slice(0, p),
        x.slice(p + if hide { marker.len() } else { 0 }, x.len()),
      )
    } else {
      return (x, none)
    }
  ...
  if "html" in dictionary(std) and target() == "html" {
    ...
    for row in x {
      ...
      for (i, val) in row.enumerate() {
        row-content.push(html.elem(
          "span",
          attrs: (class: "cell-" + repr(alignments.at(i))),
          val,
        ))
      }
...
#let split-and-align(x, format: (), align: ()) = {
  assert(
    align.len() == format.len() + 1,
    message: "Length mismatch: the length of `align` should be one longer than the length of `format`",
  )
...
#let decimal-align(
  ...
  for x in a {
    ...
    if xs-try.at(1) != none {
      ...
      if xs-try.at(1) != none {
        // found last decimal location
        xs = xs-try
      } else {
        xs-try = split-content(
          x,
          direction: rtl,
          hide: false,
          marker: regex("\d"),
...
// `columns`: Same as std.table, but a String argument uses pillar.
...
#let tblr(
  ...
  if type(n.columns) == str {
    let extra-args = pillar.cols(n.columns)
    let extra-named = extra-args.named()
    let extra-pos = extra-args.pos()

    // find column numbers with auto formatting
    let auto-cols = extra-named
      .format
      .enumerate()
      .fold((), (acc, x) => if x.at(1) == auto { acc + (x.at(0),) } else {
        acc
      })
    specs.push(col-apply(auto-cols, decimal-align, within: "body"))
    extra-named.remove("format")
    n = n + extra-named
    content = content + extra-pos
  }
  ...
  for s in specs.rev() {
    if is-type(s, "cells") {
      ...
      for (row, col) in positions {
        ...
        matrix.at(row).at(col).cell-fields = (
          remove(s, ("cells", "sets", "hooks", "within", "_type_"))
            + matrix.at(row).at(col).at("cell-fields", default: (:))
        )
        let cs = matrix
          .at(row)
          .at(col)
          .at("cell-fields", default: (:))
          .at("colspan", default: 1)
    ...
    if is-type(s, "apply") {
      let col-positions = if s.within == "body" {
        expand-positions-by-col(
          s.cells,
          range(header-rows, nrows),
          range(ncols),
          0,
        )
      } else {
    ...
    if is-type(s, "note") {
      ...
      let num = note-fun(if s.num != none { s.num } else {
        numbering(note-numbering, note-num)
      })
  ...
  if remarks != none or notes.len() > 0 {
    if type(n.columns) == array and n.columns.any(x => type(x) == fraction) {
      t = stack(t, v(0.3em), align(left, grid(
        columns: 2,
        ..notes.flatten(),
        [], remarks,
        inset: (y: 0.3em),
      ...
      t = context stack(t, v(0.3em), align(left, box(
        width: measure(t).width,
        grid(
          columns: 2,
          ..notes.flatten(),
          [], remarks,
