#let make-title(name: str) = align(center)[
  #text(size: 20pt, weight: "bold")[#name]
  #v(1em)
]

#let header_left(ident_color: str, header_logo: bool, experiment_name: str, header_path: "") = {
  grid(
    columns: (1fr, auto),
    text(size: 10pt, weight: "bold")[#experiment_name],
    if header_logo {
      if ident_color == "blue" {
        image("./assets/SJTU_header_blue.png", height: 0.5cm)
      } else if ident_color == "red" {
        image("./assets/SJTU_header_red.png", height: 0.5cm)
      } else if ident_color == "shsmu" {
        image("./assets/shsmu_header.png", height: 0.5cm)
      } else if ident_color == "custom" and header_path != "" {
        image(header_path, height: 0.5cm)
      }
    } else {
      []
    },
  )
}

#let header_right(ident_color: str, header_logo: bool, header_path: "") = {
  let headings = query(selector(heading.where(level: 1)).before(here())).last().body

  grid(
    columns: (auto, 1fr),
    if header_logo {
      if ident_color == "blue" {
        image("./assets/SJTU_header_blue.png", height: 0.5cm)
      } else if ident_color == "red" {
        image("./assets/SJTU_header_red.png", height: 0.5cm)
      } else if ident_color == "custom" and header_path != "" {
        image(header_path, height: 0.5cm)
      }
    } else {
      []
    },
    align(right)[#text(size: 10pt, weight: "bold")[#headings]],
  )
}

#let general-layout(
  ident-color: str,
  header-logo: bool,
  experiment-name: str,
  article-fonts: array,
  code-fonts: array,
  header-path: "",
  content,
) = {
  counter(page).update(1)
  set page(
    paper: "a4",
    margin: (top: 2.54cm, bottom: 2.54cm, left: 3.18cm, right: 3.18cm),
    header: context {
      if calc.odd(counter(page).get().first()) {
        header_left(
          ident_color: ident-color,
          header_logo: header-logo,
          experiment_name: experiment-name,
          header_path: header-path,
        )
      } else {
        header_right(ident_color: ident-color, header_logo: header-logo, header_path: header-path)
      }
      v(-0.25cm)
      line(length: 100%, stroke: 0.5pt + black)
    },
    footer: context align(center)[
      第 #counter(page).display() 共 #counter(page).final().last() 页
    ],
  )

  set text(font: article-fonts)

  set heading(numbering: "1.1")
  show heading: set block(above: 0.5em, below: 0.5em)
  show heading: it => {
    v(0.4em)
    it
    v(0.4em)
  }

  set par(leading: 1em, spacing: 1em, first-line-indent: (amount: 2em, all: true))

  set list(indent: 2em, body-indent: 0.25em)
  set enum(indent: 2em, body-indent: 0.25em)

  set table(stroke: (x, y) => (
    top: if y == 0 { 1pt } else if y == 1 { 0.5pt } else { 0pt },
    bottom: 1pt,
  ))

  show figure.where(kind: table): it => {
    align(center)[
      #v(0.4em)
      #if it.caption != none [
        #text(size: 9pt, weight: "bold")[
          表 #counter(figure.where(kind: table)).display(): #it.caption.body
        ]
        #v(-0.5em)
      ]
      #it.body
      #v(0.4em)
    ]
  }


  show figure.where(kind: image): it => {
    align(center)[
      #v(0.4em)
      #block(it.body)
      #if it.caption != none [
        #text(size: 9pt, weight: "bold")[
          图 #counter(figure.where(kind: image)).display(): #it.caption.body
        ]
      ]
      #v(0.4em)
    ]
  }
  show figure.where(kind: "pseudocode-list"): it => {
    align(left)[
      #v(0.4em)
      #line(length: 100%, stroke: 0.5pt + black)
      #v(-0.75em)
      #if it.caption != none [
        #text(size: 9pt, weight: "bold")[
          算法 #counter(figure.where(kind: "pseudocode-list")).display(): #it.caption.body
        ]
      ]
      #v(-0.75em)
      #line(length: 100%, stroke: 0.25pt + black)
      #v(-1.5em)
      #it.body
      #v(-1.5em)
      #line(length: 100%, stroke: 0.5pt + black)
      #v(0.4em)
    ]
  }

  show raw: it => {
    text(font: code-fonts)[#it]
  }
  show raw.where(block: true): it => {
    line(length: 100%, stroke: 0.5pt + black)
    v(-1.5em)
    align(right)[#highlight(fill: rgb(255, 255, 255))[#text(size: 8pt)[#it.lang]]]
    block(
      it,
    )
    line(length: 100%, stroke: 0.5pt + black)
  }

  content
}

