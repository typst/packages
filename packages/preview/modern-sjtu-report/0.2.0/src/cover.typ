#let make-cover(
  ...
  page(margin: (top: 2cm, bottom: 2cm, left: 2.5cm, right: 2.5cm), header: none, footer: none)[
    ...
    #let org_name = org-name
      ...
      org_name = "SHANGHAI JIAO TONG UNIVERSITY"
    } else if ident-color == "red" {
      image("./assets/SJTU_red.png", width: 8.31cm)
      org_name = "SHANGHAI JIAO TONG UNIVERSITY"
    } else if ident-color == "shsmu" {
      image("./assets/shsmu.png", width: 8.31cm)
      org_name = "SHANGHAI JIAO TONG UNIVERSITY SCHOOL OF MEDICINE"
    } else if ident-color == "custom" and name-path != "" {
      image(name-path, width: 8.31cm)
    }
    
    #v(0.3cm)
    
    #text(size: 14pt, weight: "bold")[#org_name]
    ...
    #v(1cm)
      ...
      image("./assets/shsmu_logo.png", width: 3.2cm)
    } else if ident-color == "custom" and logo-path != "" {
