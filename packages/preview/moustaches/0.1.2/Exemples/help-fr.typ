...
/// 
/// De plus, cette fonction peut être utilisée pour les résolutions d'équations (avec des flèches pour les explications) avec `equation:true` (voir l'exemple du README) ou pour les matrices avec `matrix-mode:true` ou `matrix-mode:"[]"` etc. -> content
#let math-grid(
...
///
/// Reçoit les arguments positionnels et nommés : `body` (content), `colspan`, `rowspan`, `align` et `size`. Tout autre argument nommé (ex: `stroke`, `fill`, `corner-radius`, `extrude`, `def-draw`, etc.) sera transmis directement au nœud Fletcher (`node(...)`) sous-jacent. -> content + metadata
#let mblock(
  /// arguments positionnels (body, colspan, rowspan, align, size) et nommés (colspan, rowspan, align, size, ..node-args) -> arguments
  ..args
) = {}

/// Fonction auxiliaire pour placer un contenu dans une boite sans prendre de place (avec alignement positionnel ou nommé)
#let zero-box(
