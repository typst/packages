...
#let icon-email = box(fa-icon("envelope", solid: true))
...
#let icon-birthdate = box(fa-icon("calendar", solid: true))
...
#let resume(
  ...
  personal-site: "",
  orcid: "",
  custom: (), // List of (text, icon, link) tuples for custom contact
  // ── Intro section ─────────────────────────
  ...
  let custom-items = if custom != none {
    custom.map(item => {
      let text = item.at("text", default: "")
      let icon = item.at("icon", default: none)
      let link = item.at("link", default: "")
  ...
  let items = (
    ...
    contact-item(birthdate, prefix: [#icon-birthdate ]),
    ...
    // Custom items — add more as needed:
    ..custom-items,
  ).filter(x => x != none)
  ...
  //
  // This is still an estimate — use nb-lines-override if it's off.
  let n = items.len()
  ...
  let banner-height = if nb-lines-override != none {
    66pt + nb-lines-override * 18pt
  } else {
