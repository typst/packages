...
// Will probably break at some point, but it's very convenient:
#import "@preview/matofletcher:" + version: (
  arc-placer, circle-placer, edges, place-nodes, placer, tree-placer,
)
...
- `placer()` which allows easily creating placers from a list of positions. This
  should be good enough for most uses. See #link(label("flowchart"))[this
    example]
...
It's relatively easy to create custom placers if needed. See #link(
  label("custom"),
)[here]

== About spread

It appears that fletcher "squeezes" large distances along the left-right axis,
as long as the coordinates in-between are empty. This is why it's useful to
spread out the first generation of children, even by a large factor. Their
children would then occupy the spaces in-between instead of overlapping.
...
#raw(lang: "typst", "#import \"@preview/" + name + ":" + version + "\": *")
...
that is, a function that calculates the relative positions for each child. It
should accept, in order:
