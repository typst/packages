...
#let preamble = "#import \"@preview/sprig:0.2.0\": *
...
#align(center, {
  ...
  v(0.2em)
  text(size: 13pt)[#emoji.hand.write FERGOUS Abdelhak]
  v(0.5em)
  ...
  v(0.6em)
  text(size: 9pt, fill: rgb("#666"))[version 0.2.0 · licence MIT · sans
  aucune dépendance]
})
...
== `theme: "poster"` — le style affiche de classe

#demo("#mindmap([Calcul], theme: \"poster\",
  branch(title: [Vocabulaire], children: (
    branch[Factoriser], branch[Développer])),
  branch(title: [Propriétés])[
    $k (a + b) = k a + k b$
  ])",
  mindmap([*Calcul*], theme: "poster", leaf-width: 2.6, weight: 0.9pt,
    branch(title: [Vocabulaire], child-width: 2.5, children: (
      branch[Factoriser], branch[Développer])),
    branch(title: [Propriétés])[$k (a + b) = k a + k b$]))

#note[
  Cartes blanches à bordure et titre colorés, et toute branche portant des
  enfants devient une pastille pleine à texte blanc — le style des affiches
  de cours dont ce package est né. Disponible aussi sur `mindgrid` et
  `blossom`.
]

== RTL : corps aligné à droite, `body-align`

#demo("#set text(lang: \"ar\")

#mindmap(dir: rtl, theme: \"poster\",
  [الحساب الحرفي],
  branch[تحويل مجموع إلى جداء],
  branch[تحويل جداء إلى مجموع])",
  {
    set text(font: ("DejaVu Sans",), size: 8pt, lang: "ar")
    mindmap(dir: rtl, theme: "poster", leaf-width: 2.6, weight: 0.9pt,
      [الحساب الحرفي],
      branch[تحويل مجموع إلى جداء],
      branch[تحويل جداء إلى مجموع])
  })
...
#note[
  Sous `dir: rtl`, le corps des cartes est désormais *aligné à droite* et
  composé en RTL par défaut ; `body-align:` (`left`, `center`, `right`…)
  le remplace sur `mindmap`, `mindgrid` et `blossom` (centre par défaut
  pour la fleur).
]

== `blossom` — la carte-fleur

#demo("#blossom([Méthode], numbering: \"01\",
  branch(title: [Observer])[Lire l'énoncé.],
  branch(title: [Traduire])[Mettre en équation.],
  branch(title: [Résoudre])[Isoler l'inconnue.],
  branch(title: [Conclure])[Rédiger la réponse.])",
  blossom([*Méthode*], numbering: "01", tint: 30%,
    branch(title: [Observer])[Lire l'énoncé.],
    branch(title: [Traduire])[Mettre en équation.],
    branch(title: [Résoudre])[Isoler l'inconnue.],
    branch(title: [Conclure])[Rédiger la réponse.]),
...
#demo("#blossom([Cycle], petal: \"petal\",
  numbering: (\"α\", \"β\", \"γ\", \"δ\"),
  branch[Aspirer], branch[Comprimer],
  branch[Exploser], branch[Échapper])",
  blossom([*Cycle*], petal: "petal", tint: 26%,
    numbering: ("α", "β", "γ", "δ"),
    branch[Aspirer], branch[Comprimer],
    branch[Exploser], branch[Échapper]),
  s: 45%)
...
#note[
  Un plateau circulaire, une pétale par branche — trois au minimum — et
  jamais de sous-feuilles : une fleur est un visage, pas une hiérarchie.
  `petal: "petal"` arrondit la pétale en goutte ; le texte se centre dans la
  pétale, et `length:` fixe la longueur des pétales quand `radius:` ne
  suffit pas. `numbering: true` chiffre en *chiffres occidentaux* même sous
  `rtl` (la lecture y tourne alors de droite à gauche : 02 à droite) ; un tableau —
  `("★", "✿", "❀")` — remplace les chiffres par des symboles, `false` les
  cache. Les longueurs acceptent aussi `2.1cm` partout où un nombre de
  centimètres est attendu.
]

== Dispositions empilées — `layout`

#demo("#mindmap(layout: \"both\", theme: \"poster\",
  [*Calcul littéral*],
  branch(title: [Vocabulaire], children: (
    branch[Factoriser], branch[Développer],
    branch[Égalité vraie])),
  branch(title: [Pourquoi ?], children: (
    branch[Démontrer], branch[Décrire])),
  branch(title: [Propriétés], children: (
    branch[Distributivité], branch[Commutativité])),
  branch(title: [Comment ?], children: (
    branch[Réduire], branch[Tâtonner])))",
  mindmap(layout: "both", theme: "poster", weight: 0.9pt,
    [*Calcul littéral*],
    branch(title: [Vocabulaire], children: (
      branch[Factoriser], branch[Développer],
      branch[Égalité vraie])),
...
#demo("#mindmap(layout: \"right\",
  [Plan],
  branch(title: [Intro], children: (branch[Accroche], branch[Thèse])),
  branch(title: [Développement], children: (
    branch[Argument 1], branch[Argument 2])),
  branch(title: [Conclusion]))",
  mindmap(layout: "right", weight: 0.9pt,
    [Plan],
    branch(title: [Intro], children: (branch[Accroche], branch[Thèse])),
    branch(title: [Développement], children: (
      branch[Argument 1], branch[Argument 2])),
...
#note[
  `layout: "radial"` (défaut) dispose les branches en anneau. `"both"`,
  `"right"`, `"left"`, `"down"` et `"up"` empilent au contraire les rangs en
  colonnes, à la façon des cartes en arbre : le premier rang face au moyeu,
  les enfants empilés au-delà de leur parent, sans jamais se chevaucher.
  Seule la disposition change — tiges, formes, palettes et thèmes restent
  ceux de sprig. Comme dans l'anneau, chaque carte s'ajuste à son contenu ;
  `leaf-width`, `child-width` et `width` bornent seulement la largeur
  maximale.
]
...
#block(fill: rgb("#F5F3EE"), inset: 9pt, radius: 3pt, width: 100%,
  text(size: 8.5pt)[
    *`mindmap(hub, ..branches, …)`* \
    ...
    `theme` (`auto` / `"print"` / `"poster"`) `shape` `weight` `hub-fill` `hub-ink` `hub-text` `leaf-fill`
    `leaf-ink` `tint` `stalk` `stalk-tip` `bend` `wave` `waves` `start`
    `hub-round` `min-width` `spread` `phase` `radius-leaf` `gap` `shadow` `dir` `body-align` `layout` (`"radial"` / `"both"` / `"right"` / `"left"` / `"down"` / `"up"`) `size` `seed`
    ...
    *`link(from, to, …)`* — `label` `colour` `dash` `arrow` `bend` `via`
    #v(0.4em)
    *`blossom(hub, ..branches, …)`* \
    `petal` (`"wedge"` / `"petal"`) `numbering` (`false` / `true` / motif /
    tableau de symboles / fonction) `palette` `tint` `gap` `inner`
    `hub-radius` `radius` `length` `petal-width` `start` `dir` `body-align` `theme`
    `hub-fill` `hub-ink` `hub-text` `petal-fill` `petal-ink` `petal-text`
    `weight` `shadow` `seed` `rough` `hand` `roughness`
    #v(0.4em)
    *`mindgrid(hub, ..nodes, …)`* \
