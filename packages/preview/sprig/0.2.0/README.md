...
- **`blossom`** — the flower map: a circular plate, one solid petal per
  branch (`"wedge"` or `"petal"`), optional `numbering: "01"`, no sub-leaves.
  Under `rtl` the reading runs right-to-left (petal 02 on the right) and the digits stay Western;
  `numbering` also takes an array of symbols (`("★", "✿", …)`) or a
  function, and `false` / `none` hides it. The text block sits centred in
  its petal, and `length:` sets the petals' radial length when `radius:`
  is not enough.
- **`theme: "poster"`** — the classroom-poster look: white cards with
  coloured borders and titles, and every branch that carries children drawn
  as a solid pill with white text. On `mindmap`, `mindgrid` and `blossom`.
- **RTL alignment** — under `dir: rtl` the body of every card is now
  right-aligned and set in RTL direction by default; `body-align:`
  (`left`, `center`, `right`…) overrides it on all three maps.
- **Stacked layouts** — `layout:` on `mindmap`: besides the default
  `"radial"` ring, `"both"`, `"right"`, `"left"`, `"down"` and `"up"` stack
  the ranks in columns, tree-diagram style — the first rank faces the hub
  and each brood piles up beyond its parent without ever overlapping.
  Only the arrangement changes; stalks, shapes, palettes and themes stay
  sprig's own. As in the ring, every card sizes itself to its contents —
  `leaf-width`, `child-width` and `width` only bound the maximum.
- Distances accept lengths as well as bare centimetres (`5.1cm` → `5.1`).
- The illustrated manual gains a « Nouveautés 0.2.0 » chapter; the bundle
  ships three more complete maps (`carte-mentale`, `blossom`, `mindgrid`).
...
| **modes** | `dir` (`ltr` / `rtl` / auto), `theme: "print"` / `"poster"`, `body-align`, `rough` / `roughness` |
...
* [The illustrated manual](gallery/guide.pdf) — every option, shown, with
  complete code beside it; and its [source code](gallery/guide.typ)
* [The cover](gallery/cover.png) — the map of sprig's own syntax
...
* [`carte-mentale`](examples/carte-mentale.pdf) — the bilingual teaching
  poster (`theme: "poster"`, fr/ar) · [`blossom`](examples/blossom.pdf) ·
  [`mindgrid`](examples/mindgrid.pdf)
