...
#let colors = (
  ...
  "stroke": gradient.linear(angle: 15deg,..color.map.spectral.map(x=>x.lighten(50%))),
  "function": rgb("#f9dfff"),
...
#let show-example(
  ...
  example.show-example(
    ...
    layout: example.default-layout-example.with(
      code-block: block.with(radius: 3pt, stroke: .5pt + luma(200), fill: white),
      preview-block: block.with(radius: 3pt, fill: white, stroke: .5pt + luma(200)),
      col-spacing: 5pt
    ),
  )
}