...
#let wave(
  ...
  /// Renders the tick lines, which uses @-wave.stroke-tick-lines for the stroke styling. If a function with one parameter for the tick number/index is given, allows for more specific showing/hiding of tick lines. As return value only a boolean is accepted!
  /// 
  /// #example(
  ///`#let data = (signal: (
  ///   (wave: "10|uz3x"),
  ...
  /// 
  ///*show-tick-lines: function*
  ///#wave(data, show-tick-lines: (n) => n in (2,3,5,7))`)
  /// 
  /// -> boolean | function
  ...
  /// The stroke styling of the tick lines. If a function is given, it should have a
  /// single parameter representing the tick number and return `dictionary | stroke`. This allows for special stroke styling dependent on its tick position.
  /// 
  /// #example(
  ///`#let data = (signal: (
  ///   (wave: "10|uz3x"),
  ...
  /// 
  ///*stroke-tick-lines: function*
  ///#wave(data, stroke-tick-lines: (n) => {
  ///  if calc.rem(n,2) == 0 {
  ///    (paint: red,)
  ///  } else {
  ///    (paint: blue,)
  ...
  cetz.canvas(length: size, {
    ...
    if type(config.show-tick-lines) == function or (type(config.show-tick-lines) == bool and config.show-tick-lines) {
      // a list of indexes, which determines the visibilty of the tick and its line
      let show-ticks-indexes = if type(config.show-tick-lines) == function {
        range(wave-width + 1).filter(config.show-tick-lines)
      } else if config.show-tick-lines {
        range(wave-width + 1)
      } else {
        ()
      }
      // drawing lines
      for i in show-ticks-indexes {
        let stroke-tick-lines = if type(config.stroke-tick-lines) == function {
          // allows for styling of the tick lines based on its index (for example only show odd or even lines)
          let style = (config.stroke-tick-lines)(i)

          // 'auto' can still be called in a function, which uses the stroke default!
          if style == auto {
            __digidraw-default-config.stroke-tick-lines
          } else {
            style
          }
        } else {
          config.stroke-tick-lines
        }
