#let thesis(
  ...
  align(center, text(13pt)[#smallcaps([#title])])
  ...
  show outline.entry: it => {
    if it.element.at("label", default: none) == <cv_heading> {
    [Curriculum Vitae]
    } else {
    it
    }
  ...
  show heading.where(level: 1): it => {
    ...
    if it.body == [Bibliography] {
      it
    } else if it.body == [Curriculum Vitae] {
      []
    }
    else {
      pagebreak(weak: true)
