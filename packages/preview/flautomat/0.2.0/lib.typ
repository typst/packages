...
#let get-edge-label(labels, automat-type) = {
  ...
  } else if automat-type in ("NKA", "DKA") {
    let format-label(lbl) = {
        let next-letter = lbl.at(1);
        let stack-top = lbl.at(0);
        let push-stack = lbl.at(2).join("");
        
        if next-letter == "" {
            next-letter = sym.epsilon;
        }
        if stack-top == "" {
            stack-top = sym.epsilon;
        }
        if lbl.at(2) == () {
            push-stack = sym.epsilon;
        }
        
        return "(" + stack-top + "," + next-letter + "):" + push-stack;
    }
    return labels.map(format-label).join("\n")
  } else {
    return labels.join(", ")
  }
}
...
if data.type not in ("DEA", "NEA", "MEALY", "MOORE", "TM", "DKA", "NKA") {return "Sorry, this type of automata is not supported."}
