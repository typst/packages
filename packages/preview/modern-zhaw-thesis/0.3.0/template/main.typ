#import "@preview/modern-zhaw-thesis:0.3.0": *
...
#quote(attribution: [Alan Kay, keynote address])[The best way to predict the future is to invent it.]

+ Plan the structure of each chapter
+ Write and revise the main text
  + Draft each section in a separate file and `#include` it from `main.typ`
  + Keep figures and tables near their first reference
    + Prefer vector graphics (SVG) for diagrams
    + Export raster images at sufficient resolution for print
+ Proofread the bibliography and front matter

Here is a footnote.#footnote[
  Footnote text is set at the bottom of the page; numbering restarts each page by default unless you change `footnote` rules globally.
]
...
#callout(
  ...
  + Block quotes, nested numbered lists, and footnotes
]
