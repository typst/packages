...
#let __gls(
  ...
  __mark_term_used(
    key,
    wants_reference,
    force-count-as-first-use or (not force-skip-as-first-use and default-count-as-first-use),
  )
  ...
  let mode = if (
    (requested_mode == "both" or requested_mode == "long") and not long_available
  ) {
    ...
    if current_entries.len() > 0 {
      ...
      let sorted_entries = if sort {
        current_entries
          ...
          .sorted(key: e => e.first()) // NOTE: sorted() is NOT language-aware
