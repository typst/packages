...
#let set-figure-numbering(it, new_format: none) = {
  
  // Set chapter-relative numbering for images
  let image-numbering = super => numbering(new_format, counter(heading).get().first(), super)
  ...
  // Same for tables
  let table-numbering = super => numbering(new_format, counter(heading).get().first(), super)
  ...
  let code-numbering = super => numbering(new_format, counter(heading).get().first(), super)
  ...
  // Same for algorithms
  let algorithm-numbering = super => numbering(new_format, counter(heading).get().first(), super)
  ...
  // Same for equations
  let equation-numbering = super => numbering("("+new_format+")", counter(heading).get().first(), super)
