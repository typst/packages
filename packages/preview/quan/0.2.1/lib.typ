...
///
/// 实现细节：正文 marker 和脚注条目编号默认都被 `super` 包裹，而 `super`
/// 上下文里画圈的横纵比会被压扁。因此用 `show footnote` 和 `show footnote.entry`
/// 自己渲染——手动缩放并 raise，绕开 `super`；画圈复用 `_drawn`，
/// 与 `quan` 的行为完全一致。
/// Implementation: both the inline marker and the entry's number prefix
/// are wrapped in `super` by default, which distorts the drawn-circle
/// proportions. So we use `show footnote` and `show footnote.entry` to
/// render manually (scale + raise), bypassing `super`; the drawn path
/// reuses `_drawn`, matching `quan` exactly.
///
/// 用法 / Usage:\
///   `#show: quan-footnote`
...
#let quan-footnote(body) = {
  // 正文 marker：手动缩放 + raise，绕开 super
  // Inline marker: manual scale + raise, bypassing super
  show footnote: it => context {
    let n = counter(footnote).at(it.location()).first()
    box(baseline: -0.4em, text(size: 0.7em, _quan-marker(n)))
  }
  // 脚注条目编号：用正常上下文渲染，不走 super
  // Entry: render number prefix in normal context, not wrapped in super
