...
/// CeTZ command for a cube. -> function
#let cube(
  /// Position -> array
  a,
  /// auto $->$ (red on the right, green on top, yellow on the left), otherwise one or 3 colors -> auto | color | array
  c:auto,
  /// Lines in black or in the cube color, default false -> boolean | color
  border-stroke:false,
  /// Stroke thickness (from 1pt), default 100% -> int | float | ratio
  thickness:100%,
  /// How much to lighten when using a single color, 20% by default -> float | ratio | array
  light:20%,
  /// Internal parameter for the side view with `pile` -> boolean
  side:false,
) = {}
...
/// the position in the color list (16 max: 1 to 9 and a to g + t for 3 colors).
...
/// // Leftmost slice, from bottom to top
/// ("ttttt","oooooo","oooooo","ttttt"),
/// ("ttttt","oooooo","oooooo","ttttt"),
/// ("ttttt","oooooo","oooooo","ttttt"),
/// ("ttttt","ttttt","ttttt","totot")
/// // Rightmost slice, same arrangement
/// ), y-spread:1)
///```
///
///```example
...
#let building(
  ...
  _back:false,
  /// light:20% -> how much to lighten when using a single color -> float | ratio | array
  light:20%,
  /// Changes the edge size of a base cube -> float | ratio
  draw-scale:.5,
  /// Allows different perspectives:
...
/// Top view for a building.
/// Change text to "Top view" or "" -> content
#let above-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
  text:"Vue du dessus", 
) = {}

/// Right view for a building.
/// Change text to "Right view"  -> content
#let right-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
...
/// Front view for a building.
/// Change text to "Front view"  -> content
#let front-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
...
/// Bottom view for a building.
/// Change text to "Bottom view"  -> content
#let below-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
...
/// Left view for a building.
/// Change text to "Left view"  -> content
#let left-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
...
/// Back view for a building.
/// Change text to "Back view"  -> content
#let back-view(
  /// Arguments for building -> arguments
  ..it,
  /// View name -> str | content
...
/// Random or non-random roll of $n$ dice. 
///
/// ```example
...
/// From eric1102 on Discord: draw one side of a die with the possibility
/// to change the fill color, spot fill, and stroke -> content
#let dice-face(
  /// Int from 1 to 6 -> int
  num,
  /// Color of the dice -> none | color
  fill: none,
  /// Black for a light-colored fill, white for a dark-colored fill,
  /// or a chosen color -> auto | color
  spot-fill: auto,
  /// Stroke of the dice, if a fill is provided and stroke:auto, same as the fill -> auto | stroke
  stroke: auto
) = {}