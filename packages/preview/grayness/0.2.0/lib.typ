...
#let grayscale-image(imagebytes, ..args) = {
  if args.named().keys().contains("format") and args.named().format == "svg" {
    image.decode(plg.svg_grayscale(imagebytes), ..args)
  } else {
...
///  ```
/// - start-x (int): left starting coordinate (in pixels) of the crop window
/// - start-y (int): top starting coordinate (in pixels) of the crop window
/// - crop-width (int): horizontal size (in pixels) of the crop window
/// - crop-height (int): vertical size (in pixels) of the crop window
/// - ..args (any): Arguments to pass to typst image function
///   i.e. width, height, alt and fit
///
///  _Example:_
...
///  #blur-image(data)
///  ```
/// - sigma (int): a measure of how much to blur by (standard deviation)
///
///  _Example:_
///	 ```typst
///  #let data = read("file.webp", encoding:none)
