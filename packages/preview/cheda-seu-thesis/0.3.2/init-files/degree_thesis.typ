#import "@preview/cheda-seu-thesis:0.3.2": degree-conf, degree-utils
...
每一表应有简短确切的题名，连同表号置于表上。必要时，应将表中的符号、标记、代码以及需要说明事项，以最简练的文字，横排于表题下，作为表注，也可以附注于表下。表内附注的序号宜用小号阿拉伯数字并加右圆括号置于被标注对象的右上角，如：×××1），不宜用星号“\*”，以免与数学上共轭的符号相混。目前，本模板暂未实现表内“1）”格式的附注。
...
公式首行的起始位置位于行首算起第五个中文字符之处，即在段落起始行的首行缩进位置再退后两个中文字符。不要用居中、居左或居行首排列。公式编号按阿拉伯数字顺序编号，如（1）（2）……，公式较多时可按章顺序编号，如（1.1）（1.2）……。
...
较短的公式一般一式一行并按顺序编号，后面一式若为前面一式的注解（如下标范围 i=1，3，5，……）可用括号括起来与前面一式并排一行。较长的公式必须转行时，只能在=、≈、+、-、×、÷、<、>处转行。上下式尽可能在等号“=”处对齐。公式中符号尚未说明者应有说明，符号说明之间用分号隔开，一般一个符号占一行。
...
就会自动生成参考文献表。demo 使用的 `ref.bib` 来自 https://github.com/lucifer1004/pkuthss-typst 。
