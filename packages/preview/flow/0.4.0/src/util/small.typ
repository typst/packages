...
/// If running in render mode "all", pass through transparently.
/// If running in render mode "text",
/// transform functions into functions returning `none` and any other data into `none`.
#let _maybe-stub(data) = {
  if cfg.render == "all" {
    return data
  }
