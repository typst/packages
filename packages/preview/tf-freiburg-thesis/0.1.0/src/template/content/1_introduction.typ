...
The template keeps the thesis structure close to a traditional book layout while using Typst's fast preview and readable markup. In the default mirrored book layout, sidenotes appear in the outer page margin, which is useful for short comments, source hints, or reminders that should stay visually connected to a sentence.#sidenote[
  Sidenotes are unnumbered by default. This keeps the margin quiet when the note is only a local aside.
]

If a sidenote should be referenced from the main text, keep a visible marker by setting `numbered: true`.#sidenote(numbered: true)[
...
By the end of this tutorial, you should be familiar with the template's thesis structure, front matter, citations, cross-references, mathematical formulas, figures, tables, and draft helpers.
