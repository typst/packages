...
```typ
typst init "@preview/aero-dhbw:0.4.2" PROJECT-NAME
```
...
| outline-depth | ✗ | 3 | Sets the maximum heading depth shown in the table of contents. |
...
Set up your table styling **before** `#show: aero-dhbw.with(...)`:
...
Styling configured this way applies to every table in your paper — the main
body, the annex, and the AI-usage table — so you only define it once.

The glossary is the exception: it keeps the template's own fixed styling and is
not affected by your table rules.

A plain `set table(...)` or `show table: set table(...)` is fully scoped. A
transformational `show table: it => ...` will still reach the glossary, since
Typst always lets transforms apply.
...
```typ
#import "@preview/aero-dhbw:0.4.2": pa-figure
```
