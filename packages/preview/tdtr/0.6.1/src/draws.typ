...
/// draw additional edges from given edge with metadata matching
#let metadata-match-additional-draw = (nodes, (node, edge), matches: (:)) => {
  // construct mapping from label to node name
  let map = nodes
    .map(n => (collect-metadata(n.label), n))
    .fold((:), (m, (labels, node)) => {
      for label in labels {
