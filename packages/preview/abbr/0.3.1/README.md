...
- Quality-of-Life improvement: reference-style usage
- Optional alternate definitions
...
![Example usage of the abbr package, showcasing some of its features](example.png)
...
### Listing
- **list**`(title, columns)`\
Create an outline with the used abbreviations in short and expanded form.
`title` and `columns` are _optional_ named parameters, defaulting to `"List of
Abbreviations"`, and `2`, respectively.

### Usage
`abbr` includes a show rule that automatically discovers and styles your
