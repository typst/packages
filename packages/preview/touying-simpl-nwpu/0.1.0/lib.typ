  ...
  if self.store.navigation == "sidebar" {
    ...
    grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/nwpu-vi-logo.png", height: 0.72cm)),
      ),
...
#let slide(
  ...
  let self = utils.merge-dicts(
    self,
    config-page(header: nwpu-header, footer: nwpu-footer, margin: (x: 2.5cm)),
...
#let title-slide(
  ...
  self = utils.merge-dicts(self, config, config-common(freeze-slide-counter: true), config-page(
    header: align(right + horizon, block(inset: (right: 0.5em, top: 0.45em), image("vi/nwpu-vi-logo.png", height: 0.85cm))),
  ...
  let body = {
    ...
    set page(background: {
      place(left + bottom, rect(width: 100%, height: 22%, fill: self.colors.primary.lighten(91%)))
      place(right + top, circle(radius: 2.4cm, stroke: self.colors.primary.lighten(72%) + 1.2pt))
      place(right + top, circle(radius: 1.65cm, stroke: self.colors.primary.lighten(82%) + 0.8pt))
    })
  ...
  let body = {
    ...
    block(fill: self.colors.primary, width: 100%, {
      ...
      grid(
        ...
        align(right, block(inset: (x: 2em), image("vi/nwpu-vi-logo-white.png"))),
      )
      v(2em)
    })
  }
...
#let outline-slide(config: (:), title: utils.i18n-outline-title, ..args) = touying-slide-wrapper(self => {
  self = utils.merge-dicts(self, config-common(freeze-slide-counter: true), config-page(footer: nwpu-footer, margin: (
    top: 3em,
  )))
  ...
  self = utils.merge-dicts(self, config-page(
    header: grid(
      ...
      grid(
        ...
        align(right + horizon, image("vi/nwpu-vi-logo.png", height: 1.05cm)),
      ),
      v(-2cm),
      align(center + horizon, line(length: 100%, stroke: (paint: self.colors.primary, thickness: 1.5pt))),
    ),
...
#let end-slide(config: (:), body) = touying-slide-wrapper(self => {
  ...
  let body = {
    set page(background: {
      place(left + bottom, rect(width: 100%, height: 26%, fill: self.colors.primary.lighten(91%)))
      place(right + top, circle(radius: 2.5cm, stroke: self.colors.primary.lighten(75%) + 1pt))
    })
    block(width: 80%, grid(
      columns: (40%, 1fr),
      column-gutter: 0pt,
      image("vi/nwpu-vi-logo.png"), body,
    ))
  }
  touying-slide(self: self, config: config, align(horizon + center, body))
})
...
#let end-slide-red(config: (:), body) = touying-slide-wrapper(self => {
  ...
  let body = {
    align(center, image("vi/nwpu-vi-logo-white.png", width: 42%))
    v(1.8em)
    body
  }
...
///
/// ```typst
/// #show: nwpu-theme.with(aspect-ratio: "16-9", config-colors(primary: blue))`
/// ```
///
/// The default colors:
///
