...
#let page-title-report(
  ...
  set text(normal)
  if doc.school.name != none or doc.school.url != none {[*#safe_link(url:doc.school.url, name:doc.school.name)* \ ]}
  if doc.school.major != none {[#safe_link(url:doc.school.url, name:doc.school.major)]}
  if ((doc.school.major != none and doc.school.orientation != none) or doc.school.url != none) {" - "}
  if doc.school.orientation != none or doc.school.url!= none {[#safe_link(url:doc.school.url, name:doc.school.orientation)]}
  if (doc.school.major != none or doc.school.orientation != none) {[\ ]}

  if doc.course.name != none or doc.course.url != none {[#safe_link(url:doc.course.url, name:doc.course.name)]}
  if ((doc.course.name != none and doc.course.prof != none) or doc.course.url != none) {" - "}
  if doc.course.prof != none or doc.course.url != none {[#safe_link(url:doc.course.url, name:doc.course.prof)]}
  ...
  set text(large)
  if doc.authors.first().name != none {[*#enumerating_emails(names:doc.authors.map(a => a.name), emails:doc.authors.map(a => a.email))*\ ]}
