...
#let plain-text(body) = get.text(get.text(body))
#let with-period(body) = {
  let body-text = plain-text(body).trim()
  body + if body-text.ends-with(".") { [] } else [.]
}

#let caption-emph(body) = {
  let sequence = [].func()
  if type(body) == content and body.func() == math.equation {
    show math.op: it => math.upright(plain-text(it.text))
    body
  } else if type(body) == content and body.func() == sequence {
...
#let get-caption-data(it, otherwise) = {
  ...
  if data.func() != metadata { return (otherwise(it), false) }
...
#let unirfisica(
  ...
  show math.equation.where(block: true): it => {
    show regex("\.$"): $thin #text(font: "Calibri")[.]$
    show regex(",$"): $thin #text(font: "Calibri")[,]$
    it
  }
  ...
  show math.equation: it => {
    set text(font: "Fira Math", size: 11.25pt)
    show regex("\d"): digit => text(font: "Calibri", size: 12pt, digit)
    it
  }
  ...
  */
  ...
  show heading: it => {
    ...
    assert(get.text(it).ends-with(regex("[^\.]")), message: "Los títulos de sección no deben acabar en punto!")
    it
  }
