...
#let style-scholarly = (
  part: (title, num, cfg, show-num) => {
    ...
    block(width: 100%, above: 0pt, below: 0pt)[
      #align(center)[
        #line(length: 38%, stroke: 0.6pt + secondary)
        #v(0.75em)
        #if show-num [
          #text(size: 10pt, fill: secondary, tracking: 0.18em)[#upper(cfg.part-prefix) #part-number(num, cfg)]
          #v(0.5em)
        ]
  ...
  chapter: (title, num, cfg, show-num) => {
    ...
    block(width: 100%, above: 0pt, below: 0pt)[
      #align(center)[
        ...
        #v(0.6em)
        #if show-num [
          #text(size: 10pt, fill: secondary, tracking: 0.15em)[#upper(cfg.chapter-prefix) #chapter-number(num, cfg, fallback: [#numbering("I", num)])]
          #v(0.4em)
        ]
  ...
  section: (title, ch-num, sec-num, cfg, show-num) => {
    ...
    block(width: 100%, above: 0pt, below: 0pt)[
      #text(size: cfg.section-size, weight: "bold", fill: primary)[
        #if show-num [#section-number(ch-num, sec-num, cfg, fallback: [#sec-num.])#h(0.6em)]
        #title
      ]
      #v(-0.15em)
