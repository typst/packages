...
#let labels(
  ...
  /// -> dictionary | none
  sublabels: none,
  /// Whether to show the label and/or sublabel bounds by a thin, gray stroke. Possible values are
  /// `"labels"` for only showing the labek bounds defined in @@sheet(); `"sublabels"` for only
  /// showing the sublabel bounds defined by the `sublabels` parameter; or a `bool` for showing
  /// both/neither. \
  /// Note that this option is intended for debugging or cutting. If your labels should contain a
  /// border with a specific stroke, you should include that in the content passed as `labels`.
  /// -> bool | "labels" | "sublabels"
  border: false,
  /// Deprecated; use the `border` parameter instead.
  /// -> bool
  debug: false,
  ...
  border = z.parse(border, schemas.border)
  ...
  if debug != false and border == false {
    // TODO warn against deprecated usage
    border = debug
  }
  ...
  for label-chunk in labels.chunks(rows * columns) {
    ...
    grid(
      ...
      inset: inset,
      stroke: if border in (true, "labels") { 0.5pt+gray },
      ..label-chunk.map(label => {
        grid(
          rows: sublabels.rows*(1fr,),
          columns: sublabels.columns*(1fr,),
          stroke: if border in (true, "sublabels") { 0.5pt+gray },
          ..label.map(sublabel => {
            show: {
