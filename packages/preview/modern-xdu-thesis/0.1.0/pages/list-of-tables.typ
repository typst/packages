// 西安电子科技大学硕士学位论文 Typst 模板 — 表格索引
//
// 依据：docs/索引部分规格.md §2
// - 页眉「表格索引」宋体 10.5pt 居中（基线 25.82mm）+ 双横线（27.64/28.00mm，宽 155mm）
// - 标题「表格索引」黑体 16pt 居中，基线 44.25mm
// - 条目：`表 <章-序> <题> …… <页码>`，12pt，首条基线 61.12mm，
//   行距固定 20 磅，页码右对齐到 185mm（30~185 为正文宽）
// - 数据来源：query(figure.where(kind: table))；编号为分章编号（格式规格 §2.5）
...
#let list-of-tables(
  ...
  context {
    ...
    show cite: it => none
    let 列表 = query(figure.where(kind: table))
