...
#let mzcborjson-report-spectrum-metadata(one_spectrum) = {
  [
    #mzcborjson-get-spectrum-title(one_spectrum)
    / MS level: #mzcborjson-get-spectrum-ms-level(one_spectrum)
    / index: #one_spectrum.index
    / native id: #one_spectrum.id
    / retention time: #calc.round(mzcborjson-get-spectrum-scan-start-time-in-seconds(one_spectrum),digits:2) seconds (#calc.round(mzcborjson-get-spectrum-scan-start-time-in-seconds(one_spectrum)/60,digits:2) minutes)
    #let precursor-list = one_spectrum.at("precursorList", default: none)
