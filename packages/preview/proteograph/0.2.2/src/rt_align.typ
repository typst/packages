...
#let rtalign-plot(
  ...
  let delta_min = calc.min(..rt_diff)
  let delta_max = calc.max(..rt_diff)

  if (ylim == auto) {
    ylim = (delta_min - 5, delta_max + 5)
  }
    ...
    ..if (ms2-delta-rt != none) {
      (
        lq.scatter(
          ms2-delta-rt.x,
          ms2-delta-rt.y,
          color: rgb("#0074d9").transparentize(90%),
          stroke: 1pt,
          mark: ".",
          label: [MS2 in common],
        ),
