...
#let xic-plot(
  ...
  // 2 defined values : "minutes" or "seconds"
  /// -> string
  rt-units: "seconds",
  hist: (),
  ...
  let x-label = [Retention time (s)]
  if (rt-units == "minutes") {
    x-label = [Retention time (m)]
  }
  ...
  lq.diagram(
    ...
    if (hist.len() > 0) {
      lq.yaxis(
        ...
        lim: (0, 1.2 * calc.max(..(hist.first().y))),
        ..hist
          .enumerate()
          .map(((index, one_hist)) => {
            if ("fill" not in one_hist) {
              one_hist.fill = red.transparentize(20%)
            }
            if ("label" not in one_hist) {
              one_hist.label = none
            }
            let color_index = calc.rem(index, color-cycle.len())
            (lq.bar(one_hist.x, one_hist.y, label: one_hist.label, fill: one_hist.fill))
          }),
      )
    },
    ..xic-item
        ...
        if ("peak-begin" in one_xic) {
          ...
          (
            ...
            lq.fill-between(
              ...
              int_arr,
              label: "peak " + label,
              fill: color-cycle.at(color_index).lighten(80%).transparentize(50%),
            ),
          )
        } else {
          (lq.plot(one_xic.x, one_xic.y, label: label, color: color-cycle.at(color_index).transparentize(20%)))
        ...
        if ("peak-begin" in one_xic) {
          ...
          (
            lq.plot(
              ...
              one_xic.y,
              label: label,
              stroke: (dash: ion_stroke),
              color: ion_color.transparentize(20%),
            ),
            lq.fill-between(
              z-index: -2,
              rt_arr,
              int_arr,
              label: "peak " + label,
              fill: ion_color.lighten(80%).transparentize(50%),
            ),
          )
        } else {
          (
