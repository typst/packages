...
#let ms2spectra-simple-plot(spectra) = {
  lq.stem(spectra.mz, spectra.intensity, color: black, base-stroke: none, mark: none)
}

#let ms2spectra-ion-plot(type, ion-arr) = {
  let mz_arr = ()
  let int_arr = ()
  ...
  (
    ...
    ion-arr.map(one_ion => {
      ...
      if (display) {
        let align = bottom
        if (one_ion.intensity > 0) {
          lq.place(
            clip: true,
            one_ion.mz,
            one_ion.intensity,
            pad(bottom: 0.3em, left: 20%)[#text(ion-colors.at(type), size: 8pt)[#psm-ionlabel(
              type,
              one_ion.size,
              one_ion.charge,
            )]],
          ...
          lq.place(
            ...
            one_ion.mz,
            one_ion.intensity,
            pad(top: 0.1em, left: 20%, box(baseline: (at: top, shift: 0% + 0pt), text(
              ion-colors.at(type),
              size: 8pt,
            )[#psm-ionlabel(
              type,
...
#let ms2spectra-delta-plot(delta-arr) = {
  delta-arr.map(delta => {
    let level = 1em * delta.level + 1em
    (
      lq.line(
        z-index: -1,
        stroke: (paint: ion-colors.at(delta.ion).lighten(75%), dash: "dashed"),
        (delta.mz.at(0), 0),
        (delta.mz.at(0), level),
      ),
      lq.line(
      ...
      lq.place(clip: true, (delta.mz.at(1) + delta.mz.at(0)) / 2, level, pad(bottom: 0.8em)[#text(
        ion-colors.at(delta.ion),
        size: 8pt,
      )[#delta.label]]),
    )
  })
