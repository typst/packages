...
#let mcqjson-get-xic-data(one-peptide, type: "trace") = {
  let isotope_list = ()
  for xic in one-peptide.xics {
    let isotope = ()
    if (type == "trace") {
      if ("trace" in xic) {
        isotope = xic.trace
...
#let write-isotope-summary(one_peptide) = {
  let fnct-rt(rt-peak-list) = {
    for rt in rt-peak-list {
      [#calc.round(rt, digits: 2)#sym.prime.double ]
    }
  }
  let isotope_list = ()
  set text(size: 8pt)
  stack(
    ...
    for xic in one_peptide.xics {
      if (xic.rank == 1) {
        box(width: 11em, baseline: (at: top, shift: 0% + 0pt), inset: (x: 0.5em), [
          #heading(level: 2, "isotope " + str(xic.isotope))
          / mz: #calc.round(xic.mz, digits: 5)
          / charge: #xic.charge
          / th ratio: #calc.round(xic.th_ratio, digits: 2)
          / mz range: #calc.round(xic.xic_coord.mz_range.first(), digits: 3); #sym.dash.em #calc.round(
              xic.xic_coord.mz_range.last(),
              digits: 3,
            );
          / quality: #check-peak-quality(xic.quality)
          #if "tim_im_range" in xic.xic_coord {
            [/ tims range: #xic.xic_coord.tim_im_range.first(); #sym.dash.em #xic.xic_coord.tim_im_range.last();]
          }
          #if "peak" in xic {
            [
              / rt [begin, apex, end]: #fnct-rt(xic.peak.rt)
              #if "aligned_rt" in xic.peak { [/ aligned rt: #fnct-rt(xic.peak.aligned_rt)] }
              / max int: #calc.round(xic.peak.max_intensity, digits: 0)
              / area: #calc.round(xic.peak.area, digits: 0)
            ]
          }
        ])
      }
  ...
  if (peptide-id in mcq-json.quantification_data) {
    ...
    for peptide_measures in mcq-json.quantification_data.at(peptide-id) {
      ...
      [
        ...
        / targeted retention time: #calc.round(peptide_measures.rt_target, digits: 2)#sym.prime.double (#calc.round(
            peptide_measures.rt_target / 60,
            digits: 2,
          )#sym.prime)
        ...
        #let xic-list = mcqjson-get-xic-data(peptide_measures, type: type)
        #if (xic-list.len() > 0) {
          xic-plot(
            height: 10cm,
