...
#let mass-h2o = 18.01056
#let mass-hplus = 1.007276466879
#let aa-mass-map = (
  G: 57.021463721083,
  A: 71.037113785565,
  S: 87.032028405125,
...
/// Generates raw mass array from proforma
/// -> object
#let get-mass-array-from-proforma(
  ...
  for aa_char in string_arr {
    ...
    } else if aa_char == "-" {} else if aa_char == "?" { current_mass_delta = 0 } else if in_delta {
      delta.push(aa_char)
    } else if (aa_char != "") {
...
#let get-mass-from-proforma(
  /// The peptide in proforma string *required*
  ///
  /// -> strings
  proforma,
) = {
  get-mass-array-from-proforma(proforma).sum() + mass-h2o
}

#let get-mz-from-proforma(
  /// The peptide in proforma string *required*
  ///
  /// -> strings
  proforma,
  ...
  (get-mass-array-from-proforma(proforma).sum() + mass-h2o + (mass-hplus * charge)) / charge
}
...
#let get-sequence-from-proforma(
  ...
  for aa_char in string_arr {
    if (aa_char == "[") {
      ...
      in_delta = false
    } else if in_delta {} else if (regex("[A-Z]") in aa_char) {
...
#let get-mass-ion-serie(
  ...
  (
    ion-mass-array.map(((inc-mass)) => {
      (inc-mass + mass-shift + (mass-hplus * charge)) / charge
    })
  )
}
