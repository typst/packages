...
- `proba-tree-short`: draw a probability tree with the Typst list syntax
  (`-` for a node, `+` for the probability of the branch leading to it,
  alternating and nested) instead of nested tuples. All the `proba-tree`
  options (`h`, `v`, `proba-position`, `node-style`, `extra`, …) are
  forwarded; only the construction of `data` changes.
- `proba-tree-short-data`: returns the `data` array alone, so it can be
  reused in `proba-tree`.
